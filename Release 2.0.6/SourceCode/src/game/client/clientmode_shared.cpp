...
static void __MsgFunc_MainGameMenu( bf_read &msg )
{
	int tab = msg.ReadByte();

	//char name[NAME_LENGTH];
	//msg.ReadString(name,sizeof(name));
	int playerKills = msg.ReadLong();
	int playerDeaths = msg.ReadLong();
	int monsterKills = msg.ReadLong();
	int monsterDeaths = msg.ReadLong();
	int sprees = msg.ReadShort();
	int spreeWars = msg.ReadShort();
	int biggestSpree = msg.ReadShort();
	float playTime = msg.ReadFloat();
	char model[MODEL_LENGTH];
	msg.ReadString(model,sizeof(model));
	char created[48];
	msg.ReadString(created, sizeof(created));


	CVortexMenu *pPanel = (CVortexMenu*)gViewPortInterface->FindPanelByName( PANEL_BUY );
	bool bShow = !pPanel->IsVisible();

	//if ( bShow )
		pPanel->UpdateCharacterInfo(model, created, playerKills, playerDeaths, monsterKills, monsterDeaths, sprees, spreeWars, biggestSpree, playTime);

	// if they send a tab, and we aren't showing that tab already, definately SHOW the panel
	if ( tab != NUM_TABS && pPanel->GetActiveTab() != tab )
	{
		pPanel->SwitchToTab(tab);
		bShow = true; // don't close it, cos we want to switch to a particular tab
	}
	
	gViewPortInterface->ShowPanel( PANEL_BUY, bShow );
}

static void __MsgFunc_ShowLogon( bf_read &msg )
{
	int i = msg.ReadByte();
	if ( i == MANUAL_LOG_OUT )
	{
	...
	CMsgPanel::SetNextPanel(PANEL_LOGON);
	switch( i )
	{
	case ACCOUNT_DOESNT_EXIST:
		CMsgPanel::ShowMessage("#acc_nonexistant_title","#acc_nonexistant_body");
		break;
	case ACCOUNT_ALREADY_EXISTS:
		CMsgPanel::ShowMessage("#acc_taken_title","#acc_taken_body");
		break;
	case PASSWORD_INCORRECT:
		CMsgPanel::ShowMessage("#pass_wrong_title","#pass_wrong_body");
		break;
	case ACCOUNT_ALREADY_LOGGED_IN:
		CMsgPanel::ShowMessage("#acc_in_use_title","#acc_in_use_body");
		break;
	default:
		CMsgPanel::ShowMessage("#logon_unknown_title","#logon_unknown_body");
		break;
	}
}

static void __MsgFunc_MakeCharFailed( bf_read &msg )
{
	int i = msg.ReadByte();

	CMsgPanel::SetNextPanel(PANEL_MAKECHAR);
	switch( i )
	{
	case DUPLICATE_NAME:
		CMsgPanel::ShowMessage("#duplicate_charname_title","#duplicate_charname_body");
		break;
	case INVALID_CLASS:
		CMsgPanel::ShowMessage("#char_bad_class_title","#char_bad_class_body");
		break;
	case INVALID_MODEL:
		CMsgPanel::ShowMessage("#char_bad_model_title","#char_bad_model_body");
		break;
	default:
		CMsgPanel::ShowMessage("#makechar_unknown_title","#makechar_unknown_body");
		break;
	}
}

bool g_bJustCameDirectlyFromMakeCharPanel = false; // if i hit cancel on the make char panel, don't jump back to it immediately!
static void __MsgFunc_PickChar( bf_read &msg )
{
	// we need to read stuff here to tell us about their characters!
	int totNumChars = msg.ReadByte();
	int page = msg.ReadByte();
	int numChars = msg.ReadByte();

	if ( numChars == 0 && page == 1 && !g_bJustCameDirectlyFromMakeCharPanel )
	{// skip character selection if there are none
		gViewPortInterface->ShowPanel( PANEL_MAKECHAR, true );
		return; 
	}
	g_bJustCameDirectlyFromMakeCharPanel = false;

	CPickCharPanel *pPanel = (CPickCharPanel*)gViewPortInterface->FindPanelByName( PANEL_PICKCHAR );
	pPanel->ResetCharInfo();
	pPanel->Setup(page, totNumChars);
	...
	for ( int i=0; i<numChars; i++ )
	{
		...
		char model[MODEL_LENGTH];
		msg.ReadString(model,sizeof(model));
		char created[48], lastActive[48];
		msg.ReadString(created,sizeof(created));
		msg.ReadString(lastActive,sizeof(lastActive));

		pPanel->AddCharacterInfo(id,name,model,iLevel, created, lastActive);
	}

	gViewPortInterface->ShowPanel( PANEL_PICKCHAR, true );
}

static void __MsgFunc_Spree( bf_read &msg )
{
	int i = msg.ReadByte();
	if ( i > 0 )
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence("StartSpree");
	else
		g_pClientMode->GetViewportAnimationController()->StartAnimationSequence("StopSpree");
}
...
//	ListenForGameEvent( "achievement_earned" );
	...
	HOOK_MESSAGE( Rumble );
	HOOK_MESSAGE( MainGameMenu );
	HOOK_MESSAGE( ShowLogon );
	HOOK_MESSAGE( MakeCharFailed );
	HOOK_MESSAGE( PickChar );
	HOOK_MESSAGE( Spree );
}
...
int	ClientModeShared::KeyInput( int down, ButtonCode_t keynum, const char *pszCurrentBinding )
{
	...
	else if ( pszCurrentBinding && Q_strcmp( pszCurrentBinding, "inv_use" ) == 0 )
	{
		if ( down )
			GetModuleHud()->UseSelected();
		else
			GetModuleHud()->ReleasedSelected();
		return 0;
	}
	else if ( pszCurrentBinding && Q_strcmp( pszCurrentBinding, "inv_prev" ) == 0 )
	{
		if ( down )
			GetModuleHud()->CycleBackward();
		return 0;
	}
	else if ( pszCurrentBinding && Q_strcmp( pszCurrentBinding, "inv_next" ) == 0 )
	{
		if ( down )
			GetModuleHud()->CycleForward();
		return 0;
	}
