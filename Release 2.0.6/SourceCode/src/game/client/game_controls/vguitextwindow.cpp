...
{
	Q_strcpy( m_szTitle, "Modular Combat" );
	Q_strcpy( m_szMessage, "Besure to visit our webite for all the latest updates!\n\nhttp://www.modularcombatwiki.com/!" );
	m_szExitCommand[0] = 0;
	m_nContentType = TYPE_TEXT;
	Update();
}
...
void CTextWindow::OnCommand( const char *command)
{
    if (!Q_strcmp(command, "okay"))
    {
		...
		// now show the logon window
		engine->ClientCmd("login");
		engine->ClientCmd("welcome");
	}
