...
{
	m_iDuckKey = m_iJumpKey = BUTTON_CODE_INVALID;
...
void CSpectatorMenu::OnKeyCodePressed(KeyCode code)
{
	...
	else if ( code == KEY_SPACE ) // for some reason, when I use m_iJumpKey here, it only changes when I press the OPEN CONSOLE key :S
	{// cycle mode
		engine->ClientCmd(VarArgs("spec_mode %i", GetSpectatorMode()+1));
	}
}
...
void CSpectatorMenu::Update( void )
{
	...
	if ( m_iJumpKey == BUTTON_CODE_INVALID )
...
void CSpectatorGUI::Update()
{
	...
	{
		Color c = gr->GetTeamColor( g_PR->GetFaction(playernum) ); // Player's team color
