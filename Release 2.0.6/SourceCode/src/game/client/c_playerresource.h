...
public : // IGameResources intreface
	...
	virtual const char *GetSuitName( int index );
...
//	virtual int		GetTotalExp( int index );
	virtual int		GetGameExp( int index );
	virtual int		GetFaction( int index );
	virtual bool	IsInCharacter( int index );
	virtual Vector	GetPos( int index );
...
protected:
	...
	char	m_szSuit[MAX_PLAYERS+1][255];
...
//	int		m_iTotalExp[MAX_PLAYERS+1];
	int		m_iGameExp[MAX_PLAYERS+1];
	
	bool	m_bInChar[MAX_PLAYERS+1];
	int		m_iFaction[MAX_PLAYERS+1];
	Vector	m_vecPos[MAX_PLAYERS+1];

	Color	m_Colors[4]; // NUM_FACTIONS
};
