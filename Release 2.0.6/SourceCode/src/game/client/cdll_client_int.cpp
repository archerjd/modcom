...
#include "modcom/attached_sprites.h"
...
INetworkStringTable *g_pNPCNamesStringTable = NULL;
INetworkStringTable *g_pSkinOverrideStringTable = NULL;
INetworkStringTable *g_pMapListStringTable = NULL;
...
int CHLClient::Init( CreateInterfaceFn appSystemFactory, CreateInterfaceFn physicsFactory, CGlobalVarsBase *pGlobals )
{
	...
	if(filesystem->MountSteamContent(-400)==FILESYSTEM_MOUNT_FAILED)
		return false;
	...
	CreateModules(); // the global list must be populated before we init the module panel
	modemanager->Init( );
...
CON_COMMAND(testmount, "test mounted content")
{*/
	// attempt to precache "signature" materials from hl2dm and ep2 ... if they're not found, then assume that whole game is not available

	bool missingEp2 = false, missingHl2dm = false, missingThirdContent = false;

	IMaterial *pMaterial = materials->FindMaterial( "models/ministrider/mini_skin_basecolor.vmt", TEXTURE_GROUP_PRECACHED );
	if ( IsErrorMaterial( pMaterial ) )
		missingEp2 = true;
	else
	{// ensure material is properly precached
		pMaterial->IncrementReferenceCount();
		m_CachedMaterials.AddToTail( pMaterial );
	}

	pMaterial = materials->FindMaterial( "models/weapons/v_slam/v_slam.vmt", TEXTURE_GROUP_PRECACHED );
	if ( IsErrorMaterial( pMaterial ) )
		missingHl2dm = true;
	else
	{// ensure material is properly precached
...
/*
	pMaterial = materials->FindMaterial( "models/made-up.vmt", TEXTURE_GROUP_PRECACHED );
	if ( IsErrorMaterial( pMaterial ) )
		missingThirdContent = true;
	else
	{// ensure material is properly precached
		pMaterial->IncrementReferenceCount();
		m_CachedMaterials.AddToTail( pMaterial );
...
*/
	...
	if ( missingHl2dm )
	{
		if ( missingEp2 )
		{
			if ( missingThirdContent )
				pPanel->SetMessage("Missing Content", "Half-Life 2: Deathmatch, Episode 2, and some additional content");
			else
				pPanel->SetMessage("Missing Content", "Half-Life 2: Deathmatch and Episode 2");
		}
	}
	else if ( missingThirdContent )
	{
		if ( missingThirdContent )
			pPanel->SetMessage("Missing Content", "Half-Life 2: Episode 2 and some additional content");
		else
			pPanel->SetMessage("Missing Content", "Half-Life 2: Episode 2");
	}
	else if ( missingThirdContent )
		pPanel->SetMessage("Missing Content", "some additional content");
/*
#ifdef RELEASE
	else
		CheckForUpdates();
...
void CHLClient::Shutdown( void )
{
	...
	AttachedSpriteManager_Shutdown();
...
void CHLClient::HudProcessInput( bool bActive )
{
	...
	/*if ( C_HL2MP_Player::GetLocalHL2MPPlayer() && C_HL2MP_Player::GetLocalHL2MPPlayer()->InSpawnProtect() )
	{
		input->ClearInputButton( IN_ATTACK | IN_ATTACK2 );
		input->GetButtonBits( 0 ); // needed to recalculate them... apparently.
	}*/
}
...
void CHLClient::ResetStringTablePointers()
{
	...
	g_pNPCNamesStringTable = NULL;
	g_pSkinOverrideStringTable = NULL;
	g_pMapListStringTable = NULL;
}
...
void CHLClient::InstallStringTableCallback( const char *tableName )
{
	...
	else if ( !Q_strcasecmp( tableName, "NPCNames" ) )
	{
		g_pNPCNamesStringTable = networkstringtable->FindTable( tableName );
		//g_pNPCNamesStringTable->SetStringChangedCallback( NULL, OnSceneStringTableChanged );
	}
	else if ( !Q_strcasecmp( tableName, "SkinOverrides" ) )
	{
		g_pSkinOverrideStringTable = networkstringtable->FindTable( tableName );
		//g_pSkinOverrideStringTable->SetStringChangedCallback( NULL, OnSceneStringTableChanged );
	}
	else if ( !Q_strcasecmp( tableName, "MapList" ) )
	{
		g_pMapListStringTable = networkstringtable->FindTable( tableName );
	}
