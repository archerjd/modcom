...
static void RecvProxy_TypeName( const CRecvProxyData *pData, void *pStruct, void *pOut )
{
	((C_AI_BaseNPC*)pStruct)->SetTypeName( g_pNPCNamesStringTable->GetString(pData->m_Value.m_Int) );
}
...
//	RecvPropFloat( RECVINFO( m_flTimePingEffect ) ),
	RecvPropBool( RECVINFO( m_bLikesMaster ) ),
	RecvPropInt( RECVINFO( m_iTypeName ), 0, RecvProxy_TypeName ),
	RecvPropInt( RECVINFO( m_iSkinOverride ) ),
	RecvPropEHandle( RECVINFO( m_pMyPlayer ) ),
...
{
	m_bImportanRagdoll = m_bFadeCorpse = m_bPerformAvoidance = false;
	m_lifeState = LIFE_ALIVE;
	m_szTypeName[0] = '\0';
}

void C_AI_BaseNPC::SetTypeName(const char *name)
{
	Q_snprintf( m_szTypeName, sizeof(m_szTypeName), name);
}
...
int C_AI_BaseNPC::GetPlayerIndex()
{
	if ( LikesMaster() && GetMasterPlayer() )
		return ClientEntityList().GetBaseEntityFromHandle(GetMasterPlayer())->GetPlayerIndex() + 32; // minions add 32
