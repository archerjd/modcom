...
C_ClientRagdoll::C_ClientRagdoll( bool bRestoring )
{
	...
	m_flEffectTime = gpGlobals->curtime + 7.5f;
...
{
	int retVal;
	if ( m_iSkinOverride > 0 )
	{
		modelrender->ForcedMaterialOverride( materials->FindMaterial( g_pSkinOverrideStringTable->GetString(m_iSkinOverride), TEXTURE_GROUP_CLIENT_EFFECTS ) );
...
ConVar g_ragdoll_fadespeed( "g_ragdoll_fadespeed", "65" );
...
void C_ClientRagdoll::ClientThink( void )
{
	...
	if ( m_bFadingOut == false && m_flEffectTime <= gpGlobals->curtime )
		m_bFadingOut = true;
...
C_BaseAnimating *C_BaseAnimating::CreateRagdollCopy()
{
	...
	if ( IsNPC() ) // copy skin overrides onto the ragdoll
	{
		C_AI_BaseNPC *pNPC = MyNPCPointer();
		pRagdoll->SetSkinOverride(pNPC->GetSkinOverride());
	}
