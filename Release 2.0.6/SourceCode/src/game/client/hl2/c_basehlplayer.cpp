...
ConVar cl_avoid_speed( "cl_avoid_speed", "1600", FCVAR_CLIENTDLL | FCVAR_CHEAT );
#define AVOID_SPEED cl_avoid_speed.GetFloat()
...
void C_BaseHLPlayer::PerformClientSideObstacleAvoidance( float flFrameTime, CUserCmd *pCmd )
{
	...
	// don't avoid if not moving, don't want pushed around
	if ( !IsMoving() )
		return;
	...
	// reduce avoidance when moving extremely slowly
	float avoidSpeed = AVOID_SPEED;
	if ( curspeed < 32 )
		avoidSpeed *= curspeed / 32.0f;
	...
	for ( int i = 0; i < c; i++ )
	{
		C_BaseCombatCharacter *obj = dynamic_cast< C_BaseCombatCharacter *>(avoid.GetObject( i ));
		if ( !obj || !( obj->IsPlayer() || obj->IsNPC() ))
			continue;
		...
		float sidescale = 0.9f; // 2.0f
		float forwardscale = 0.45f; // 1.0f
...
//		}
		...
		float force = 1.0f;
		float forward = forwardscale * fwd * force * avoidSpeed;
		float side = sidescale * rt * force * avoidSpeed;
