...
public:
	...
	virtual int DrawModel( int flags ); // this lets us use ForcedMaterialOverride just for one character
	virtual void			EyeVectors( Vector *pForward, Vector *pRight = NULL, Vector *pUp = NULL );

	// abilities
	virtual int GetModuleLevel(Module *a) { return 0; }
	virtual int GetModuleLevel(int index) { return 0; }
	virtual bool HasModule(Module *a) { return false; }
	virtual bool HasModule(int index) { return false; }
	virtual bool IsModuleActive(Module *a) { return false; }
	virtual bool IsModuleActive(int index) { return false; }
	virtual float GetCooldownEnd(Module *a) { return 0.0f; }

	// afflictions
	bool IsBuffActive(Buff *a);
	virtual bool IsBuffActive(int i);

	virtual int	GetLevel() { return m_iLevel; }
	virtual int	GetNumScoreTokens() { return 0; }

public:

	float			m_flNextAttack;
	int				m_iParticleEffect;
	float			m_flNextParticleEffect;

protected:
	int				m_iRememberedParticleEffect;

	void		ManageBuffs();	// client side only cares about turning effects on & off
	void		StartBuffEffects(int i);
	void		MaintainBuffEffects(int i);
	void		StopBuffEffects(int i);

	int			m_bloodColor;			// color of blood particless

	int m_iModules[NUM_MODULES];
	bool m_bModulesActive[NUM_MODULES];
	float m_flModuleCooldownTime[NUM_MODULES];
	float m_flLastModuleCooldownTime[NUM_MODULES]; // when cooldown changes, do effect
...
private:
	...
	CNetworkArray( bool, m_bBuffs, NUM_BUFFS );
	bool m_bRememberedBuffStates[NUM_BUFFS];
