...
ConVar cl_showhints("cl_showhints", "1", FCVAR_ARCHIVE, "Controls whether hints will be shown during play", true, 0, true, 1);
...
void CHudHintDisplay::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	m_hFont = pScheme->GetFont( /*"HudHintText"*/"HudHintTextSmall", true );
...
void CHudHintDisplay::PerformLayout()
{
	...
	int x, y;
	GetPos( x, y );
	x -= (iDesiredLabelWide + m_iTextX*2 - wide);
	SetPos(x,y);
	SetSize(iDesiredLabelWide + m_iTextX*2, labelTall + m_iTextY*2);
	...
	for ( i=0; i<m_Labels.Count(); ++i )
	{
		int xOffset = m_iTextX;//(wide - m_Labels[i]->GetWide()) * 0.5;
...
{
	bool alwaysShow = msg.ReadByte() != 0;

	// Read the string(s)
	char szString[255];
	msg.ReadString( szString, sizeof(szString) );

	if ( !alwaysShow && cl_showhints.GetInt() == 0 )
		return;
...
void CHudHintDisplay::ShowHint(const char *text, bool alwaysShow)
{
	if ( !alwaysShow && cl_showhints.GetInt() == 0 )
		return;

	char *tmpStr = hudtextmessage->LookupString( text, NULL );
	LocalizeAndDisplay( tmpStr, text );
}

void CHudHintDisplay::ClearHint()
{
	g_pClientMode->GetViewportAnimationController()->StartAnimationSequence( "HintMessageHide" );
}
...
bool CHudHintKeyDisplay::SetHintText( const char *text )
{
	...
	SetPos( ox, oy );
	SetSize( newWide, newTall +m_iTextY+m_iTextGapY );
	m_iBaseY = oy;
...
// Purpose: Activates the hint display
//-----------------------------------------------------------------------------
void CHudHintKeyDisplay::ShowHint(const char *text)
{
	if ( SetHintText( text ) )
	{
		SetVisible( true );
		...
		Msg("Hint message not found: %s\n", text);
}

void CHudHintKeyDisplay::ClearHint()
{
