...
void CHudDeathNotice::SetColorForNoticePlayer( int iTeamNumber )
{
	if ( HL2MPRules()->IsTeamplay() )
		...
		surface()->DrawSetTextColor( GameResources()->GetTeamColor( FACTION_NONE ) );
}
...
void CHudDeathNotice::Paint()
{
	...
	for ( int i = 0; i < iCount; i++ )
	{
		...
		int iKillerFaction = 0;
		int iVictimFaction = 0;

		if( g_PR )
		{
			iKillerFaction = g_PR->GetFaction( m_DeathNotices[i].Killer.iEntIndex );
			iVictimFaction = g_PR->GetFaction( m_DeathNotices[i].Victim.iEntIndex );
		}
		...
		if ( !m_DeathNotices[i].iSuicide )
		{
			...
			SetColorForNoticePlayer( iKillerFaction );
		...
		SetColorForNoticePlayer( iVictimFaction );
...
void CHudDeathNotice::FireGameEvent( IGameEvent * event )
{
	...
	// killer == 0 means BOSS did it (pvm), unless they were killed by "the world"
	deathMsg.iSuicide = ( /*!killer ||*/ FStrEq(killedwith,"world") || killer == victim );
