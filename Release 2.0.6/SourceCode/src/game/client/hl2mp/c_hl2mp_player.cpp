...
BEGIN_RECV_TABLE_NOBASE( C_HL2MP_Player, DT_HL2MPLocalPlayerExclusive )
	...
	RecvPropArray3( RECVINFO_ARRAY(m_iModules), RecvPropInt( RECVINFO(m_iModules[0]) ) ),
	RecvPropArray3( RECVINFO_ARRAY(m_bModulesActive), RecvPropBool( RECVINFO(m_bModulesActive[0]) ) ),
	RecvPropArray3( RECVINFO_ARRAY(m_flModuleCooldownTime), RecvPropFloat( RECVINFO(m_flModuleCooldownTime[0]) ) ),
	
	RecvPropInt( RECVINFO( m_iTotalExp ) ),
//	RecvPropInt( RECVINFO( m_iGameExp ) ),
	RecvPropInt( RECVINFO( m_iModulePoints ) ), 
//	RecvPropInt( RECVINFO( m_iWeaponPoints ) ), 
//	RecvPropInt( RECVINFO( m_iCredits ) ),

	RecvPropInt( RECVINFO( m_iMaxAuxPower ) ),

	RecvPropInt( RECVINFO( m_iNumMinions ) ),
	//RecvPropBool( RECVINFO( m_bInSpawnProtect ) ),

	RecvPropInt( RECVINFO( m_iLoggedOn ) ),
	RecvPropInt( RECVINFO( m_iFaction ) ),

	RecvPropArray3( RECVINFO_ARRAY(m_iBuffs), RecvPropInt( RECVINFO(m_iBuffs[0])) ),
	RecvPropArray3( RECVINFO_ARRAY(m_flBuffEndTimes), RecvPropFloat( RECVINFO(m_flBuffEndTimes[0])) ),
END_RECV_TABLE()

BEGIN_RECV_TABLE_NOBASE( C_HL2MP_Player, DT_HL2MPNonLocalPlayerExclusive )
	RecvPropVector( RECVINFO_NAME( m_vecNetworkOrigin, m_vecOrigin ) ),
	RecvPropArray3( RECVINFO_ARRAY(m_bBuffs), RecvPropBool( RECVINFO(m_bBuffs[0])) ),
END_RECV_TABLE()


IMPLEMENT_CLIENTCLASS_DT(C_HL2MP_Player, DT_HL2MP_Player, CHL2MP_Player)
	RecvPropDataTable( "playerlocaldata", 0, 0, &REFERENCE_RECV_TABLE(DT_HL2MPLocalPlayerExclusive) ),
	RecvPropDataTable( "playernonlocaldata", 0, 0, &REFERENCE_RECV_TABLE(DT_HL2MPNonLocalPlayerExclusive) ),

	RecvPropInt( RECVINFO( m_iSprintSpeed ) ),
	RecvPropInt( RECVINFO( m_iNumScoreTokens ) ),
...
#define	HL2_NORM_SPEED mc_player_base_walk_speed.GetFloat()

static ConVar cl_playermodel( "cl_playermodel", "none", FCVAR_USERINFO | FCVAR_ARCHIVE | FCVAR_SERVER_CAN_EXECUTE, "Default Player Model");
static ConVar cl_defaultweapon( "cl_defaultweapon", "weapon_physcannon", FCVAR_USERINFO | FCVAR_ARCHIVE, "Default Spawn Weapon");
static ConVar cl_fp_ragdoll ( "cl_fpdeath", "1", FCVAR_ARCHIVE, "Enable first-person view on death" );
ConVar cl_earringing("cl_earringing", "1", FCVAR_ARCHIVE, "Controls the 'ear ringing' effect on nearby explosions", true, 0, true, 1);
...
extern ConVar mc_gamemode, forcerespawn;
extern McConVar gamemode_hoarder_respawn_delay_no_tokens, mc_respawn_delay;
extern wchar_t* ConvertCRtoNL( wchar_t *str );
int g_iLastRespawnCountdownVal = 0, g_LastAliveScoreTokens = 0;
void C_HL2MP_Player::ClientThink( void )
{
	...
	if ( IsInCharacter() )
		if ( IsAlive() )
		{
			g_LastAliveScoreTokens = GetNumScoreTokens();
			// sometimes the countdown doesn't clear
			if ( g_iLastRespawnCountdownVal > 0 )
			{
				g_iLastRespawnCountdownVal = 0;
				wchar_t outputBuf[8];
				g_pVGuiLocalize->ConstructString( outputBuf, sizeof(outputBuf), "", 0);
				internalCenterPrint->Print( ConvertCRtoNL( outputBuf ) );
			}
		}
		else
		{
			float deathDuration = (HL2MPRules() && HL2MPRules()->ShouldUseScoreTokens() && g_LastAliveScoreTokens == 0 ) ? gamemode_hoarder_respawn_delay_no_tokens.GetFloat() : mc_respawn_delay.GetFloat();
			int timeLeft = (GetDeathTime() + deathDuration) - gpGlobals->curtime + 1;
			if ( timeLeft != g_iLastRespawnCountdownVal )
			{
				g_iLastRespawnCountdownVal = timeLeft;
				wchar_t outputBuf[32];
				if ( timeLeft > 0 )
					g_pVGuiLocalize->ConstructString( outputBuf, sizeof(outputBuf), forcerespawn.GetInt() > 0 ? VarArgs("Respawning in %is...", timeLeft) : VarArgs("Respawn allowed in %is...", timeLeft), 0);
				else
					g_pVGuiLocalize->ConstructString( outputBuf, sizeof(outputBuf), "", 0);
				internalCenterPrint->Print( ConvertCRtoNL( outputBuf ) );
			}
	...
	{
		// and handle the cached spreer stuff so we can show spree war messages from the client
		if ( HL2MPRules()->GetSpreer() != HL2MPRules()->GetCachedSpreer() )
		{
			HL2MPRules()->SetCachedSpreer(HL2MPRules()->GetSpreer());
			if(HL2MPRules()->GetSpreer() != NULL)
			{
				if ( HL2MPRules()->GetSpreer() == C_BasePlayer::GetLocalPlayer() )
					SHOW_HINT_ALWAYS("Hint_SpreeSelf")
				else
					SHOW_HINT("Hint_SpreeOther")
			}
			else
			{
				SHOW_HINT("Hint_SpreeOver")
			}
		}

		if ( (IsInCharacter() && m_bShowFirstModeHint ) || HL2MPRules()->GetRememberedGameMode() != mc_gamemode.GetInt() )
		{
			m_bShowFirstModeHint = false;
			HL2MPRules()->SetRememberedGameMode(mc_gamemode.GetInt());
			switch ( mc_gamemode.GetInt() )
			{
				case DEATHMATCH:
					SHOW_HINT_ALWAYS("Hint_ModeDM") break;
				case RANDOM_PVM:
					SHOW_HINT_ALWAYS("Hint_ModePVM") break;
				case FFA:
					SHOW_HINT_ALWAYS("Hint_ModeFFA") break;
				case TEAM_DEATHMATCH:
					SHOW_HINT_ALWAYS("Hint_ModeTDM") break;
				case HOARDER:
					if ( HL2MPRules()->ShouldUseScoreTokens() )
						SHOW_HINT_ALWAYS("Hint_ModeHoarder1")
					else
						SHOW_HINT_ALWAYS("Hint_ModeHoarder2")
					break;
			}
		}
	}
...
void C_HL2MP_Player::PreThink( void )
{
	...
	if ( m_flUpdateSessionExpTime > gpGlobals->curtime )
		m_iSessionStartExp = m_iTotalExp;
...
void C_HL2MP_Player::PostDataUpdate( DataUpdateType_t updateType )
{
	if ( m_iSpawnInterpCounter != m_iSpawnInterpCounterCache )
	{
		...
		if ( IsLocalPlayer() && IsInCharacter() && GetAP() > 0 )
		{// just spawned
			SHOW_HINT("Hint_SpendAP")
		}
	}
...
void C_HL2MP_Player::CalcView( Vector &eyeOrigin, QAngle &eyeAngles, float &zNear, float &zFar, float &fov )
{
	...
	if ( m_lifeState != LIFE_ALIVE && !IsObserver() )
	{
		...
		if ( cl_fp_ragdoll.GetBool() && m_hRagdoll.Get() )
		{
			...
			// Don't use first person view when we are very close to something
			trace_t tr;
...
int C_HL2MP_Player::GetModuleLevel(Module *a)
{
	return GetModuleLevel(a->GetModuleIndex());
}

int C_HL2MP_Player::GetModuleLevel(int index)
{
	return m_iModules[index];
}

bool C_HL2MP_Player::HasModule(Module *a)
{
	return GetModuleLevel(a->GetModuleIndex()) != 0;
}

bool C_HL2MP_Player::HasModule(int index)
{
	return GetModuleLevel(index) != 0;
}

bool C_HL2MP_Player::IsModuleActive(Module *a)
{
	return IsModuleActive(a->GetModuleIndex());
}

bool C_HL2MP_Player::IsModuleActive(int index)
{
	return m_bModulesActive[index];
}

float C_HL2MP_Player::GetCooldownEnd(Module *a)
{
	return m_flModuleCooldownTime[a->GetModuleIndex()];
}

bool C_HL2MP_Player::IsInCharacter()
{
	if ( C_BasePlayer::GetLocalPlayer() == this )
		return m_iLoggedOn == 2;
	return g_PR->IsInCharacter(entindex());
}

int C_HL2MP_Player::GetFaction()
{
	if ( C_BasePlayer::GetLocalPlayer() == this )
		return m_iFaction;

	return g_PR->GetFaction(entindex());
}

bool C_HL2MP_Player::IsBuffActive(int i)
{// local player doesn't network the bool used by the base class, and instead networks an int for the level
	return C_BaseCombatCharacter::IsBuffActive(i) || m_iBuffs[i] > 0;
}