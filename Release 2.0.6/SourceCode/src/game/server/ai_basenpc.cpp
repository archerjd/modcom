...
extern McConVar mc_mechanical_damage_scale_energy;
...
{
	if ( m_bIsBoss )
		HL2MPRules()->BossNpcRemoved();

	StopParticleEffects(this);
	
	if ( LikesMaster() && GetMasterPlayer() )
	{
		CHL2MP_Player *pPlayer = ToHL2MPPlayer( GetMasterPlayer() );
		if ( pPlayer )
		{
			pPlayer->RemoveMinionFromFormation(this);
			if ( info.GetAttacker() && info.GetAttacker()->IsPlayer() )
				ClientPrint(pPlayer,HUD_PRINTTALK,UTIL_VarArgs("Your %s was killed by %s.\n",GetClassname()+4,ToHL2MPPlayer(info.GetAttacker())->GetPlayerName()));
			else
				ClientPrint(pPlayer,HUD_PRINTTALK,UTIL_VarArgs("Your %s died.\n",GetClassname()+4));
			if ( IsBuffActive(GetBuff(BUFF_MINION)) ) // be careful not to occur for crows or manhacks, which are counted separate
				pPlayer->GetLimitedQuantities()->Remove(LQ_MINION);
		}
	}

	RemoveAllBuffs();

	// Award experience for killing me.
	if ( info.GetAttacker() && (info.GetAttacker()->IsNPC() || info.GetAttacker()->IsPlayer()) )
		HL2MPRules()->AwardExpForKill(this,info);
...
int CAI_BaseNPC::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	// don't get hurt by our master if we're friends with them
	if ( LikesMaster()
		&& GetMasterPlayer() != NULL
		&& info.GetAttacker() == GetMasterPlayer() )
		return 0;

	// don't get hurt by our master's friends.
	if ( LikesMaster()
		&& GetMasterPlayer() != NULL
		&& HL2MPRules()->PlayerRelationship(GetMasterPlayer(), info.GetAttacker()) == GR_TEAMMATE )
		return 0;

	// don't take damage when frozen
	if ( GetFlags() & FL_FROZEN )
		return 0;

	m_flLastActionTime = gpGlobals->curtime;
	Forget( bits_MEMORY_INCOVER );

	if ( IsMechanical() && (info.GetDamageType() & (DMG_SHOCK|DMG_ENERGYBEAM)) )
		info.ScaleDamage(mc_mechanical_damage_scale_energy.GetFloat());

	bool wasAlive = m_iHealth > 0;
	int retVal = BaseClass::OnTakeDamage_Alive( info );
	
	// record this attack for shared experience and also for data logging
	if( info.GetAttacker() && info.GetAttacker()->MyCombatCharacterPointer() )
		info.GetAttacker()->MyCombatCharacterPointer()->RegisterAttackOnEntity( info, this, wasAlive && m_iHealth <= 0 );
...
void CAI_BaseNPC::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	...
	if ( subInfo.GetDamage() >= 1.0 && !(subInfo.GetDamageType() & DMG_SHOCK ) )
	{
		...
		{	// don't bleed/hurt from our master's damage, but still get knocked about
			if ( !LikesMaster() || GetMasterPlayer() == NULL || info.GetAttacker() != GetMasterPlayer() )
...
void CAI_BaseNPC::SetScriptedScheduleIgnoreConditions( Interruptability_t interrupt )
{
	...
	if ( interrupt > GENERAL_INTERRUPTMODULE )
		SetIgnoreConditions( g_GeneralConditions, ARRAYSIZE(g_GeneralConditions) );

	if ( interrupt > DAMAGEORDEATH_INTERRUPTMODULE )
...
extern LEVEL_EXTERN(npc_aux_max);
extern LEVEL_EXTERN(npc_aux_recharge);
...
extern McConVar mod_magmine_npc_scale, gamemode_hoarder_monster_score_token_expiry_time;
bool CAI_BaseNPC::PreThink( void )
{
	ManageModules();
	ManageBuffs();
	
	// monsters lose their score tokens after a certain period of time
	if ( GetNumScoreTokens() > 0 && gpGlobals->curtime > m_flLastActionTime + gamemode_hoarder_monster_score_token_expiry_time.GetFloat() )
	{
		RemoveScoreToken();
		m_flLastActionTime = gpGlobals->curtime;
	}
	
	// if in one of the "follow master" schedules, interrupt continually if close to the target, and only infrequently if far away
	// each interrupt allows a "recalculating" of the path to target, but doing so too frequently when far away causes indecision paralysis
	if ( IsCurSchedule(SCHED_FOLLOW_MASTER) || IsCurSchedule(SCHED_FOLLOW_MASTER_VAGUE) || IsCurSchedule(SCHED_HUNT_MASTER) )
	{
		if ( gpGlobals->curtime >= m_flNextPathRecalcTime )
		{
			SetCondition(COND_IDLE_INTERRUPT);
			float dist = (GetNavigator()->GetGoalPos() - GetAbsOrigin()).Length2D();
			if ( IsCurSchedule(SCHED_FOLLOW_MASTER_VAGUE) )
				dist += 750; // we check less frequently when only vague accuracy is required
			float interval;
			if ( dist < 225 )
				interval = 0.f;
			else if ( dist < 800 )
				...
				interval = 13.0f;
			//float interval = 1.3231f * log(dist) - 6.1042f;
			//if ( interval < 0 )
				//interval = 0;
			m_flNextPathRecalcTime = gpGlobals->curtime + interval;
		}
	}

//  now handle gravity wells from MAGD magmines
    Vector gravWellForce(0,0,0);
	float interval = ( GetMoveEfficiency() == AIME_NORMAL || GetEfficiency() == AIE_NORMAL ) ? 0.1f : 0.2f;
    for ( int i=0; i<MAX_MAGMINES; i++ )
    {
		CBaseCombatCharacter *well = HL2MPRules()->GetGravityWell(i);
		if ( !well ) // we shunt them all up so there's never any after a NULL
			break;
				
		gravWellForce += CalcMagdForce(well,this,interval);
	}
	SetAbsVelocity( GetAbsVelocity() + gravWellForce * mod_magmine_npc_scale.GetFloat() );

	// handle AUX recharge - interval *should* be our think interval, afaik
	m_flAuxPower = min(GetMaxAuxPower(), m_flAuxPower + LEVEL(npc_aux_recharge,GetLevel()) * interval);
...
int CAI_BaseNPC::MeleeAttack1Conditions ( float flDot, float flDist )
{
	...
	else if (flDot < 0.7 && ( GetEnemy() == NULL || GetEnemy()->Classify() != CLASS_BULLSEYE ))
	{
		return 0;
	}
...
*/
	if ( m_NPCState != NPC_STATE_NONE && m_NPCState != NPC_STATE_DEAD )
	{
		...
		{
			if ( GetEnemy()->IsPlayer() && !ToHL2MPPlayer(GetEnemy())->IsInCharacter() )
				SetEnemy( NULL );
			else if ( !IsFlaggedEfficient() && !g_fGameOver )
			{
...
void CAI_BaseNPC::NPCInit ( void )
{
	...
	SetBaseMaxHealth(GetHealth()); // important if we ever give monsters vitality

	if ( m_pType == NULL )
		Warning("%s has not had stats initialised!\n", GetClassname());
	...
	m_bFadeCorpse = true;//ShouldFadeOnDeath();
...
{
	if ( HL2MPRules()->IsFriendly(this,pEnemy) )
		return false;
	...
	// if we're a minion, whether they're a valid enemy depends on their distance from our master's target
	if ( LikesMaster() && IsRegularMinionType() )
	{
		CHL2MP_Player *pMaster = ToHL2MPPlayer(GetMasterPlayer());
		if ( !pMaster )
			return true;

		float enemyDist = ( pEnemy->GetAbsOrigin() - pMaster->GetFormationPosition(this) ).Length();
		switch ( pMaster->MinionStance() )
		{
		case 1: // defensive, very short range
			if ( enemyDist > 200 )
				return false;
			break;
		case 2: // medium
...
CBaseEntity *CAI_BaseNPC::BestEnemy( void )
{
	...
	for( AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst(&iter); pEMemory != NULL; pEMemory = GetEnemies()->GetNext(&iter) )
	{
		...
		if ( pEnemy->entindex() == 0) //Dont target the world, please, its really quite annoying (Josh)
		{
			DbgEnemyMsg( this, "    %s rejected: is world\n", pEnemy->GetDebugName() );
			continue;
		}
		...
		// enemies with the "spawn" buff active should be considered unreachable - don't want to attack the invulnerable guy if there's someone else we can hurt
		bUnreachable = IsUnreachable(pEnemy) || ( pEnemy->MyCombatCharacterPointer() != NULL && pEnemy->MyCombatCharacterPointer()->IsBuffActive(BUFF_SPAWN) );
...
//	SendPropFloat( SENDINFO( m_flTimePingEffect ) ),
	SendPropBool( SENDINFO( m_bLikesMaster ) ),
	SendPropInt( SENDINFO( m_iTypeName ), BITS_FOR_MAX_NPC_TYPES, SPROP_UNSIGNED ),
	SendPropInt( SENDINFO( m_iSkinOverride ), BITS_FOR_MAX_NPC_TYPES, SPROP_UNSIGNED ),
	SendPropEHandle( SENDINFO( m_pMyPlayer ) ),
	SendPropInt( SENDINFO( m_iHealth ) ),
	SendPropInt( SENDINFO( m_iNumScoreTokens ), 3, SPROP_UNSIGNED ),
//	SendPropInt( SENDINFO( m_iMaxHealth ) ),
	SendPropArray3( SENDINFO_ARRAY3(m_bBuffs), SendPropInt( SENDINFO_ARRAY(m_bBuffs) ) ),

	//SendPropExclude( "DT_BaseAnimating", "m_flPoseParameter" ), // eh... yeah. not used as far as i can see.
	SendPropExclude( "DT_BaseFlex", "m_viewtarget" ), // what I'm looking it (turned head) ... not used by any current NPCs
END_SEND_TABLE()
...
void CAI_BaseNPC::ToggleFreeze(void) 
{
	...
	{
		// Freeze them.
		ClearCondition(COND_NPC_UNFREEZE);
    ...
    m_bDeferredNavigation( false )
{
	...
	m_bLikesMaster = m_bIsBoss = false;
	m_iTypeName = 0;
	
	m_flMaxAuxPower = m_flAuxPower = m_flAuxRecharge = 0;
	m_flLastActionTime = gpGlobals->curtime;
	...
	m_LagTrack = new CUtlFixedLinkedList< LagRecord >();
	m_RestoreData = m_ChangeData = NULL;

	m_flNextPathRecalcTime = gpGlobals->curtime;

	for ( int i=0; i<NUM_MODULES; i++ )
		m_abilityDispositions[i] = 0;
}
...
CAI_BaseNPC::~CAI_BaseNPC(void)
{
	...
	m_LagTrack->Purge(); // PurgeAndDeleteAll() ?
...
static bool currentSoundOverridden = false; // any of the different EmitSound functions could be called by AI code, so we have overridden them all. As they also call each other, this variable ensures we only call GetSoundOverride once per emit

//-----------------------------------------------------------------------------
// Purpose: allow custom monsters to override sounds
//-----------------------------------------------------------------------------
void CAI_BaseNPC::EmitSound( const char *soundname, float soundtime /*= 0.0f*/, float *duration /*=NULL*/ )
{
	const char *soundOverride = currentSoundOverridden ? NULL : GetSoundOverride(soundname);
	if ( soundOverride != NULL )
	{
		soundname = soundOverride;
		currentSoundOverridden = true;
	}
...
void CAI_BaseNPC::EmitSound( const char *soundname, HSOUNDSCRIPTHANDLE& handle, float soundtime /*= 0.0f*/, float *duration /*=NULL*/ )
{
	const char *soundOverride = currentSoundOverridden ? NULL : GetSoundOverride(soundname);
	if ( soundOverride != NULL )
	{
...
void CAI_BaseNPC::EmitSound( IRecipientFilter& filter, int iEntIndex, const char *soundname, const Vector *pOrigin /*= NULL*/, float soundtime /*= 0.0f*/, float *duration /*=NULL*/ )
{
	if ( !currentSoundOverridden )
	{
		const char *soundOverride = GetSoundOverride(soundname);
		if ( soundOverride != NULL )
		{
			soundname = soundOverride;
			currentSoundOverridden = true;
...
void CAI_BaseNPC::EmitSound( IRecipientFilter& filter, int iEntIndex, const char *soundname, HSOUNDSCRIPTHANDLE& handle, const Vector *pOrigin /*= NULL*/, float soundtime /*= 0.0f*/, float *duration /*=NULL*/ )
{
	if ( !currentSoundOverridden )
	{
		const char *soundOverride = GetSoundOverride(soundname);
...
void CAI_BaseNPC::EmitSound( IRecipientFilter& filter, int iEntIndex, const EmitSound_t & params )
{
	EmitSound_t params2; // can't just copy it directly, unfortunately
	params2.m_bEmitCloseCaption = params.m_bEmitCloseCaption;
	params2.m_bWarnOnDirectWaveReference = params.m_bWarnOnDirectWaveReference;
	params2.m_bWarnOnMissingCloseCaption = params.m_bWarnOnMissingCloseCaption;
	params2.m_flSoundTime = params.m_flSoundTime;
	params2.m_flVolume = params.m_flVolume;
	params2.m_hSoundScriptHandle = params.m_hSoundScriptHandle;
	params2.m_nChannel = params.m_nChannel;
	params2.m_nFlags = params.m_nFlags;
	params2.m_nPitch = params.m_nPitch;
	params2.m_nSpeakerEntity = params.m_nSpeakerEntity;
	params2.m_pflSoundDuration = params.m_pflSoundDuration;
	params2.m_pOrigin = params.m_pOrigin;
	params2.m_pSoundName = params.m_pSoundName;
	params2.m_SoundLevel = params.m_SoundLevel;
	params2.m_UtlVecSoundOrigin = params.m_UtlVecSoundOrigin;

	if ( !currentSoundOverridden )
	{
		const char *soundOverride = GetSoundOverride(params2.m_pSoundName);
		if ( soundOverride != NULL )
			params2.m_pSoundName = soundOverride;
	}

	currentSoundOverridden = false; // reset this
	((CSoundEmitterSystem*)SoundEmitterSystem())->EmitSound( filter, iEntIndex, params2 );
}

void CAI_BaseNPC::EmitSound( IRecipientFilter& filter, int iEntIndex, const EmitSound_t & params, HSOUNDSCRIPTHANDLE& handle )
{
	EmitSound_t params2; // can't just copy it directly, unfortunately
	params2.m_bEmitCloseCaption = params.m_bEmitCloseCaption;
	params2.m_bWarnOnDirectWaveReference = params.m_bWarnOnDirectWaveReference;
	...
	currentSoundOverridden = false; // reset this
	((CSoundEmitterSystem*)SoundEmitterSystem())->EmitSoundByHandle( filter, iEntIndex, params2, handle );
}

const char *CAI_BaseNPC::GetSoundOverride(const char *soundName)
{
	return m_pType->GetSoundOverride(soundName);
}

//-----------------------------------------------------------------------------
// Purpose: Set up our relationship with our master
//-----------------------------------------------------------------------------
void CAI_BaseNPC::SetLikesMaster(bool b)
{
	if ( !GetMasterPlayer() )
		return; // don't set up relationships with no one

	m_bLikesMaster = b;

	if ( b ) // love him
	{
		AddEntityRelationship( GetMasterPlayer(), D_LI, 99 );
		if ( IsRegularMinionType() )
			ApplyBuff(BUFF_MINION); // heal slowly
	}
	else // loathe him, more than anyone else. Essentially, used for PvM monsters
	{
		AddEntityRelationship( GetMasterPlayer(), D_HT, 99 );
	}

}

//-----------------------------------------------------------------------------
// Purpose: Returns who our attacks should be attributed to:
//			The master player, if there is one, otherwise myself
//-----------------------------------------------------------------------------
CBaseCombatCharacter *CAI_BaseNPC::MyAttacker()
{
	if ( !LikesMaster() || GetMasterPlayer() == NULL )
		return this;
	else
		return GetMasterPlayer();
}

void CAI_BaseNPC::SetStats(CNPCTypeInfo *info, int level, bool bShouldCallSpawn)
{
	m_iTypeName = info->m_iNameIndex;
	m_flLastActionTime = gpGlobals->curtime;
	
	SetLevel(level);
	m_bIsBoss = info->m_bIsBoss;
	
	m_iSkinOverride = info->m_iSkinOverride; // if we enable multiple overrides,  we only network the first index in the base class. Classes whose default models use multiple materials will network the rest of them. That won't work when overriding the skin on a non-default model that uses more materials than the original model does, though. For that, we'd need to have another network string table, that sends a comma-seperated list of all skin override indices used by each monster type. That's getting a bit silly though. Alternatively, just network all 16 override materials for each NPC, i guess.
	
	if ( info->m_bHasEffectColor )
		SetEffectColor(info->m_effectColor);

	if ( info == NULL )
		Warning("Setting NULL type for %s\n", GetClassname());

	// we set this to give access to the GetVarValue function ... we also read several values directly out of this, rather than storing them on the NPC itself.
	m_pType = info;
	
	// all monsters & minions now have AUX
	m_flMaxAuxPower = LEVEL(npc_aux_max,level);
	m_flAuxPower = m_flMaxAuxPower;
	m_flAuxRecharge = LEVEL(npc_aux_recharge,level);
	
	// set up abilities here!
	/*
	for ( int i=0; i<info.numModules; i++ )
	{
		CNPCModule abilityInfo = info.m_Modules[i];
		int level = max(0,GetLevel() + abilityInfo.LevelOffset);
		Module *a = GetModule(abilityInfo.ModuleNumber);
		if ( a )
		{
			SetModuleLevel(a, level);
			m_abilityDispositions[a->GetModuleIndex()] = max(1,abilityInfo.UseWeighting);
		}
	}*/
	...
	{
		DispatchSpawn(this);
		if ( info->m_szModelName[0] != '\0' )
		{
			SetModel(info->m_szModelName);

			if ( info->m_iSkinNumber != -1 )
				m_nSkin = info->m_iSkinNumber;
		}
	}

	// clear any capabilities that have been "disabled" in the NPC script file
	CapabilitiesRemove(info->m_iBitsDisabledCapacities);
}

// master parameter only actually used if onlyMinions is true
void KillAllNPCsWithMaster(CHL2MP_Player *master, bool onlyMinions)
{
	CAI_BaseNPC **ppAIs = g_AI_Manager.AccessAIs();
	...
	for ( int i = 0; i < nAIs; i++ )// look through all the AIs and check each
	{
		...
		if ( ( !onlyMinions && !pNPC->LikesMaster() ) || ( pNPC->LikesMaster() /*&& pNPC->IsRegularMinionType()*/ && pNPC->GetMasterPlayer() == master ) )
		{
			//pNPC->SetMasterPlayer(NULL); // stop this counting as a kill for anyone
			pNPC->RemoveAllBuffs(); // otherwise, spawning monsters wouldn't be killed, leading to the "Unknown" exploit
			CTakeDamageInfo info( NULL, pNPC, pNPC->GetMaxHealth()*2, DMG_GENERIC|DMG_DIRECT ); // enough to splat it
			pNPC->TakeDamage(info); // kill
		}
	}

}

ConVar pvm_difficulty("pvm_difficulty", "1.0", FCVAR_NOTIFY | FCVAR_GAMEDLL, "Changes monster difficulty level", true, 1.0, true, 10.0 );
float CAI_BaseNPC::GetStatValue(const char *name)
{
	if ( m_pType == NULL )
	{
		Warning(UTIL_VarArgs("NPC %s has no type set!\n", GetClassname()));
		return 1;
	}
	float retVal = m_pType->GetVarValue(name, GetLevel());
	if ( name[0] == 'd' && name[1] == 'm' && name[2] == 'g' && name[3] == '_' )
	{
		retVal *= GetDamageDealtScale(); // scale all damage stats by damage dealt scale
		retVal *= pvm_difficulty.GetFloat();
	}
	//Msg("Stat %s of level %i %s is %.1f\n", name, GetLevel(), GetTypeName(), retVal);
	return retVal;
}

int CAI_BaseNPC::GetPlayerIndex()
{
	if ( LikesMaster() && GetMasterPlayer() )
		return GetMasterPlayer()->GetPlayerIndex() + 32; // minions add 32

	return BaseClass::GetPlayerIndex();
}