...
//==================================================
	...
	case TASK_GET_PATH_TO_OBEY_MASTER:
		{
			if ( !GetMasterPlayer() )
			{
				//Msg("Failed, can't get player\n");
				TaskFail(FAIL_NO_TARGET);
			}
			...
			goal.dest = pMaster->GetFormationPosition(this);
			//goal.pTarget = pMaster;

			if ( GetNavigator()->SetGoal( goal, AIN_CLEAR_PREVIOUS_STATE ) )
				TaskComplete();
			//else
				//Msg("Unable to get path to goal\n");
			break;
		}

		case TASK_GET_PATH_TO_MASTER:
		{
			if ( !GetMasterPlayer() )
			{
				//Msg("Failed, can't get player\n");
			...
			goal.type = GOALTYPE_LOCATION;
			goal.dest = GetMasterPlayer()->GetAbsOrigin();
			goal.pTarget = GetMasterPlayer();
...
void CAI_BaseNPC::RunTask( const Task_t *pTask )
{
	...
	case TASK_FREEZE:
//		if ( HasCondition(COND_NPC_UNFREEZE) )
//		{
//			Msg("Completing freeze task!\n");
//			TaskComplete();
//		}
		break;
...
//#define MASTER_FOLLOW_DISTANCE	140
//#define MASTER_DIST_SQ	MASTER_FOLLOW_DISTANCE*MASTER_FOLLOW_DISTANCE
int CAI_BaseNPC::SelectIdleSchedule()
{
	...
	if ( !HasCondition(COND_SEE_DISLIKE) && !HasCondition(COND_SEE_HATE) && !HasCondition(COND_SEE_ENEMY) && !HasCondition( COND_SEE_NEMESIS ) && GetMasterPlayer() )
	{
		if ( LikesMaster() )
		{// we're alert, but can't see an enemy, & we like our master ... see if we should move to them!
			//if ( ( GetMasterPlayer()->GetAbsOrigin() - GetAbsOrigin() ).Length() > MASTER_FOLLOW_DISTANCE )
			CHL2MP_Player *pMaster = ToHL2MPPlayer(GetMasterPlayer());
			if ( pMaster->MinionsOrderedToPosition() )
			{
				if ( (pMaster->GetFormationPosition(this) - GetAbsOrigin()).Length() < 80 )
					return SCHED_WANDER_SMALL; // if we're right on our target pos, do a small wander
				return SCHED_FOLLOW_MASTER_VAGUE;
			}
			else
				return SCHED_FOLLOW_MASTER;
		}
		//else if ( !LikesMaster() && !(GetMasterPlayer()->GetFlags() & FL_NOTARGET) )
		//{// we're alert, but can't see an enemy, & we hate our master ... hunt them!
			//return SCHED_HUNT_MASTER;
		//}
	}

	if ( !LikesMaster() && GetMasterPlayer() && !(GetMasterPlayer()->GetFlags() & FL_NOTARGET) )
	{// we're alert, but can't see an enemy, & we hate our master ... hunt them!
		//Msg("Choosing to hunt master\n");
		return SCHED_HUNT_MASTER;
	}

//	Msg("NPC idle, not following master!\n");
	if ( HasCondition ( COND_HEAR_DANGER ) ||
	...
	if (GetNavigator()->GetGoalType() == GOALTYPE_NONE)
	{
		//Msg("navigator GoalType == GOALTYPE_NONE\n");
...
int CAI_BaseNPC::SelectAlertSchedule()
{
	...
	if ( !HasCondition(COND_SEE_DISLIKE) && !HasCondition(COND_SEE_HATE) && !HasCondition(COND_SEE_ENEMY) && !HasCondition( COND_SEE_NEMESIS ) && GetMasterPlayer() && !HasCondition(COND_LIGHT_DAMAGE)  && !HasCondition(COND_HEAVY_DAMAGE) )
	{
		if ( LikesMaster() )
		{// we're alert, but can't see an enemy, & we like our master ... see if we should move to them!
			//if ( ( GetMasterPlayer()->GetAbsOrigin() - GetAbsOrigin() ).Length() > MASTER_FOLLOW_DISTANCE )
			CHL2MP_Player *pMaster = ToHL2MPPlayer(GetMasterPlayer());
			return pMaster->MinionsOrderedToPosition() ? SCHED_FOLLOW_MASTER_VAGUE : SCHED_FOLLOW_MASTER;
		}
		//else if ( !LikesMaster() && !(GetMasterPlayer()->GetFlags() & FL_NOTARGET) )
		//{// we're alert, but can't see an enemy, & we hate our master ... hunt them!
			//return SCHED_HUNT_MASTER;
