...
//
// Purpose: Combine Zombie... Zombie Combine... its like a... HEV_ZOMB... get it?
//
// $NoKeywords: $
//=============================================================================//
...
enum
{	
	SQUAD_SLOT_HEV_ZOMB_SPRINT1 = LAST_SHARED_SQUADSLOT,
	SQUAD_SLOT_HEV_ZOMB_SPRINT2,
};

#define MIN_SPRINT_TIME 3.5f
#define MAX_SPRINT_TIME 15.0f

#define MIN_SPRINT_DISTANCE 64.0f
#define MAX_SPRINT_DISTANCE 2048.0f

#define SPRINT_CHANCE_VALUE 20 // up from 10
#define SPRINT_CHANCE_VALUE_DARKNESS 50

#define GRENADE_PULL_MAX_DISTANCE 256.0f

#define HEV_ZOMB_MAX_GRENADES 0

int ACT_HEV_ZOMB_GRENADE_PULL;
int ACT_HEV_ZOMB_GRENADE_WALK;
int ACT_HEV_ZOMB_GRENADE_RUN;
int ACT_HEV_ZOMB_GRENADE_IDLE;
int ACT_HEV_ZOMB_ATTACK_FAST;
int ACT_HEV_ZOMB_GRENADE_FLINCH_BACK;
int ACT_HEV_ZOMB_GRENADE_FLINCH_FRONT;
int ACT_HEV_ZOMB_GRENADE_FLINCH_WEST;
int ACT_HEV_ZOMB_GRENADE_FLINCH_EAST;
int ACT_HEV_ZOMB_DIE_RAGDOLL;

int AE_HEV_ZOMB_PULLPIN;

int SOUNDENT_CHANNEL_HEV_ZOMB_GRENADE;


//extern bool IsAlyxInDarknessMode();

ConVar	sk_hev_zomb_health( "sk_hev_zomb_health","0");

float g_flHEV_ZOMBGrenadeTimes = 0;

class CNPC_Hev_Zomb : public CAI_BlendingHost<CNPC_BaseZombie>, public CDefaultPlayerPickupVPhysics
{
	DECLARE_DATADESC();
	DECLARE_CLASS( CNPC_Hev_Zomb, CAI_BlendingHost<CNPC_BaseZombie> );
...
public:
	...
	virtual const char *GetHeadcrabTypeName( void );
	...
	enum
	{
		COND_HEV_ZOMB_GRENADE = LAST_BASE_ZOMBIE_CONDITION,
	};

	enum
	{
		SCHED_HEV_ZOMB_PULL_GRENADE = LAST_BASE_ZOMBIE_SCHEDULE,
	};
...
LINK_ENTITY_TO_CLASS( npc_hev_zomb, CNPC_Hev_Zomb );

BEGIN_DATADESC( CNPC_Hev_Zomb )
...
//---------------------------------------------------------
//---------------------------------------------------------
const char *CNPC_Hev_Zomb::pMoanSounds[] =
{
	"ATV_engine_null",
};

void CNPC_Hev_Zomb::Spawn( void )
{
	Precache();
...
/*#else
	...
	g_flHEV_ZOMBGrenadeTimes = gpGlobals->curtime;
	m_flGrenadePullTime = gpGlobals->curtime;

	m_iGrenadeCount = HEV_ZOMB_MAX_GRENADES;
}

void CNPC_Hev_Zomb::Precache( void )
{
	BaseClass::Precache();

	PrecacheModel( "models/zombie/hev_zomb.mdl" );
...
void CNPC_Hev_Zomb::SetZombieModel( void )
{
	SetModel( "models/zombie/hev_zomb.mdl" );
...
void CNPC_Hev_Zomb::PrescheduleThink( void )
{
	...
	if ( HasGrenade () )
	{
		CSoundEnt::InsertSound ( SOUND_DANGER, GetAbsOrigin() + GetSmoothedVelocity() * 0.5f , 256, 0.1, this, SOUNDENT_CHANNEL_HEV_ZOMB_GRENADE );
...
void CNPC_Hev_Zomb::OnScheduleChange( void )
{
...
bool CNPC_Hev_Zomb::CanRunAScriptedNPCInteraction( bool bForced )
{
	if ( HasGrenade() == true )
		return false;
...
int CNPC_Hev_Zomb::SelectSchedule( void )
{
	if ( GetHealth() <= 0 )
		return BaseClass::SelectSchedule();

	if ( HasCondition( COND_HEV_ZOMB_GRENADE ) )
	{
		ClearCondition( COND_HEV_ZOMB_GRENADE );
		
		return SCHED_HEV_ZOMB_PULL_GRENADE;
	}

	return BaseClass::SelectSchedule();
}

void CNPC_Hev_Zomb::BuildScheduleTestBits( void )
{
	BaseClass::BuildScheduleTestBits();

	SetCustomInterruptCondition( COND_HEV_ZOMB_GRENADE );
}

Activity CNPC_Hev_Zomb::NPC_TranslateActivity( Activity baseAct )
{
	if ( baseAct == ACT_MELEE_ATTACK1 )
	{
		if ( m_flSuperFastAttackTime > gpGlobals->curtime || HasGrenade() )
		{
			return (Activity)ACT_HEV_ZOMB_ATTACK_FAST;
		}
	}
	...
	if ( baseAct == ACT_IDLE )
	{
		if ( HasGrenade() )
		{
			return (Activity)ACT_HEV_ZOMB_GRENADE_IDLE;
		}
	}
...
int CNPC_Hev_Zomb::MeleeAttack1Conditions ( float flDot, float flDist )
{
...
void CNPC_Hev_Zomb::GatherGrenadeConditions( void )
{
	if ( m_iGrenadeCount <= 0 )
		return;

	if ( g_flHEV_ZOMBGrenadeTimes > gpGlobals->curtime )
		return;
	...
	if ( pPlayer && pPlayer->FVisible( this ) )
	{
		...
		if ( flLengthToPlayer <= GRENADE_PULL_MAX_DISTANCE && flLengthToEnemy <= GRENADE_PULL_MAX_DISTANCE )
		{
			...
			{
				g_flHEV_ZOMBGrenadeTimes = gpGlobals->curtime + 10.0f;
				SetCondition( COND_HEV_ZOMB_GRENADE );
			}
		}
	}
}

int CNPC_Hev_Zomb::TranslateSchedule( int scheduleType ) 
{
	return BaseClass::TranslateSchedule( scheduleType );
}

void CNPC_Hev_Zomb::DropGrenade( Vector vDir )
{
	if ( m_hGrenade == NULL )
		 return;
...
void CNPC_Hev_Zomb::Event_Killed( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
bool CNPC_Hev_Zomb::HandleInteraction( int interactionType, void *data, CBaseCombatCharacter *sourceEnt )
{
...
void CNPC_Hev_Zomb::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
...
void CNPC_Hev_Zomb::HandleAnimEvent( animevent_t *pEvent )
{
	if ( pEvent->event == AE_HEV_ZOMB_PULLPIN )
	{
		Vector vecStart;
		QAngle angles;
		GetAttachment( "grenade_attachment", vecStart, angles );

		CBaseGrenade *pGrenade = Frag_Grenade_Create( vecStart, vec3_angle, vec3_origin, AngularImpulse( 0, 0, 0 ), this, 3.5f, true );
		...
		if ( pGrenade )
		{
			...
			if ( pPhysicsObject )
			{
				...
				EmitSound( "HEV_ZOMB.ReadyGrenade" );
...
bool CNPC_Hev_Zomb::AllowedToSprint( void )
{
	if ( IsOnFire() )
		return false;
	...
	{
		/*if ( IsStrategySlotRangeOccupied( SQUAD_SLOT_HEV_ZOMB_SPRINT1, SQUAD_SLOT_HEV_ZOMB_SPRINT2 ) == true )
			return false;*/
...
void CNPC_Hev_Zomb::StopSprint( void )
{
...
void CNPC_Hev_Zomb::Sprint( bool bMadSprint )
{
	if ( IsSprinting() )
		return;

	OccupyStrategySlotRange( SQUAD_SLOT_HEV_ZOMB_SPRINT1, SQUAD_SLOT_HEV_ZOMB_SPRINT2 );
	...
	EmitSound( "HEV_ZOMB.Charge" );
}

void CNPC_Hev_Zomb::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		case TASK_WAIT_FOR_MOVEMENT:
		{
			...
			if ( GetEnemy() )
			{
				...
				if ( HasGrenade() )
				{
					if ( IsSprinting() )
					{
						GetNavigator()->SetMovementActivity( (Activity)ACT_HEV_ZOMB_GRENADE_RUN );
					}
					else
					{
						GetNavigator()->SetMovementActivity( (Activity)ACT_HEV_ZOMB_GRENADE_WALK );
					}
...
void CNPC_Hev_Zomb::InputStartSprint ( inputdata_t &inputdata )
{
	Sprint();
}

void CNPC_Hev_Zomb::InputPullGrenade ( inputdata_t &inputdata )
{
	g_flHEV_ZOMBGrenadeTimes = gpGlobals->curtime + 5.0f;
	SetCondition( COND_HEV_ZOMB_GRENADE );
}
...
//-----------------------------------------------------------------------------
const char *CNPC_Hev_Zomb::GetMoanSound( int nSound )
{
...
//-----------------------------------------------------------------------------
void CNPC_Hev_Zomb::FootstepSound( bool fRightFoot )
{
	if( fRightFoot )
	{
...
//-----------------------------------------------------------------------------
// Purpose: Overloaded so that explosions don't split the HEV_ZOMB in twain.
//-----------------------------------------------------------------------------
bool CNPC_Hev_Zomb::ShouldBecomeTorso( const CTakeDamageInfo &info, float flDamageThreshold )
{
	return false;
}
...
//-----------------------------------------------------------------------------
void CNPC_Hev_Zomb::FootscuffSound( bool fRightFoot )
{
	if( fRightFoot )
	{
		EmitSound( "HEV_ZOMB.ScuffRight" );
	}
	else
	{
		EmitSound( "HEV_ZOMB.ScuffLeft" );
	}
}
...
//-----------------------------------------------------------------------------
void CNPC_Hev_Zomb::AttackHitSound( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_Hev_Zomb::AttackMissSound( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Hev_Zomb::PainSound( const CTakeDamageInfo &info )
{
	...
	EmitSound( "HEV_ZOMB.Pain" );
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Hev_Zomb::DeathSound( const CTakeDamageInfo &info ) 
{
	EmitSound( "HEV_ZOMB.Die" );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Hev_Zomb::AlertSound( void )
{
	EmitSound( "HEV_ZOMB.Alert" );
...
//-----------------------------------------------------------------------------
void CNPC_Hev_Zomb::IdleSound( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_Hev_Zomb::AttackSound( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CNPC_Hev_Zomb::GetHeadcrabModel( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CNPC_Hev_Zomb::GetLegsModel( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
const char *CNPC_Hev_Zomb::GetTorsoModel( void )
{
...
//---------------------------------------------------------
void CNPC_Hev_Zomb::MoanSound( envelopePoint_t *pEnvelope, int iEnvelopeSize )
{
	if( IsOnFire() )
	{
...
//-----------------------------------------------------------------------------
const char *CNPC_Hev_Zomb::GetHeadcrabTypeName( void )
{
	return "Headcrab";
}

void CNPC_Hev_Zomb::ReleaseGrenade( Vector vPhysgunPos )
{
	if ( HasGrenade() == false )
		return;
	...
	if ( flDotRight > flDotForward )
	{
		if ( bNegativeRight == true )
			aActivity = (Activity)ACT_HEV_ZOMB_GRENADE_FLINCH_WEST;
		else 
			aActivity = (Activity)ACT_HEV_ZOMB_GRENADE_FLINCH_EAST;
	}
	else
	{
		if ( bNegativeForward == true )
			aActivity = (Activity)ACT_HEV_ZOMB_GRENADE_FLINCH_BACK;
		else 
			aActivity = (Activity)ACT_HEV_ZOMB_GRENADE_FLINCH_FRONT;
	}
...
CBaseEntity *CNPC_Hev_Zomb::OnFailedPhysGunPickup( Vector vPhysgunPos )
{
...
AI_BEGIN_CUSTOM_NPC( npc_hev_zomb, CNPC_Hev_Zomb )

	//Squad slots
	DECLARE_SQUADSLOT( SQUAD_SLOT_HEV_ZOMB_SPRINT1 )
	DECLARE_SQUADSLOT( SQUAD_SLOT_HEV_ZOMB_SPRINT2 )

	DECLARE_CONDITION( COND_HEV_ZOMB_GRENADE )

	DECLARE_ACTIVITY( ACT_HEV_ZOMB_GRENADE_PULL )
	DECLARE_ACTIVITY( ACT_HEV_ZOMB_GRENADE_WALK )
	DECLARE_ACTIVITY( ACT_HEV_ZOMB_GRENADE_RUN )
	DECLARE_ACTIVITY( ACT_HEV_ZOMB_GRENADE_IDLE )
	DECLARE_ACTIVITY( ACT_HEV_ZOMB_ATTACK_FAST )
	DECLARE_ACTIVITY( ACT_HEV_ZOMB_GRENADE_FLINCH_BACK )
	DECLARE_ACTIVITY( ACT_HEV_ZOMB_GRENADE_FLINCH_FRONT )
	DECLARE_ACTIVITY( ACT_HEV_ZOMB_GRENADE_FLINCH_WEST)
	DECLARE_ACTIVITY( ACT_HEV_ZOMB_GRENADE_FLINCH_EAST )
	DECLARE_ACTIVITY( ACT_HEV_ZOMB_DIE_RAGDOLL )

	DECLARE_ANIMEVENT( AE_HEV_ZOMB_PULLPIN )


	DEFINE_SCHEDULE
	(
	SCHED_HEV_ZOMB_PULL_GRENADE,

	"	Tasks"
	"		TASK_PLAY_SEQUENCE					ACTIVITY:ACT_HEV_ZOMB_GRENADE_PULL"
