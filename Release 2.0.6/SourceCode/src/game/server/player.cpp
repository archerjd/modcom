...
int CBasePlayer::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
	...
	// Armor. 
	if (m_ArmorValue && !(info.GetDamageType() & (DMG_FALL | DMG_DROWN | DMG_POISON | DMG_RADIATION | DMG_NERVEGAS )) )// armor doesn't protect against fall or drown damage!
	{
...
void CBasePlayer::OnDamagedByExplosion( const CTakeDamageInfo &info )
{
	...
	// IF you like having a banshees cock in your ear this will pass true
	if ( FStrEq(engine->GetClientConVarValue( engine->IndexOfEdict( edict() ), "cl_earringing" ), "1") )
	{
		int effect = shock ? 
...
// choose to respawn.
	...
	// the code that was here, I've moved to CHL2MP_Player
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Vector CBasePlayer::GetAutoaimVector( float flScale, bool forceAutoaim )
{
	...
	GetAutoaimVector( params, false, forceAutoaim );
	return params.m_vecAutoAimDir;
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Vector CBasePlayer::GetAutoaimVector( float flScale, float flMaxDist, bool forceAutoaim )
{
	autoaim_params_t params;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBasePlayer::GetAutoaimVector( autoaim_params_t &params, bool friendly, bool forceAutoaim )
{
	// Assume autoaim will not be assisting.
	params.m_bAutoAimAssisting = false;

	if ( ( ShouldAutoaim() == false && forceAutoaim == false ) || ( params.m_fScale == AUTOAIM_SCALE_DIRECT_ONLY ) )
	{
		Vector	forward;
	...
	QAngle angles = AutoaimDeflection( vecSrc, params, friendly, forceAutoaim );
...
//-----------------------------------------------------------------------------
#define TARGET_ANGLE_DOT_THRESHOLD 0.72
...
/*	float radiusSqr;
	...
	return 0.0f;
*/
	Vector dirFacing = viewDir;
	Vector targetDir = vecTarget - eyePosition;
	VectorNormalize(dirFacing);
	VectorNormalize(targetDir);

	float dot = DotProduct(dirFacing,targetDir);
	if ( dot > TARGET_ANGLE_DOT_THRESHOLD )
		return dot;
	else
		return TARGET_ANGLE_DOT_THRESHOLD;
}
...
// Output : Vector
//-----------------------------------------------------------------------------
QAngle CBasePlayer::AutoaimDeflection( Vector &vecSrc, autoaim_params_t &params, bool friendly, bool forceAutoaim )
{
	float		bestscore;
	float		score;
	QAngle		eyeAngles;
	...
	if ( ShouldAutoaim() == false && forceAutoaim == false )
	{
		m_fOnTarget = false;
		return vec3_angle;
	}
	...
	if ( pEntHit && pEntHit->m_takedamage != DAMAGE_NO && pEntHit->GetHealth() > 0 )
	{
		...
		if (!((GetWaterLevel() != 3 && pEntHit->GetWaterLevel() == 3) || (GetWaterLevel() == 3 && pEntHit->GetWaterLevel() == 0)))
		{
			if( pEntHit->ShouldAttractAutoAim(this) )
			{
				...
				if( ( pEntHit->IsNPC() || pEntHit->IsPlayer() ) && g_pGameRules->GetAutoAimMode() > AUTOAIM_NONE )
				{
					bool thisTargetFriendly = HL2MPRules()->IsFriendly(this,pEntHit);
					if ( friendly != thisTargetFriendly )
//					if ( ( friendly && relationship != D_LI ) || ( !friendly && IRelationType( pEntHit ) != D_HT ) )
					{
						bAimAtThis = false;
					}
				}
...
//			if( pEntity->MyNPCPointer() )
			{
				...
				bool thisTargetFriendly = HL2MPRules()->IsFriendly(this,pEntity);
				if ( friendly != thisTargetFriendly )
//				if ( ( friendly && IRelationType( pEntity ) != D_LI ) || ( !friendly && IRelationType( pEntity ) != D_HT ) )
				{
...
void CBasePlayer::Weapon_Drop( CBaseCombatWeapon *pWeapon, const Vector *pvecTarget /* = NULL */, const Vector *pVelocity /* = NULL */ )
{
	...
	{
#ifdef USE_OMNIBOT
		Omnibot::Notify_RemoveWeapon(this,pWeapon->GetClassname());
#endif
		if ( bWasActiveWeapon )
		{
...
void CBasePlayer::Weapon_Equip( CBaseCombatWeapon *pWeapon )
{
	...
	if ( bShouldSwitch && !IsBot() )
	{
		Weapon_Switch( pWeapon );
	}

#ifdef USE_OMNIBOT
	Omnibot::Notify_AddWeapon(this,pWeapon->GetClassname());
#endif
}
