...
public:
	...
	int		GetBaseMaxHealth()  const	{ return m_iBaseMaxHealth; }
	void	SetBaseMaxHealth( int amt )	{ m_iBaseMaxHealth = amt; }

	int					TakeHealth( float flHealth, int bitsDamageType );
	void				CauseDeath( const CTakeDamageInfo &info );
	virtual int			GetLevel() { return m_iLevel; } // everything's level 0 by default
	virtual void		SetLevel(int i) { m_iLevel = i; }
	virtual entity_effect_t GetSpawnEffect() { return NO_ENTITY_EFFECT; }
	...
	void				RemoveAllAmmo( );
	int					GetAmmoCount( int iAmmoIndex, bool considerWeaponClips=false ) const;
	int					GetAmmoCount( char *szName, bool considerWeaponClips=false ) const;
	...
	// damage scalers (not scalars :P), for afflictions and abilities
	virtual void ScaleDamageDealt(float f) { m_flDamageDealtScale *= f; }
	virtual void ScaleDamageReceived(float f) { m_flDamageReceivedScale *= f; }
	virtual float GetDamageDealtScale() { return m_flDamageDealtScale; }
	virtual float GetDamageReceivedScale() { return m_flDamageReceivedScale; }
	virtual void ResetDamageScales() { m_flDamageDealtScale = m_flDamageReceivedScale = 1.0f; }

	// get the most likely thing I'm aiming at
	virtual CBaseEntity *GetAimTarget(bool bFriendly = false) { return NULL; }
	...
	CUtlVector<std::string> m_storedDamageCommands;

	// modules
	int GetModuleLevel(Module *a);
	int GetModuleLevel(int index);
	void SetModuleLevel(Module *a,int level);
	bool HasModule(Module *a);
	bool HasModule(int index);
	bool IsModuleActive(Module *a);
	bool IsModuleActive(int index);
	...
	// used for minions, lasers, magmines, etc ... for attributing damage to the correct module
	void SetControllingModule(int index) { m_iControllingModule = index; }
	int GetControllingModule() { return m_iControllingModule; }
	
	// buffs
	virtual bool ApplyBuff(int buffIndex, CBaseCombatCharacter *pInflictor=NULL, int level=1);
	bool IsBuffActive(Buff *a);
	bool IsBuffActive(int i);
	void RemoveAllBuffs();
	void RemoveBuff(int i);
	float GetBuffEndTime(int i) { return m_flBuffEndTimes[i]; }
	float GetBuffTickTime(int i) { return m_flBuffCooldownTimes[i]; }
	void SetBuffTickTime(int i, float f) { m_flBuffCooldownTimes[i] = f; }
	int GetBuffLevel(int i) { return m_iBuffs[i]; }
	CBaseCombatCharacter *GetInflictor(int i) { return m_hInflictors[i]; }

	// aux power
	virtual float GetAuxPower() { return 0; }
	virtual void DrainAuxPower(float f) { }
	virtual void AddAuxPower(float f) { }
	virtual void SetAuxPower(float f) { }
	virtual float GetMaxAuxPower() { return 0; }
	
	// cloaking on / off
	void EnableCloaking(bool bOn,bool bIsFiring = false);

	// score tokens for "hoarder" game mode
	int GetNumScoreTokens() { return m_iNumScoreTokens; }
...
protected:
	CNetworkArray(int, m_iModules, NUM_MODULES);
	CNetworkArray(bool, m_bModulesActive, NUM_MODULES);
	CNetworkArray(float, m_flModuleCooldownTime, NUM_MODULES);
	bool m_bEnqueuedModules[NUM_MODULES];
	float m_flEnqueuedCastTime[NUM_MODULES];
	float m_flModuleStartTime[NUM_MODULES];
...
public:
	...
	virtual void SetRenderColorA( byte a );
	virtual void SetMaxRenderColorA( byte a );
...
public:
	...
	CNetworkVar( int, m_iParticleEffect );
...
private:
	...
	int m_iControllingModule;

	CNetworkVar(int, m_iNumScoreTokens);

protected:
	void		ManageModules();	// players and npcs use the same code for this
	void		ManageBuffs();// players and npcs use the same code for this
...
private:
	...
	static Relationship_t		m_hate;
	static Relationship_t		m_loathe;
	static Relationship_t		m_like;
	static Relationship_t		m_ignore;
	...
	// damage dealt & damage received scaling
	float m_flDamageDealtScale;
	float m_flDamageReceivedScale;

	// all buffs now apply to basecombatcharacters rather than just players
	CNetworkArrayForDerived( bool, m_bBuffs, NUM_BUFFS); // for networking only
	CHandle<CBaseCombatCharacter> m_hInflictors[NUM_BUFFS];
	float m_flBuffCooldownTimes[NUM_BUFFS];
protected:
	CNetworkArrayForDerived( int, m_iBuffs, NUM_BUFFS);
	CNetworkArrayForDerived( float, m_flBuffEndTimes, NUM_BUFFS);
};
