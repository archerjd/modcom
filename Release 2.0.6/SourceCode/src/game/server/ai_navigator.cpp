...
AIMoveResult_t CAI_Navigator::MoveClimb() 
{
	...
	AIMoveTrace_t moveTrace;
	double offsetMag = min(0.1,(climbDist - 0.005));
	if ( climbDist > 0.01 &&
		 !GetMoveProbe()->MoveLimit( NAV_CLIMB, GetLocalOrigin(), GetLocalOrigin() + ( climbDir * offsetMag ), MASK_NPCSOLID, GetNavTargetEntity(), &moveTrace ) )
	{
