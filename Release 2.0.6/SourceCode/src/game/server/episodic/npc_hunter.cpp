...
bool CHunterFlechette::CreateSprites( bool bBright )
{
	if ( bBright )
	{
		...
		m_iParticleEffect = HUNTER_FLECHETTE_STRIDERBUSTER;
	}
	else
	{
		//DispatchParticleEffect( "hunter_flechette_trail", PATTACH_ABSORIGIN_FOLLOW, this );
		m_iParticleEffect = HUNTER_FLECHETTE;
	}
...
void CHunterFlechette::Precache()
{
	...
	PrecacheParticleSystem( "flechette_halo" );
}
...
void CHunterFlechette::StickTo( CBaseEntity *pOther, trace_t &tr )
{
	...
	bool bAttachedToBuster = false;//StriderBuster_OnFlechetteAttach( pOther, vecVelocity );
...
void CHunterFlechette::Explode()
{
	...
	RadiusDamage( CTakeDamageInfo( this, GetOwnerEntity(), m_iBlastDamage, nDamageType ), GetAbsOrigin(), sk_hunter_flechette_explode_radius.GetFloat(), CLASS_NONE, NULL );
...
impactdamagetable_t s_HunterImpactDamageTable =
{
	...
	360*360,	// maximum angular speed squared (360 deg/s to cause spin/slice damage)
...
public:
	...
	void			Precache();
	void			Spawn();
	virtual entity_effect_t GetSpawnEffect() { return SPAWN_HUMAN; } // may need a custom-sized one?
	void			PostNPCInit();
	void			Activate();
	void			UpdateOnRemove();
	void			OnRestore();
...
IMPLEMENT_SERVERCLASS_ST( CNPC_Hunter, DT_NPC_Hunter )
...
void CNPC_Hunter::Spawn()
{
	...
	//m_pKv->SaveToFile( g_pFullFileSystem, UTIL_VarArgs("hunter%i.txt",GetLevel()), "MOD" ); // check it here?
...
void CNPC_Hunter::GatherChargeConditions()
{
	...
	if ( gpGlobals->curtime < m_flNextChargeTime )
		return;
...
int CNPC_Hunter::SelectCombatSchedule()
{
	...
	{
		if ( random->RandomInt( 1, 100 ) < 100*GetStatValue("charge_chance") )
		{
			//if ( OccupyStrategySlot( SQUAD_SLOT_HUNTER_CHARGE ) ) // uh ...
			{
...
void CNPC_Hunter::TaskFindDodgeActivity()
{
	...
	while ( !bFoundDir && ( nTries < 2 ) )
	{
		...
		trace_t tr;
		HunterTraceHull_SkipPhysics( GetAbsOrigin(), GetAbsOrigin() + vecDelta, testHullMins, GetHullMaxs(), MASK_NPCSOLID, this, GetCollisionGroup(), &tr, VPhysicsGetObject() ? VPhysicsGetObject()->GetMass() * 0.5f : 100.0f );
	...
	switch ( pTask->iTask )
	{
		...
		case TASK_HUNTER_CHARGE:
		{
			...
			if ( eActivity == ACT_HUNTER_CHARGE_STOP || eActivity == ACT_HUNTER_CHARGE_CRASH ) 
			{
				if ( IsActivityFinished() )
				{
					m_flNextChargeTime = random->RandomFloat(GetStatValue("min_charge_delay"), GetStatValue("max_charge_delay"));// gpGlobals->curtime + hunter_charge_min_delay.GetFloat() + random->RandomFloat( 0, 2.5 ) + random->RandomFloat( 0, 2.5 );
...
void CNPC_Hunter::ChargeLookAhead( void )
{
	...
	Vector testHullMins = GetHullMins();
	testHullMins.z += (StepHeight() * 2);
	HunterTraceHull_SkipPhysics( GetAbsOrigin(), vecTestPos, testHullMins, GetHullMaxs(), MASK_SHOT_HULL, this, COLLISION_GROUP_NONE, &tr, VPhysicsGetObject() ? VPhysicsGetObject()->GetMass() * 0.5f : 100.0f );
...
float CNPC_Hunter::ChargeSteer()
{
	...
	// Probe
	HunterTraceHull_SkipPhysics( GetAbsOrigin(), testPos, testHullMins, GetHullMaxs(), MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr, VPhysicsGetObject() ? VPhysicsGetObject()->GetMass() * 0.5f : 100.0f );
...
void CNPC_Hunter::ChargeDamage( CBaseEntity *pTarget )
{
	...
	float flDamage = ( pPlayer == NULL ) ? 250 : GetStatValue("dmg_charge");
...
bool CNPC_Hunter::HandleChargeImpact( Vector vecImpact, CBaseEntity *pEntity )
{
	...
	if ( pEntity->GetMoveType() == MOVETYPE_VPHYSICS )
	{
		...
		if ( pPhysics )
		{
			...
			float entMass = PhysGetEntityMass( pEntity ) ;
			float minMass = VPhysicsGetObject() ? VPhysicsGetObject()->GetMass() * 0.5f : 100.0f;
			if ( entMass < minMass )
			{
...
void CNPC_Hunter::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_HUNTER_MELEE_ATTACK_LEFT )
	{
		...
		MeleeAttack( HUNTER_MELEE_REACH, GetStatValue("dmg_slash"), QAngle( 25, 30, -20 ), right + forward, HUNTER_BLOOD_LEFT_FOOT );
		return;
	}
...
/*				if ( gpGlobals->curtime - StriderBuster_GetPickupTime( pTarget ) > hunter_hate_held_striderbusters_delay.GetFloat())
			...
			bool bAttached = false;//StriderBuster_IsAttachedStriderBuster( pTarget );
...
int CNPC_Hunter::MeleeAttack1Conditions ( float flDot, float flDist )
{
	...
	if (flDot < 0.7 && ( GetEnemy() == NULL || GetEnemy()->Classify() != CLASS_BULLSEYE ))
	{
		return COND_NOT_FACING_ATTACK;
	}
...
int CNPC_Hunter::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( pInflictor )
	{
		...
		{
			if ( !( (CHunterFlechette*)pInflictor )->IsPlayerOwned() )
			{
				// Flechettes only hurt us if they were launched by the player. This prevents
	...
	// Spark at 30% health.
	if ( !IsBleeding() && ( GetHealth() <= GetStatValue("health") * 0.3 ) )
	{
		StartBleeding();
	}
...
void CNPC_Hunter::DoMuzzleFlash( int nAttachment )
{
	...
	unsigned char uchAttachment = nAttachment;
	
	EntityMessageBegin( this, true );
		WRITE_BYTE( HUNTERFX_MUZZLEFLASH );
		//WRITE_LONG( entindex() );
...
bool CNPC_Hunter::ShouldSeekTarget( CBaseEntity *pTargetEntity, bool bStriderBuster )
{
	...
	if ( bStriderBuster )
	{
		...
		else if ( true )//StriderBuster_NumFlechettesAttached( pTargetEntity ) == 0 )
		{
...
bool CNPC_Hunter::ShootFlechette( CBaseEntity *pTargetEntity, bool bSingleShot )
{
	...
	pFlechette->SetDamage( GetStatValue("dmg_shot"), GetStatValue("dmg_shot_hit") );
