...
{
	return (pSightEnt->Classify() == CLASS_BULLSEYE && ( GetOuter()->GetAbsOrigin() - pSightEnt->GetAbsOrigin()).LengthSqr() < Square(96) ) || ( GetOuter()->FInViewCone( pSightEnt ) && GetOuter()->FVisible( pSightEnt ) );
}
...
//		while ( pEnt )
		for ( int i=0; i<g_AI_SensedObjectsManager.Count(); i++ )
		{
			CBaseEntity *pEnt = g_AI_SensedObjectsManager.Get(i);
			if ( !pEnt )
				continue;
...
CBaseEntity *CAI_SensedObjectsManager::Get( int pos )
{
	if ( pos >= 0 && pos < m_SensedObjects.Count() )
		return m_SensedObjects[pos];
