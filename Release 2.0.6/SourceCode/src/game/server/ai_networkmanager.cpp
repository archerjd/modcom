...
void CAI_NetworkManager::InitializeAINetworks()
{
	...
	{
		char szNodeTextFilename[MAX_PATH], szAirNodeTextFilename[MAX_PATH];
		Q_snprintf( szNodeTextFilename, sizeof( szNodeTextFilename ), "maps/graphs/%s_graph.txt", STRING( gpGlobals->mapname ));
		Q_snprintf( szAirNodeTextFilename, sizeof( szAirNodeTextFilename ), "maps/graphs/%s_airgraph.txt", STRING( gpGlobals->mapname ));

		int numGround = ReadNodeTextFile(szNodeTextFilename, "info_node"),
			   numAir = ReadNodeTextFile(szAirNodeTextFilename, "info_node_air");
		
		if ( numGround > 0 || numAir > 0 )
		{
			Msg("Created %i ground nodes and %i air nodes from text files\n",numGround, numAir);
...
int CAI_NetworkManager::ReadNodeTextFile(const char *szFilename, const char *szNodeType)
{
	CUtlBuffer buf( 0, 0, CUtlBuffer::TEXT_BUFFER );
	if ( filesystem->ReadFile( szFilename, "MOD", buf ) )
	{
		if (!buf.Size())
			return 0;
		...
		while( true )
		{
			...
			//DevMsg(" splitting floats...\n");
			if ( floats.Count() < 3 )
				continue; // windows line endings. linux doesn't like

			Vector origin( atof( floats[0] ), atof( floats[1] ), atof( floats[2] ) );
			//DevMsg("  Parsed Vector(%.2f, %.2f, %.2f)\n", VectorExpand(origin));

			floats.PurgeAndDeleteElements();
			//DevMsg("  purging...");

			// now we want to create a CNodeEnt (info_node) at the location these coordinates describe
			CNodeEnt *pNode = (CNodeEnt*)CBaseEntity::CreateNoSpawn( szNodeType, origin, vec3_angle, NULL );
			//DevMsg("  made node...");
			...
			{//	setting this index stops it moaning, doesn't seem to affect anything else though
				...
				DispatchSpawn(pNode); // spawning it adds it into the node graph
				//DevMsg("  spawned node\n");
			}
			num ++;
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if there's a node text file, and its newer than the network file
//-----------------------------------------------------------------------------
bool CAI_NetworkManager::IsTextFileNewer( const char *szMapName )
{
	char szGraphFilename[MAX_PATH], szTextFilename[MAX_PATH], szAirTextFilename[MAX_PATH];
	Q_snprintf( szGraphFilename, sizeof( szGraphFilename ), "maps/graphs/%s%s.ain", szMapName, GetPlatformExt() );
	Q_snprintf( szTextFilename, sizeof( szTextFilename ), "maps/graphs/%s_graph.txt", szMapName );
	Q_snprintf( szAirTextFilename, sizeof( szAirTextFilename ), "maps/graphs/%s_airgraph.txt", szMapName );
	...
	if ( filesystem->FileExists( szAirTextFilename ) )
	{
		int iCompare;
		if ( engine->CompareFileTime( szAirTextFilename, szGraphFilename, &iCompare ) )
		{
			if ( iCompare > 0 )
			{
				Msg("Text file is newer\n");
	...
	return false; // if there's no text file, or neither is newer, we ignore them
}
