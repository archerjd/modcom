...
#ifdef USE_OMNIBOT
int CNPC_Antlion::GetOmnibotClass() const {
	return IsWorker() ? Omnibot::MC_CLASSEX_ANTLION_WORKER : Omnibot::MC_CLASSEX_ANTLION;
}
#endif
...
//#else										// DRYDEN: This whole section was commented out because who CARES IF BLOOD IS YELLOW.
//	SetModel( ANTLION_MODEL );
...
//#endif // HL2_EPISODIC
	...
	// the first skin is exclusively for workers, when we have them share a model
	/*if ( IsWorker() )
		m_nSkin = 0;
	else
		m_nSkin = random->RandomInt( 1, ANTLION_SKIN_COUNT-1 );*/
...
void CNPC_Antlion::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( IsWorker() && !m_bDontExplode )
	{
		...
		CTakeDamageInfo info( this, MyAttacker(), GetStatValue("dmg_burst"), DMG_BLAST_SURFACE | ( ANTLION_WORKER_BURST_IS_POISONOUS() ? DMG_POISON : DMG_ACID ) );
		Event_Gibbed( info );
	}
...
void CNPC_Antlion::MeleeAttack( float distance, float damage, QAngle &viewPunch, Vector &shove )
{
	...
	if ( ( GetEnemy() != NULL ) && ( GetEnemy()->Classify() == CLASS_BULLSEYE ) )
	{
		...
		CTakeDamageInfo info( this, MyAttacker(), damage, DMG_SLASH );
	...
	if ( pHurt )
	{
		...
		{
			CTakeDamageInfo	dmgInfo( this, MyAttacker(), pHurt->m_iHealth+25, DMG_SLASH );
...
bool CNPC_Antlion::InnateWeaponLOSCondition( const Vector &ownerPos, const Vector &targetPos, bool bSetConditions )
{
	...
	{
		return GetSpitVector( GetSpitPos(), targetPos, &m_vecSaveSpitVelocity );
	}
...
Vector CNPC_Antlion::GetSpitPos()
{
	Vector facingDir;
	AngleVectors( GetAbsAngles(), &facingDir );
	return GetAbsOrigin() + Vector(0,0,13) + facingDir * 8;
}
...
//#if HL2_EPISODIC
	...
	return true;
//#else					// DRYDEN: Commented out. Workers exist when I say they do.
//	return false;
//#endif
}
...
//#endif // HL2_EPISODIC
	...
	{
		MeleeAttack( ANTLION_MELEE1_RANGE, GetStatValue("dmg_swipe"), QAngle( 20.0f, 0.0f, -12.0f ), Vector( -250.0f, 1.0f, 1.0f ) );
		return;
	}

	if ( pEvent->event == AE_ANTLION_MELEE_HIT2 )
	{
		MeleeAttack( ANTLION_MELEE1_RANGE, GetStatValue("dmg_swipe"), QAngle( 20.0f, 0.0f, 0.0f ), Vector( -350.0f, 1.0f, 1.0f ) );
		return;
	}

	if ( pEvent->event == AE_ANTLION_MELEE_POUNCE )
	{
		MeleeAttack( ANTLION_MELEE2_RANGE, GetStatValue("dmg_swipe"), QAngle( 4.0f, 0.0f, 0.0f ), Vector( -250.0f, 1.0f, 1.0f ) );
		return;
	}
...
//		ResetIdealActivity( ACT_RANGE_ATTACK1 );
		...
		if ( GetEnemy() )
		{
			...
			for ( int i = 0; i < 6; i++ )
			{
				...
				pGrenade->SetDamage( GetStatValue("dmg_spit") );
...
void CNPC_Antlion::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	...
	if ( IsFlipped() )
	{
		...
		newInfo.ScaleDamage( 1.5f ); // reduced greatly from 4
	}
...
int CNPC_Antlion::MeleeAttack1Conditions( float flDot, float flDist )
{
	...
	if ( flPrDot < 0.5f && ( GetEnemy() == NULL || GetEnemy()->Classify() != CLASS_BULLSEYE ))
		return COND_NOT_FACING_ATTACK;
		...
		if ( GetSequence() != sequence )
		{
			...
			if ( GetEnemy() && GetEnemy()->IsPlayer()  )
			{
				...
				if ( pPlayer && pPlayer->IsInAVehicle() == false )
					 MeleeAttack( ANTLION_MELEE1_RANGE, GetStatValue("dmg_swipe"), QAngle( 4.0f, 0.0f, 0.0f ), Vector( -250.0f, 1.0f, 1.0f ) );
			}
...
//#ifdef HL2_EPISODIC 
	...
	{
		CTakeDamageInfo	dmgInfo( this, MyAttacker(), GetStatValue("dmg_in_air"), DMG_SLASH );
...
void CNPC_Antlion::DoPoisonBurst()
{
	if ( GetWaterLevel() < 2 )
	{
		CTakeDamageInfo info( this, MyAttacker(), GetStatValue("dmg_swipe"), DMG_BLAST_SURFACE | ( ANTLION_WORKER_BURST_IS_POISONOUS() ? DMG_POISON : DMG_ACID ) );
...
AI_BEGIN_CUSTOM_NPC( npc_antlion, CNPC_Antlion )
	...
	DECLARE_TASK( TASK_ANTLION_SPIT )
...
//		"		TASK_RANGE_ATTACK1		0"
		"		TASK_SET_ACTIVITY		ACTIVITY:ACT_ANTLION_LAND"
		"		TASK_ANTLION_SPIT		0"
		""
		"	Interrupts"
		"		COND_TASK_FAILED"
...
{
public:
	DECLARE_CLASS( CNPC_Antlion_Worker, CNPC_Antlion  );
...
LINK_ENTITY_TO_CLASS( npc_antlion_worker, CNPC_Antlion_Worker );