...
//
// Purpose:	Antlion Guard	-- Mine beautiful Antlion Guard
//
//=============================================================================//
...
//#if HL2_EPISODIC - DRYDEN: TIME TO GET BLOODY.
...
//#pragma message ("Wow it works! - Im in your compile output! (Josh)")
// Spawnflags 
...
#define MIN_CRUSH_DAMAGE		25.0f					//Originally 20.0f - changed for lowering physics damage interuptions
...
// Antlion summoning
#define ANTLIONGUARD_SUMMON_COUNT		10				// because 10 is better than 3

// Sight
#define	ANTLIONGUARD_FOV_NORMAL			-0.4f

// cavern guard's poisoning behavior					This is purely test for now - poison > aids plague
//#if HL2_EPISODIC
...
public:
	...
	virtual entity_effect_t GetSpawnEffect() { return SPAWN_LARGE; }
...
//#if HL2_EPISODIC		--- uncommented for antlion guard... not sure if its the best thing to do but well see (Josh)
	//---------------------------------
...
{
	BaseClass::Activate();
	m_iLastBoomTime = gpGlobals->curtime;
...
int CNPC_AntlionGuard::SelectSchedule( void )
{
	...
	}																-- Commented out to remove flinching on heavy weapon damage(Josh)*/
...
*/
	...
	if ( flDot < ANTLIONGUARD_MELEE1_CONE && ( GetEnemy() == NULL || GetEnemy()->Classify() != CLASS_BULLSEYE ) )
...
void CNPC_AntlionGuard::Shove( void )
{
	...
	{
		pTarget->TakeDamage( CTakeDamageInfo( this, this, GetStatValue("dmg_shove"), DMG_CRUSH ) );
		m_hShoveTarget = NULL;
		return;
	}

	float damage = GetStatValue("dmg_shove");//( pTarget->IsPlayer() ) ? sk_antlionguard_dmg_shove.GetFloat() : 250;
...
//#if HL2_EPISODIC																					-- Poison damage if trapped (Josh)
			...
			{ 
				// That didn't finish them. Take them down to one point with poison damage. "It'll heal."  - Confucious 479 BC
...
public:
	...
	virtual bool ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask )
	{
		...
		if ( pEntity )
		{
			...
			float	flDamage = m_pAttacker->GetStatValue("dmg_shove");//( pEntity->IsPlayer() ) ? sk_antlionguard_dmg_shove.GetFloat() : 250;;
			...
			if ( pVictimBCC )
			{
				...
				if ( !HL2MPRules()->IsFriendly(m_pAttacker, pEntity) )
				{
...
void CNPC_AntlionGuard::Footstep( bool bHeavy )
{
	...
	if ( bHeavy )
	{
		if ( flNearVolume > 0.0f )
		{
			...
			UTIL_ScreenShake( GetAbsOrigin(), 0.2f, 20.0f, 0.2f, 1000.0f, SHAKE_START ); //shake the world up a bit when she comes a runnin
		}
...
void CNPC_AntlionGuard::HandleAnimEvent( animevent_t *pEvent )
{
	...
	{
		UTIL_ScreenShake( GetAbsOrigin(), 40.0f, 4.0f, 3.0f, 512, SHAKE_START ); // modded for more 'shock', original: GetAbsOrigin(), 32.0f, 4.0f, 1.0f, 512, SHAKE_START (Josh)
		EmitSound( "NPC_AntlionGuard.HitHard" );
...
int CNPC_AntlionGuard::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( IsHeavyDamage( dInfo ) )
	{
		...
		{
			dInfo.SetDamage( 50 );
		}			-- commented out for custom CombineBall damage (Josh)*/
	...
	if ( !m_bInCavern && HasCondition( COND_HEAVY_DAMAGE ) == false && ( info.GetDamageType() & DMG_BULLET ) )
	{
		...
		/* Flinch if we should - but we shouldnt so we comment it out
		if ( bTakeHeavyDamage )
		{
			// Set our response animation
			SetHeavyDamageAnim( dInfo.GetDamagePosition() );
...
//#if HL2_EPISODIC																				-- commented for poison ability (Josh)
	// If I am a cavern guard attacking the player, and he still lives, then poison him too.
	...
	{
		// That didn't finish them. Take them down to one point with poison damage. It'll heal. - "Finish him!" - Mortal Kombat Announcer 200BC
	...
	virtual bool ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask )
	{
		...
		if ( pEntity )
		{
			...
			if ( pEntity->Classify() == CLASS_ANTLION ||
				 pEntity->Classify() == CLASS_ZOMBIE  ||
				 pEntity->Classify() == CLASS_HEADCRAB||
				 pEntity->Classify() == CLASS_VORTIGAUNT)
			{
				CBaseEntity *pGuard = (CBaseEntity*)EntityFromEntityHandle( m_pPassEnt );
				ApplyChargeDamage( pGuard, pEntity, pEntity->GetHealth() + 500 );
				return false;
			}
		}
...
void CNPC_AntlionGuard::ChargeLookAhead( void )
{
	...
	if ( tr.fraction != 1.0 )
	{
		...
		AddGesture( ACT_ANTLIONGUARD_CHARGE_ANTICIPATION ); // Get ready for a head on collision and whack the hell out of it
		m_iNextBoomTime = gpGlobals->curtime;
		if(m_iNextBoomTime >= m_iLastBoomTime)
		{
		ImpactShock( WorldSpaceCenter(), 150, 400.0f );		// Used to GTFO any objects in my way when charging
		m_iLastBoomTime = gpGlobals->curtime + 0.5f;		// Resetting the next allowed boomtime to half a second from now
		}
	}
}
...
bool CNPC_AntlionGuard::HandleChargeImpact( Vector vecImpact, CBaseEntity *pEntity )
{
	...
	if ( FStrEq(pEntity->GetClassname(), "hunter_flechette") )
		return false; // don't let flechettes stop you charging!

	// Hit anything we don't like
	if ( !HL2MPRules()->IsFriendly(this, pEntity) && ( GetNextAttack() < gpGlobals->curtime ) )
	{
		EmitSound( "NPC_AntlionGuard.Shove" );
		...
		if ( !pEntity->IsNPC() && (pEntity->GetMoveType() != MOVETYPE_VPHYSICS || pEntity->VPhysicsGetObject() != NULL) ) // only punt things that aren't NPCs, as some NPC was having problems
...
float CNPC_AntlionGuard::ChargeSteer( void )
{
	...
	steer += ( right * 0.7f ) * ( 1.0f - tr.fraction ); // modded (Josh) - original: steer += ( right * 0.5f ) * ( 1.0f - tr.fraction );
...
void CNPC_AntlionGuard::RunTask( const Task_t *pTask )
{
	...
	{
		// the cavern antlion can clothesline gordon - WWE style
//		if ( m_bInCavern )
//		{

			// See if we're going to run into anything soon (slightly troublesome task with no eyes)
			ChargeLookAhead();
...
void CNPC_AntlionGuard::SummonAntlions( void )
{
	...
	for ( int i = 0; (m_iNumLiveAntlions < ANTLIONGUARD_SUMMON_COUNT) && (iSpawnPoint < ARRAYSIZE(sAntlionSpawnPositions)); i++ )
	{
		...
		// Ensure it's dirt or sand <----------------| FOR SPAWNING ANTLIONS this is an intrest point (Josh)
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_AntlionGuard::PrescheduleThink( void ) //Sounds familiar
{
...
void CNPC_AntlionGuard::ImpactShock( const Vector &origin, float radius, float magnitude, CBaseEntity *pIgnored )
{
	...
	while ( ( pEntity = gEntList.FindEntityInSphere( pEntity, origin, radius ) ) != NULL )
	{
		...
		if ( pEntity == this )
			continue;
		if ( pEntity->IsPlayer() && !ToHL2MPPlayer(pEntity)->IsInCharacter() )
			continue;
...
void CNPC_AntlionGuard::ChargeDamage( CBaseEntity *pTarget )
{
	...
	float flDamage = GetStatValue("dmg_shove");//( pPlayer == NULL ) ? 250 : sk_antlionguard_dmg_charge.GetFloat();
...
bool CNPC_AntlionGuard::IsHeavyDamage( const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageType() & DMG_CRUSH )
	{
		...
		{
			/* Always take hits from a combine ball - Not anymore, combine balls are too cold --- (Josh)
			...
			// "Ignore physics damages that don't do much damage" - Arnold 'Terminator' Schwazenegger 2500BC
...
//#if HL2_EPISODIC		---- Commented out as the poor antlion guard keeps having anxiety attacks and freezing up (Josh)
//---------------------------------------------------------
