...
#define	VORTIGAUNT_LIMP_HEALTH				25
...
extern McConVar mc_vortigaunt_armor_charge_per_token, mc_vortigaunt_armor_charge_per_level, sk_vortigaunt_zap_range;
...
#ifdef USE_OMNIBOT
int CNPC_Vortigaunt::GetOmnibotClass() const {
	return Omnibot::MC_CLASSEX_VORTIGAUNT;
}
#endif
...
void CNPC_Vortigaunt::StartTask( const Task_t *pTask )
{
	...
	case TASK_VORTIGAUNT_HEAL:
	{
		...
		float flArmorDelta = (float) min(pPlayer->GetMaxArmor(),GetLevel() * mc_vortigaunt_armor_charge_per_level.GetFloat()) - pPlayer->ArmorValue();
		m_nNumTokensToSpawn = ceil( flArmorDelta / mc_vortigaunt_armor_charge_per_token.GetInt() );
	...
	case TASK_RANGE_ATTACK1:
		{
			...
			// limit yaw speed for the duration of the charge-up
			//float prevYawSpeed = GetMotor()->GetYawSpeed();
			//GetMotor()->SetYawSpeed(prevYawSpeed*0.1f);
			BaseClass::RunTask( pTask );
			AutoMovement( );
			//GetMotor()->SetYawSpeed(prevYawSpeed);
			break;
		}
...
{
	if ( InAttackSequence() )
		return 12; // don't turn too quickly when zapping
...
int CNPC_Vortigaunt::MeleeAttack1Conditions( float flDot, float flDist )
{
	...
	{
		// if I have a ranged attack, only do this when 2 or more enemies nearby. Otherwise, its my only attack, so do it for anyone
		if ( NumEnemiesInRadius(128) > ((CapabilitiesGet() & bits_CAP_INNATE_RANGE_ATTACK1) ? 1 : 0) )
		{
			m_flDispelTestTime = gpGlobals->curtime + GetStatValue("dispel_interval");
			return COND_VORTIGAUNT_DISPEL_ANTLIONS;
		}
	}

	m_flDispelTestTime = gpGlobals->curtime + 0.5f;
	return COND_NONE;
}
...
// Output : int
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt::NumEnemiesInRadius( float flRadius )
{
	CBaseEntity *sEnemySearch[16];
	int nNumEnemies = 0;
	int nNumEntities = UTIL_EntitiesInBox( sEnemySearch, ARRAYSIZE(sEnemySearch), GetAbsOrigin()-Vector(flRadius,flRadius,flRadius), GetAbsOrigin()+Vector(flRadius,flRadius,flRadius), FL_NPC|FL_GRENADE|FL_CLIENT );
	for ( int i = 0; i < nNumEntities; i++ )
	{
		if ( sEnemySearch[i] == NULL || HL2MPRules()->IsFriendly(this,sEnemySearch[i]) )
			continue;
		nNumEnemies++;
	}
...
void CNPC_Vortigaunt::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_VORTIGAUNT_ZAP_SHOOT )
	{
		...
		Vector vecAim = DetermineZapTarget();

		if ( ( nHand == HAND_LEFT ) || (nHand == HAND_BOTH ) )
		{
			ZapBeam( HAND_LEFT, vecAim );
		}
		
		if ( ( nHand == HAND_RIGHT ) || (nHand == HAND_BOTH ) )
		{
			ZapBeam( HAND_RIGHT, vecAim );
		}
...
void CNPC_Vortigaunt::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( LikesMaster() && GetMasterPlayer() )
	{
		CHL2MP_Player *pPlayer = ToHL2MPPlayer( GetMasterPlayer() );
		if ( pPlayer )
		{// hack to make this clean up for 2, as it counts for 2
...
void CNPC_Vortigaunt::Precache()
{
	...
	PrecacheModel( "models/vortigaunt.mdl" );//STRING( GetModelName() ) );
...
CBaseEntity *CNPC_Vortigaunt::FindHealTarget( void )
{
	...
	CBaseEntity *pEntity = GetMasterPlayer() && (GetMasterPlayer()->GetAbsOrigin() - GetAbsOrigin()).Length() <= HEAL_SEARCH_RANGE ? GetMasterPlayer() : NULL;
...
/*
	...
	// and only friendly targets
	if ( !HL2MPRules()->IsFriendly(this,pTarget) )
		return false;
...
//	if ( pPlayer->ArmorValue() >= (sk_vortigaunt_armor_charge.GetFloat()*0.66f) )
	...
	if ( PlayerBelowHealthPercentage( pPlayer, PLAYER_CRITICAL_HEALTH_PERC ) == false || m_bPlayerRequestedHeal )
	{
		...
		if ( GetEnemy() )
			return false;
/* 
		// DRYDEN: We don't care if the player is damaged anymore.
		// No recent damage
...
int CNPC_Vortigaunt::SelectHealSchedule( void )
{
	...
	if ( m_LeadBehavior.HasGoal() || GetState() != NPC_STATE_IDLE )
		return SCHED_NONE;
...
/*
		...
		CBasePlayer *pPlayer = GetMasterPlayer();
...
int CNPC_Vortigaunt::SelectSchedule( void )
{
	...
	if ( HasCondition( COND_VORTIGAUNT_CAN_HEAL ) && GetState() == NPC_STATE_IDLE )
		return SCHED_VORTIGAUNT_HEAL;
	...
	if ( m_eHealState == HEAL_STATE_HEALING )
	{
		...
		if ( HasCondition( COND_VORTIGAUNT_HEAL_VALID ) )
		{
			if ( m_flNextHealTokenTime < gpGlobals->curtime )
			{
				...
				if ( m_nNumTokensToSpawn <= 0 || ( m_bForceArmorRecharge == false && ( pPlayer && pPlayer->ArmorValue() >= pPlayer->GetMaxArmor() ) ) )
				{
					m_flHealHinderedTime = 0.0f;
					m_nNumTokensToSpawn = 0;
...
Vector CNPC_Vortigaunt::DetermineZapTarget()
{
	Vector forward;
	Vector vecSrc = GetAbsOrigin() + GetViewOffset();
	GetVectors( &forward, NULL, NULL );

	Vector vecAim = forward;//GetShootEnemyDir( vecSrc, false );	// We want a clear shot to their core

	if ( GetEnemy() )
	{
		Vector vecTarget = GetEnemy()->WorldSpaceCenter() + Vector(0,0,random->RandomFloat(-16,16));//BodyTarget( vecSrc, false );
	...
	// now add some small inaccuracy to vecAim
	//DebugDrawLine(vecSrc, vecSrc + ( vecAim * InnateRange1MaxRange()*2 ), 255, 64, 0, false, 5);
	QAngle angle;
	VectorAngles(vecAim, angle);
	float inaccuracy = GetStatValue("zap_spread")/2.0f;
	angle[PITCH] += random->RandomFloat(-inaccuracy,inaccuracy);
	angle[YAW] += random->RandomFloat(-inaccuracy,inaccuracy);
	AngleVectors(angle,&vecAim);
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt::ZapBeam( int nHand, Vector vecAim )
{
	Vector vecSrc = GetAbsOrigin() + GetViewOffset();
	//DebugDrawLine(vecSrc, vecSrc + ( vecAim * InnateRange1MaxRange()*2 ), 0, 255, 128, false, 5);
	trace_t tr;
...
*/	{
		AI_TraceLine( vecSrc, vecSrc + ( vecAim * InnateRange1MaxRange()*2 ), MASK_SHOT, this, COLLISION_GROUP_NONE, &tr);
	}
	...
	if ( pEntity != NULL && m_takedamage )
	{
		...
		CTakeDamageInfo dmgInfo( this, MyAttacker(), GetStatValue("dmg_zap"), DMG_SHOCK );
...
bool CNPC_Vortigaunt::HealGestureHasLOS( void )
{
	...
	CBaseEntity *pTargetEnt = GetMasterPlayer();
	if ( pTargetEnt == NULL )
		return false;
...
void CNPC_Vortigaunt::DispelAntlions( const Vector &vecOrigin, float flRadius, bool bDispel /*= true*/ )
{
	...
	// Hurt enemies all around us ... and make them flip if they're antlions!
	trace_t tr;
	CBaseEntity *pEnemySearch[32];
	int nNumEnemies = UTIL_EntitiesInBox( pEnemySearch, ARRAYSIZE(pEnemySearch), vecOrigin-Vector(flRadius,flRadius,flRadius), vecOrigin+Vector(flRadius,flRadius,flRadius), FL_NPC|FL_CLIENT|FL_GRENADE );
	for ( int i = 0; i < nNumEnemies; i++ )
	{
		// We only flip antlions (and not workers)
		CBaseCombatCharacter *pEnemy = pEnemySearch[i]->MyCombatCharacterPointer();
		if ( pEnemy == NULL )
			continue;

		// don't let me hurt myself with the dispel blast, unless I'm a suicide vort
		if ( pEnemy == this && GetStatValue("can_hurt_self") == 0 )
			continue;
		
		CNPC_Antlion *pAntlion = NULL;
		if ( IsAntlion( pEnemy ) )
		{
			pAntlion = static_cast<CNPC_Antlion *>(pEnemySearch[i]);
			if ( pAntlion->IsWorker() )
				pAntlion = NULL; // don't flip workers
		}
		
		// Attempt to trace a line to hit the target
		UTIL_TraceLine( vecOrigin, pEnemy->BodyTarget( vecOrigin ), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr );
		if ( tr.fraction < 1.0f && tr.m_pEnt != pEnemy )
			continue;

		Vector vecDir = ( pEnemy->GetAbsOrigin() - vecOrigin );
		vecDir[2] = 0.0f;
		...
		if ( bDispel ) // only push non-antlions if we're doing a proper damage dispel
			pEnemy->ApplyAbsVelocityImpulse( vecDir );
		else if ( pAntlion )
		...
		if ( flDist < 128 && bDispel )
		{
			...
			CTakeDamageInfo dmgInfo( this, this, vecDir, pEnemy->GetAbsOrigin(), GetStatValue("dmg_dispel"), DMG_SHOCK );
			pEnemy->TakeDamage( dmgInfo );
		}
		
		// overturn any antlions we hit
		if ( pAntlion && pAntlion->IsAlive() )
		{
			// Turn them over
			pAntlion->Flip( true );
...
{
//	if( info.GetDamageType() & (DMG_CRUSH | DMG_BURN) )		// DRYDEN: We need physics deaths in our mod.
//		return 0;
...
//	CBasePlayer *pPlayer = ToBasePlayer( pOther );
	...
	if ( pPlayer->ArmorValue() < pPlayer->GetMaxArmor() )
	{
		pPlayer->IncrementArmorValue( mc_vortigaunt_armor_charge_per_token.GetInt(), pPlayer->GetMaxArmor() );
	}
