...
ConVar	rollermine_health_mode("rollermine_health_mode", "3", FCVAR_GAMEDLL | FCVAR_NOTIFY, "Controls how rollermines take damage. 1 is hl2 default; 1 health, only explosions hurt. 2 is normal health, explosions doing extra damage. 3 is normal health, only explosions doing damage.", true, 1, true, 3);
...
public:
	...
	void	Spawn( void );
	virtual entity_effect_t GetSpawnEffect() { return SPAWN_SMALL; }
	bool	CreateVPhysics();
	void	RunAI();
	...
	//void	OnKilledNPC( CBaseEntity *pKilled );  //This is actually called on killing an npc, no shit (Josh)
	...
	virtual bool IsMechanical() { return true; }
	...
	virtual void SetRollerSkin( void );
...
protected:
	...
	CNetworkVar( bool,	m_bHackedByAlyx );	//Will we ever use this? -would be fun to temp hack a rollermine(Josh)
...
void CNPC_RollerMine::Spawn( void )
{
	...
	if ( rollermine_health_mode.GetInt() != 1 ) // 2 and 3 use health, 1 doesn't
	{
...
{
	if ( m_pType->m_szModelName[0] != '\0' || m_pType->m_iSkinNumber != -1 )
		return; // don't mess with skins if we're using something else
...
void CNPC_RollerMine::CloseTouch( CBaseEntity *pOther )
{
	...
	if ( !bOtherIsDead && !bOtherIsNotarget && pOther->entindex() != 0 )
	{
...
{
	//Msg( "Using ShockTarget - shocking some shit?\n");
...
void CNPC_RollerMine::NotifyInteraction( CAI_BaseNPC *pUser )
{
	...
	// Play the hax0red sound HAXHAXHAX
...
void CNPC_RollerMine::ShockTouch( CBaseEntity *pOther )
{
	...
	if ( pOther->entindex() == 0 )
		return; // don't shock the freakin world, man!

	// bombs hop and go boom
	if ( IsBomb() )
	{
		Hop( 300 );
	...
	CTakeDamageInfo	info( this, this, vecForce, out, GetStatValue("dmg_shock"), DMG_SHOCK );
...
//-----------------------------------------------------------------------------
// Purpose: To handle what happens when you we kill npcs
//-----------------------------------------------------------------------------
/*void CNPC_RollerMine::OnKilledNPC( CBaseEntity *pKilled )
{
	if ( pKilled )
	{
		if ( !pKilled->IsNPC() || 
		...
		{
			Msg("\n\nOnKilledNPC was called - WOO \n\n");
			CBasePlayer *pPlayer = HasPhysicsAttacker( 6.0f ); //6 seconds
			if (pPlayer)
			{
...
void CNPC_RollerMine::OnPhysGunDrop( CBasePlayer *pPhysGunUser, PhysGunDrop_t Reason )
{
	...
	{
		//Msg( "BOOM cut that shit up - just shot me from physgun\n");

		m_hPhysicsAttacker = pPhysGunUser;
		m_flLastPhysicsInfluenceTime = gpGlobals->curtime;

		if ( m_bIsOpen ) // the zappy spike majigs
		{
			//m_bIsPrimed = true;
...
// Did someone toss me? if so who?
CBasePlayer *CNPC_RollerMine::HasPhysicsAttacker( float dt )
{
	// If the player is holding me now, or I've been recently thrown
	// then return a pointer to that player
	if ( m_bHeld || (gpGlobals->curtime - dt <= m_flLastPhysicsInfluenceTime) )
	{
		return m_hPhysicsAttacker; //TWAS ME MUAHAHA!
	}
	return NULL;
}
...
{
	if ( rollermine_health_mode.GetInt() > 1 ) // 1 doesn't use normal health, 2 & 3 do
	...
	if ( info.GetDamageType() & (DMG_BURN|DMG_BLAST|DMG_DIRECT) )
	{
...
int CNPC_RollerMine::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	int retVal;
	if ( rollermine_health_mode.GetInt() == 2 ) // normal health
	{
		if ( info.GetDamageType() & DMG_BURN )
			info.SetDamage( 0 ); // take no damage from fire

		if ( info.GetDamageType() & DMG_BLAST )
			info.ScaleDamage(3); // take a lot of damage from explosions, still

		retVal = CAI_BaseNPC::OnTakeDamage_Alive( info );
	}
	else // explosions hurt only
	{
		if ( !(info.GetDamageType() & DMG_BLAST|DMG_DIRECT) )
			info.SetDamage(0);
		retVal = CAI_BaseNPC::OnTakeDamage_Alive( info );
	}
...
void CNPC_RollerMine::Explode( void )
{
	...
	float expDamage = IsBomb() ? GetStatValue("dmg_explode") : 25;
	...
	{
		ExplosionCreate( WorldSpaceCenter(), GetLocalAngles(), m_hAttacker, expDamage, IsBomb() ? 256 : 128, true );
	}

	CTakeDamageInfo	info( this, m_hAttacker, 1, DMG_GENERIC );
	Event_Killed( info );
...
float CNPC_RollerMine::GetStunDelay()
{
	...
	{
		return GetStatValue("stun_delay");
	}
}
...
class CNPC_RollerBomb : public CNPC_RollerMine
{
public:
	DECLARE_CLASS( CNPC_RollerBomb, CNPC_RollerMine  );

	virtual bool IsBomb() { return true; }
};
LINK_ENTITY_TO_CLASS( npc_rollerbomb, CNPC_RollerBomb );