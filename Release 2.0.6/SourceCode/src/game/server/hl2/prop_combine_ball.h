...
public:
	...
	void SetModuleLaunched( bool state = true ) { m_bModuleLaunched = state; }
	bool WasModuleLaunched( void ) const { return m_bModuleLaunched; }
	...
	void SetModuleDamage( int damage ) { m_iDamage = damage; }
	int GetModuleDamage() { return m_iDamage; }
...
private:
	...
	{
		return ( m_nState != STATE_HOLDING && m_nMaxBounces != 0 && m_nBounceCount >= m_nMaxBounces );
	}
...
CBaseEntity *CreateCombineBall( const Vector &origin, const Vector &velocity, float radius, float mass, float lifetime, CBaseEntity *pOwner, bool IsModuleLaunched, int bounces, int damage = 0);
