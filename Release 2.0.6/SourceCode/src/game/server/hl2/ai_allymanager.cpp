...
*/
	...
	for ( int i = 0; i < nAIs; i++ )
	{
		if ( ppAIs[i]->IsAlive() && ppAIs[i]->IsPlayerAlly() )
		{
			...
			CBasePlayer *pNearestPlayer = UTIL_GetNearestPlayer(ppAIs[i]->GetAbsOrigin());
			if ( pNearestPlayer == NULL )
				continue;
				
			// They only count if I can use them.
			if( ppAIs[i]->IRelationType( pNearestPlayer ) != D_LI )
				continue;

			// Skip distant NPCs
			Vector vNearestPlayerPos = pNearestPlayer->GetAbsOrigin();
