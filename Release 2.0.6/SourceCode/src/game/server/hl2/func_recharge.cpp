...
public:
	...
	int GetOmnibotClass() const {
		return Omnibot::MC_CLASSEX_ENERGY_WALLUNIT;
	}
#endif
...
{ 
	...
	CHL2MP_Player *pl = ToHL2MPPlayer( pActivator );

	if ( pl->IsBuffActive(DEBUFF_ATTRITION) )
		return;
	...
	// charge the player
	int nMaxArmor = pl->GetMaxArmor();
	int nIncrementArmor = 1;
	if ( HasSpawnFlags(	SF_CITADEL_RECHARGER ) )
	{
		nMaxArmor = min(sk_suitcharger_citadel_maxarmor.GetInt(), nMaxArmor * 2);
		nIncrementArmor = 10;
...
{ 
	...
	CHL2MP_Player *pl = ToHL2MPPlayer( pPlayer );
	...
	// DRYDEN: In order to fail activation, we MUST have max armor and max aux power.
	if ( pPlayer->ArmorValue() >= nMaxArmor && pl->GetAuxPower() >= pl->GetMaxAuxPower() )
	{
	...
	if (  pl->GetAuxPower() < pl->GetMaxAuxPower() )
		pl->AddAuxPower( nIncrementArmor );
...
{
	if(ent && ent->GetOmnibotClass() == Omnibot::MC_CLASSEX_ENERGY_WALLUNIT)
	{
