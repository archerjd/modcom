...
IMPLEMENT_SERVERCLASS_ST( CBaseHeadcrab, DT_NPC_Headcrab )
END_SEND_TABLE()

#ifdef USE_OMNIBOT
int CBaseHeadcrab::GetOmnibotClass() const {
	return Omnibot::MC_CLASSEX_HEADCRAB;
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
{
	BaseClass::Precache();
	PrecacheScriptSound("Flesh_Bloody.ImpactHard"); //our bloody squishing noise for headcrab hopping fun
}	
...
{
	pInfo->Set( this, MyAttacker(), GetStatValue("dmg_melee"), DMG_SLASH );
...
#ifdef USE_OMNIBOT
int CFastHeadcrab::GetOmnibotClass() const {
	return Omnibot::MC_CLASSEX_HEADCRAB_FAST;
}
#endif
		...
		if ( pOther->IsAlive() && pOther->m_iHealth > 1)
		{
			...
			if ( hl2_episodic.GetBool() )
			{
				if ( pOther->IsPlayer() )
				{
					...
					pOther->TakeDamage( CTakeDamageInfo( this, MyAttacker(), pOther->m_iHealth - 1, DMG_POISON ) );
				}
				else
				{
					// Just take some amount of slash damage instead
					pOther->TakeDamage( CTakeDamageInfo( this, MyAttacker(), GetStatValue("dmg_melee"), DMG_SLASH ) );
				}
			}
			else
			{
