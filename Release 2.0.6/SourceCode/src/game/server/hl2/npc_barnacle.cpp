...
// Output :
//-----------------------------------------------------------------------------
int	CNPC_Barnacle::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	//	DropTongue(); no, this stops us sucking stuff up as we get hurt by that now
...
bool CNPC_Barnacle::CanPickup( CBaseCombatCharacter *pBCC )
{
	...
	// Don't pickup turrets or energy balls
	if( FClassnameIs( pBCC, "npc_turret_floor" ) || FClassnameIs( pBCC, "prop_combine_ball" ) )
		return false;
...
void CNPC_Barnacle::BitePrey( void )
{
	...
	int nDamage = GetStatValue("dmg_bite"); 
	...
	pVictim->TakeDamage( CTakeDamageInfo( this, GetMasterPlayer(), nDamage, iDamageType | DMG_CRUSH ) );
	...
	if ( enemyClass == CLASS_ZOMBIE )
	{
		...
		TakeDamage( CTakeDamageInfo( this, this, GetStatValue("dmg_bite_self"), iDamageType | DMG_CRUSH ) );
		return;
	}
...
CBaseEntity *CNPC_Barnacle::TongueTouchEnt ( float *pflLength )
{
	...
	for ( int i = 0; i < nCount; i++ )
	{
		...
		// don't lift gibs, period
		if ( FClassnameIs(pTest, "gib") )
			continue;
