...
public:
	...
	virtual bool	UseServerBulletEffects() { return true; } // i don't have a gun, so my bullets must be handled by the server
	...
	virtual bool IsRegularMinionType() { return false; } // do I use minion targeting rules, obey orders, apply minion drain?
	...
	virtual void	Kill( void ); // how to kill this remotely
	void	SetFaction(int i) { m_iFaction = i; }
	int		GetFaction( void ) { return m_iFaction; }
protected:
...
protected:
	...
	bool m_bOutOfAmmo;
	float m_flNextAmmoRegenTime, m_flNextHealthRegenTime, m_flAllowedToSettleTime;
