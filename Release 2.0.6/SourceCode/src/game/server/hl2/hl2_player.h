...
public:
	...
	float	GetDeviceDrainRate( CHL2_Player *pPlayer ) const;
};
...
public:
	...
	virtual float GetMaxArmor() { return m_flMaxBattery; };
	void SetMaxArmor(float f) { m_flMaxBattery = f; }
	void SetStartArmor(float f) { m_flStartBattery = f; }
	virtual float GetStartArmor() { return m_flStartBattery; }

	virtual float GetMaxAuxPower() { return 100; }
	...
	virtual void		GetAutoaimVector( autoaim_params_t &params, bool forceAutoaim = false );
...
private:
	...
	float m_flMaxBattery;
	float m_flStartBattery;
};
