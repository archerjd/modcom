...
CHL2_Player::CHL2_Player()
{
	...
	m_flMaxBattery = 100; // Set max battery initial.
}
...
#define SUITPOWER_CHARGE_RATE	mc_player_base_aux_recharge.GetFloat()

CSuitPowerDevice SuitDeviceSprint( bits_SUIT_DEVICE_SPRINT, 3.333333f );			// 100 units in 30 seconds
...
float CHL2_Player::GetSprintSpeed()
{
	return HL2_SPRINT_SPEED;
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
extern McConVar mc_player_base_sprint_drain;
void CHL2_Player::SuitPower_Update( void )
{
	...
	else if( m_HL2Local.m_bitsActiveDevices )
	{
		...
		if ( !sv_stickysprint.GetBool() )
		{
			if( SuitPower_IsDeviceActive(SuitDeviceSprint) )
			{
				if( !fabs(GetAbsVelocity().x) && !fabs(GetAbsVelocity().y) )
				{
					...
					flPowerLoad -= SuitDeviceSprint.GetDeviceDrainRate(this);
				}
			}
		}
		...
		if( SuitPower_IsDeviceActive(SuitDeviceFlashlight) )
		{
			...
			flPowerLoad -= ( SuitDeviceFlashlight.GetDeviceDrainRate(this) * (1.0f - factor) );
		}
...
void CHL2_Player::SuitPower_Charge( float flPower )
{
	...
	if( m_HL2Local.m_flSuitPower > GetMaxAuxPower() )
	{
		// Full charge, clamp.
		m_HL2Local.m_flSuitPower = GetMaxAuxPower();
	}
}

float CSuitPowerDevice::GetDeviceDrainRate(CHL2_Player *pPlr) const
{	
	//CHL2MP_Player *pPlayer = ToHL2MPPlayer(pPlr);
	/*if( g_pGameRules->GetSkillLevel() == SKILL_EASY && hl2_episodic.GetBool() && ! )
		return m_flDrainRate * 0.5f;
	else
		return m_flDrainRate; */
	if ( GetDeviceID() & bits_SUIT_DEVICE_SPRINT )
		return mc_player_base_sprint_drain.GetFloat();// + LEVEL(mod_runningman_drain, pPlayer->GetModuleLevel(GetModule(SPRINT_SPEED)));
	else
		return m_flDrainRate;
}
...
bool CHL2_Player::SuitPower_AddDevice( const CSuitPowerDevice &device )
{
	...
	m_flSuitPowerLoad += device.GetDeviceDrainRate(this);
	return true;
}
...
bool CHL2_Player::SuitPower_RemoveDevice( const CSuitPowerDevice &device )
{
	...
	SuitPower_Drain( device.GetDeviceDrainRate(this) * 0.1f );
...
bool CHL2_Player::ApplyBattery( float powerMultiplier )
{
	...
	const float MAX_NORMAL_BATTERY = GetMaxArmor();

	CHL2MP_Player *pHL2MP = ToHL2MPPlayer(this);
	if (( ArmorValue() < MAX_NORMAL_BATTERY ) || ( pHL2MP->GetAuxPower() < pHL2MP->GetMaxAuxPower() ) && IsSuitEquipped() )
	{
		int pct;
		char szcharge[64];
		...
		if ( pHL2MP->GetAuxPower() < pHL2MP->GetMaxAuxPower() )
			SuitPower_Charge( sk_battery.GetFloat() * powerMultiplier );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CHL2_Player::GetAutoaimVector( autoaim_params_t &params, bool forceAutoaim )
{
	BaseClass::GetAutoaimVector( params, forceAutoaim );
...
void CHL2_Player::Weapon_Equip( CBaseCombatWeapon *pWeapon )
{
	...
	if ( pWeapon->UsesClipsForAmmo1() && pWeapon->GetMaxClip1() > 1 )
	    pWeapon->SetClip(pWeapon->GetMaxClip1()); // now that we have an owner to take their ammo stat into account?
}
