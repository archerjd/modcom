...
extern McConVar sk_dmg_combineball;
...
// Output : CBaseEntity
//-----------------------------------------------------------------------------
CBaseEntity *CreateCombineBall( const Vector &origin, const Vector &velocity, float radius, float mass, float lifetime, CBaseEntity *pOwner, bool abilitylaunched, int bounces, int damage )
{
	...
	pBall->SetModuleLaunched( abilitylaunched );
	...
	pBall->SetMaxBounces ( bounces );
	pBall->SetModuleDamage( damage );
...
bool CPropCombineBall::CreateVPhysics()
{
	...
	if ( WasModuleLaunched() )
	{// no idea why, but this works. lets LEAVE IT :@:@:@
...
void CPropCombineBall::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
	...
	if ( reason != PUNTED_BY_CANNON )
	{
		...
		// Make it not explode
//		ClearLifetime( );		// DRYDEN: Cleared this to see if regular lifetime will work.
...
void CPropCombineBall::SetPlayerLaunched( CBasePlayer *pOwner )
{
	...
	if( VPhysicsGetObject() && !WasModuleLaunched() )
	{
...
void CPropCombineBall::OnHitEntity( CBaseEntity *pHitEntity, float flSpeed, int index, gamevcollisionevent_t *pEvent )
{
	...
	float damage = 0;
	if ( WasModuleLaunched() )
		damage = GetModuleDamage();
	else
		damage = sk_dmg_combineball.GetFloat();	

	CTakeDamageInfo info( this, GetOwnerEntity(), GetAbsVelocity(), GetAbsOrigin(), damage, DMG_DISSOLVE );
	if ( WasModuleLaunched() )
		info.SetModule(ENERGYBALL);
		
//	CTakeDamageInfo info( this, GetOwnerEntity(), GetAbsVelocity(), GetAbsOrigin(), sk_dmg_combineball.GetFloat(), DMG_DISSOLVE );

	bool bIsDissolving = (pHitEntity->GetFlags() & FL_DISSOLVING) != 0;
	bool bShouldHit = pHitEntity->PassesDamageFilter( info );

	//One more check - don't hit friendlies!
	CBaseCombatCharacter *pBCC = pHitEntity->MyCombatCharacterPointer();

	if ( bShouldHit && pBCC )
	{
		bShouldHit = !HL2MPRules()->IsFriendly(GetOwnerEntity(),pBCC);
	}
	...
	if ( !bIsDissolving && bShouldHit == true )
	{
		...
		{
//			if( WasModuleLaunched() || WasFiredByNPC() || m_nMaxBounces == -1 )
			{
...
void CPropCombineBall::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent )
{
	...
	if ( !WasModuleLaunched() && m_nMaxBounces == -1 )
	{
	...
	//Msg(UTIL_VarArgs("Bounced %i/%i times\n", m_nBounceCount, m_nMaxBounces));
