...
#ifdef USE_OMNIBOT
int CNPC_Manhack::GetOmnibotClass() const {
	return Omnibot::MC_CLASSEX_MANHACK;
}
#endif
...
void CNPC_Manhack::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( LikesMaster() && GetMasterPlayer() )
		ToHL2MPPlayer(GetMasterPlayer())->GetLimitedQuantities()->Remove(LQ_MANHACK);
...
void CNPC_Manhack::Slice( CBaseEntity *pHitEntity, float flInterval, trace_t &tr )
{
	...
	float flDamage = GetStatValue("dmg_slash") * flInterval;
	...
	if ( IsHeldByPhyscannon() )
	{
		// Deal high damage / sec when held
		flDamage = GetStatValue("dmg_held") * flInterval;

		// take significant damage when being used as a weapon
		CTakeDamageInfo selfInfo( HasPhysicsAttacker( MANHACK_SMASH_TIME ), this, flDamage * 0.5f, DMG_SLASH );
		TakeDamage( selfInfo );
	}
	...
	if ( LikesMaster() && GetMasterPlayer() == pHitEntity )
		flDamage = 0;

	CTakeDamageInfo info( this, MyAttacker(), flDamage, DMG_SLASH );
...
void CNPC_Manhack::StartEye( void )
{
	...
	if ( m_pEyeGlow == NULL )
	{
		...
		if ( LikesMaster() )
		{
			m_pEyeGlow->SetTransparency( kRenderTransAdd, 0, 128, 255, 128, kRenderFxNoDissipation );
			m_pEyeGlow->SetColor( 0, 128, 255 );
		}
		else if ( HasEffectColor() ) // if this NPC has an effect override color
		{
			Color c = GetEffectColor();
			m_pEyeGlow->SetTransparency( kRenderTransAdd, c.r(), c.g(), c.b(), c.a()/2, kRenderFxNoDissipation );
			m_pEyeGlow->SetColor( c.r(), c.g(), c.b() );
		}
		else
		{
	...
	if ( m_pLightGlow == NULL )
	{
		...
		if ( LikesMaster() )
		{
			m_pLightGlow->SetTransparency( kRenderTransAdd, 0, 128, 255, 128, kRenderFxNoDissipation );
			m_pLightGlow->SetColor( 0, 128, 255 );
		}
		else if ( HasEffectColor() ) // if this NPC has an effect override color
		{
			Color c = GetEffectColor();
			m_pLightGlow->SetTransparency( kRenderTransAdd, c.r(), c.g(), c.b(), c.a()/2, kRenderFxNoDissipation );
			m_pLightGlow->SetColor( c.r(), c.g(), c.b() );
		}
		else
		{
...
float CNPC_Manhack::ManhackMaxSpeed( void )
{
	...
	{
		// Slower in water!
		return MANHACK_MAX_SPEED * 0.1 * GetMaxEnginePower();
	}

	if ( HasPhysicsAttacker( MANHACK_SMASH_TIME ) )
	{
		return MANHACK_NPC_BURST_SPEED * GetMaxEnginePower();
	}

	return MANHACK_MAX_SPEED * GetMaxEnginePower();
}
...
{
	return GetStatValue("engine_power") / 10.0f;
}
