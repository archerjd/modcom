...
#ifdef USE_OMNIBOT
int CNPC_BaseZombie::GetOmnibotClass() const {
	return Omnibot::MC_CLASSEX_ZOMBIE_FAST;
}
#endif
...
bool CNPC_BaseZombie::ShouldBecomeTorso( const CTakeDamageInfo &info, float flDamageThreshold )
{
	...
	// Break in half IF:
	// 
	// Take 65% or more of max health in DMG_BLAST
	if( (info.GetDamageType() & DMG_BLAST) && flDamageThreshold >= 0.65 )
	{
		return true;
	}
...
HeadcrabRelease_t CNPC_BaseZombie::ShouldReleaseHeadcrab( const CTakeDamageInfo &info, float flDamageThreshold )
{
	...
	if ( m_iHealth <= 0 )
	{
		// DMG_DIRECT|DMG_GENERIC is precisely the damage type dealt by the KillAllMonsters function, and nothing else
		if ( (info.GetDamageType() & DMG_REMOVENORAGDOLL) || (info.GetDamageType() == (DMG_DIRECT|DMG_GENERIC)) )
			return RELEASE_NO;
		...
		if ( info.GetDamageType() & DMG_BULLET )
		{
			...
			else if ( GetStatValue("dropnocrab") != 1 ) // stat override to have a crab show on this zombie, but never actually release one
			{
	...
	else
	{
		...
		if( ShouldBecomeTorso( info, flDamageThreshold ) )
		{
			...
			if ( CanBecomeLiveTorso() )
			{
				...
				{
					ApplyBuff(DEBUFF_BURN,NULL,random->RandomInt(5, 10) );
				}
				...
				if ( hl2_episodic.GetBool() )
				{
					...
					{
						// Catch on fire.
						ApplyBuff(DEBUFF_BURN,NULL,random->RandomInt(1, 5) );
					}
				}
...
//---------------------------------------------------------
// hijacked this function. If pOther IsAlive, will copy all afflictiosn to it instead
//---------------------------------------------------------
void CNPC_BaseZombie::CopyRenderColorTo( CBaseEntity *pOther )
{
	...
	if ( pBCC != NULL )
	{
		for ( int i=0; i<NUM_BUFFS; i++ )
			if ( IsBuffActive(GetBuff(i)) )
				pBCC->ApplyBuff(i,GetInflictor(i),GetBuffLevel(i));
	}
}
...
CBaseEntity *CNPC_BaseZombie::ClawAttack( float flDist, int iDamage, QAngle &qaViewPunch, Vector &vecVelocityPunch, int BloodOrigin  )
{
	...
	{ 
		...
		pHurt = GetEnemy();
		CTakeDamageInfo info( this, MyAttacker(), vec3_origin, GetAbsOrigin(), iDamage, DMG_SLASH );
	...
	if ( !pHurt && m_hPhysicsEnt != NULL && IsCurSchedule(SCHED_ZOMBIE_ATTACKITEM) )
	{
		...
		CTakeDamageInfo info( this, MyAttacker(), vForce, GetAbsOrigin(), iDamage, DMG_SLASH );
		pHurt->TakeDamage( info );
...
void CNPC_BaseZombie::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_RIGHT )
	{
		...
		ClawAttack( GetClawAttackRange(), GetStatValue("dmg_one"), QAngle( -15, -20, -10 ), right + forward, ZOMBIE_BLOOD_RIGHT_HAND );
		return;
	}
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_LEFT )
	{
		...
		ClawAttack( GetClawAttackRange(), GetStatValue("dmg_one"), QAngle( -15, 20, -10 ), right + forward, ZOMBIE_BLOOD_LEFT_HAND );
		return;
	}
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_BOTH )
	{
		...
		forward = forward * 200;
		ClawAttack( GetClawAttackRange(), GetStatValue("dmg_both"), qaPunch, forward, ZOMBIE_BLOOD_BOTH_HANDS );
		return;
	}
...
void CNPC_BaseZombie::Spawn( void )
{
	...
	if ( HasStat("crabless") && GetStatValue("crabless") == 1 )
		m_fIsHeadless = true;
...
{
	CNPCTypeInfo *pStats = GetNPCInfo(GetHeadcrabTypeName());
	if ( pStats )
		UTIL_PrecacheOther( pStats->m_szClassname );
...
{
	if ( HasCondition( COND_ZOMBIE_RELEASECRAB ) && !(LikesMaster() && GetMasterPlayer()) )
	{
...
int CNPC_BaseZombie::GetSwatActivity( void )
{
	...
	else
	{
		...
		// poison zombie doesn't have this anim??
...
void CNPC_BaseZombie::BecomeTorso( const Vector &vecTorsoForce, const Vector &vecLegsForce )
{
	...
	{
		Extinguish();
		ApplyBuff(DEBUFF_BURN,NULL,random->RandomInt(5, 15) );
	}
...
void CNPC_BaseZombie::ReleaseHeadcrab( const Vector &vecOrigin, const Vector &vecVelocity, bool fRemoveHead, bool fRagdollBody, bool fRagdollCrab )
{
	...
	if ( GetStatValue("dropnocrab") == 1 )
		fRagdollCrab = true; // force the crab to be a ragdoll (not alive!)
	...
	{
		CNPCTypeInfo* pStats = GetNPCInfo(GetHeadcrabTypeName());
		pCrab = (CAI_BaseNPC*)CreateNoSpawn( pStats->m_szClassname, vecSpot, GetAbsAngles() );

		if ( !pCrab )
		{
			Warning( "**%s: Can't make %s!\n", GetClassname(), GetHeadcrabTypeName() );
			return;
		}
		...
		// Set level
		pCrab->SetStats( pStats, GetLevel()); // calls DispatchSpawn

		// pass on ownership
		pCrab->SetMasterPlayer( GetMasterPlayer() );
		pCrab->SetLikesMaster( LikesMaster() );
		...
		{
			pCrab->ApplyBuff(DEBUFF_BURN,NULL,random->RandomInt(5, 15) );
		}
