...
ConVar	sk_fastzombie_leapdamage( "sk_fastzombie_leapdamage","0");
ConVar	sk_fastzombie_clawdamage( "sk_fastzombie_clawdamage","0");
...
void CFastZombie::Spawn( void )
{
	...
	m_fIsHeadless = false;
	m_fIsTorso = FClassnameIs( this, "npc_fastzombie_torso" );
...
//-----------------------------------------------------------------------------
const char *CFastZombie::GetHeadcrabTypeName( void )
{
...
void CFastZombie::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_RIGHT )
	{
		...
		ClawAttack( GetClawAttackRange(), GetStatValue("dmg_claw"), QAngle( -3, -5, -3 ), right, ZOMBIE_BLOOD_RIGHT_HAND );
		return;
	}
	...
	if ( pEvent->event == AE_ZOMBIE_ATTACK_LEFT )
	{
		...
		AngleVectors( GetLocalAngles(), NULL, &right, NULL );
		right = right * 50;
		ClawAttack( GetClawAttackRange(), GetStatValue("dmg_claw"), QAngle( -3, 5, -3 ), right, ZOMBIE_BLOOD_LEFT_HAND );
		return;
	}
...
void CFastZombie::LeapAttackTouch( CBaseEntity *pOther )
{
	...
	ClawAttack( GetClawAttackRange(), GetStatValue("dmg_leap"), qaPunch, forward * 500, ZOMBIE_BLOOD_BOTH_HANDS );
