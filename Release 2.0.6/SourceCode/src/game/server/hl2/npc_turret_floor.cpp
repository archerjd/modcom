...
extern McConVar mod_turret_clip, mod_turret_ammo_regen_interval, mod_turret_health_regen_interval, mod_turret_min_ammo;
...
IMPLEMENT_SERVERCLASS_ST(CNPC_FloorTurret, DT_NPC_FloorTurret)
	SendPropInt( SENDINFO( m_iBullets ), 7, SPROP_UNSIGNED ),
END_SEND_TABLE()
...
#ifdef USE_OMNIBOT
int CNPC_FloorTurret::GetOmnibotClass() const {
	return Omnibot::MC_CLASSEX_TURRET;
}
#endif
...
void CNPC_FloorTurret::Precache( void )
{
	...
	PropBreakablePrecacheAll( MAKE_STRING(FLOOR_TURRET_MODEL) );
	//PropBreakablePrecacheAll( MAKE_STRING(FLOOR_TURRET_MODEL_CITIZEN) );
...
{ 
	...
	m_takedamage	= DAMAGE_YES;//DAMAGE_EVENTS_ONLY;
	m_iHealth		= m_iMaxHealth	= GetStatValue("health");
	...
	m_iBullets = mod_turret_clip.GetInt();
	m_bOutOfAmmo = false;
	m_flNextAmmoRegenTime = gpGlobals->curtime + mod_turret_ammo_regen_interval.GetFloat();
	m_flNextHealthRegenTime = gpGlobals->curtime + mod_turret_health_regen_interval.GetFloat();
	m_flAllowedToSettleTime = gpGlobals->curtime + 0.05f;

	SetState(NPC_STATE_IDLE);
	RemoveBuff(BUFF_MINION);
}
...
bool CNPC_FloorTurret::ForcePhysgunOpen( CBasePlayer *pPlayer )
{
	return LikesMaster() && pPlayer == GetMasterPlayer();
}
...
{
	if ( LikesMaster() && pPhysGunUser == GetMasterPlayer() )
		VPhysicsGetObject()->EnableMotion(true);
	else
		return;
...
{
	m_flAllowedToSettleTime = gpGlobals->curtime + 0.2f; // don't freeze in place in mid air
	m_hPhysicsAttacker = pPhysGunUser;
	m_flLastPhysicsInfluenceTime = gpGlobals->curtime;
	...
	{// snap to vertical & immobile
		QAngle a = GetAbsAngles();
		a[PITCH] = a[ROLL] = 0;
		if ( VPhysicsGetObject() )
		{
			VPhysicsGetObject()->SetPosition(GetAbsOrigin(),a,false);
			Vector v = vec3_origin;
			VPhysicsGetObject()->SetVelocity(&v, &v);
		}
	}
...
bool CNPC_FloorTurret::HasPreferredCarryAnglesForPlayer( CBasePlayer *pPlayer )
{
	...
	if ( LikesMaster() && pPlayer == GetMasterPlayer() )
		return true;
...
void CNPC_FloorTurret::SuppressThink( void )
{
	...
	if ( !m_bOutOfAmmo && m_flShotTime < gpGlobals->curtime && m_vecEnemyLKP != vec3_invalid )
	{
		Vector vecMuzzle, vecMuzzleDir;
		UpdateMuzzleMatrix();
		MatrixGetColumn( m_muzzleToWorld, 0, vecMuzzleDir );
...
void CNPC_FloorTurret::ActiveThink( void )
{
	...
	if ( m_flShotTime < gpGlobals->curtime )
	{
		...
		if ( bCanShoot ) // 10 degree slop XY
		{
			...
			{
				bool enemyIsSpawning = GetEnemy()->MyCombatCharacterPointer() != NULL && GetEnemy()->MyCombatCharacterPointer()->IsBuffActive(BUFF_SPAWN);
				if ( dot3d >= minCos3d && !IsHeldByPhyscannon() && !enemyIsSpawning ) 
				{
					ResetActivity();
					SetActivity( (Activity) ACT_FLOOR_TURRET_FIRE );
...
void CNPC_FloorTurret::Shoot( const Vector &vecSrc, const Vector &vecDirToEnemy, bool bStrict )
{
	...
	if ( !bStrict && GetEnemy() != NULL )
	{
		...
		info.m_iShots = 1;
		info.m_pAttacker = GetMasterPlayer();
		info.m_vecSpread = VECTOR_CONE_PRECALCULATED;
		info.m_flDistance = MAX_COORD_RANGE;
		info.m_iAmmoType = m_iAmmoType;
		info.m_iDamage = info.m_iPlayerDamage = GetStatValue("dmg_shoot");
	}
	else
	{
		info.m_vecSrc = vecSrc;
	...
	m_flNextAmmoRegenTime = gpGlobals->curtime + 0.5f;
}
...
void CNPC_FloorTurret::TippedThink( void )
{
	...
	if ( gpGlobals->curtime < m_flThrashTime )
	{
		if ( m_flShotTime < gpGlobals->curtime )
		{
			if( m_iBullets <= 0 || (m_spawnflags & SF_FLOOR_TURRET_OUT_OF_AMMO) )
			{
				DryFire();
			}
...
//-----------------------------------------------------------------------------
extern McConVar npc_turret_linear_limit, npc_turret_angular_limit;
ConVar mc_dev_debug_turret_balance("mc_dev_debug_turret_balance", "0", FCVAR_NOTIFY | FCVAR_CHEAT, "Debug the balanace of turrets", true, 0, true, 1);
bool CNPC_FloorTurret::PreThink( turretState_e state )
{
	// if movement is enabled, i'm not being held, wait til i'm not moving ... and disable movement
	if ( VPhysicsGetObject()->IsMotionEnabled() && !IsHeldByPhyscannon() && m_flAllowedToSettleTime <= gpGlobals->curtime )
	{
		Vector linear; AngularImpulse angular;
		VPhysicsGetObject()->GetVelocity(&linear, &angular);

		//QAngle a = GetAbsAngles();
		//if ( linear.LengthSqr() < 16 /*&& GetGroundEntity() != NULL*/ && a[PITCH] < 30 && a[PITCH] > -30 && a[ROLL] < 30 && a[ROLL] > -30 )
		//if ( linear == vec3_origin && angular == vec3_origin )
		if ( mc_dev_debug_turret_balance.GetInt() == 1 )
			UTIL_ClientPrintAll(HUD_PRINTCONSOLE,UTIL_VarArgs("Linear: %.2f, Angular: %.2f\n", linear.LengthSqr(), angular.LengthSqr()));
		if ( linear.LengthSqr() < npc_turret_linear_limit.GetFloat() && angular.LengthSqr() < npc_turret_angular_limit.GetFloat() )
		{
			if ( mc_dev_debug_turret_balance.GetInt() == 1 )
				UTIL_ClientPrintAll(HUD_PRINTCONSOLE,"Locking!\n");
			VPhysicsGetObject()->EnableMotion(false);
		}
	}

	/*if ( mc_dev_turret_balancing.GetInt() )
	{
		QAngle a = GetAbsAngles();
		if ( a[PITCH] < 30 && a[PITCH] > -30 && a[ROLL] < 30 && a[ROLL] > -30 )
		{
			a[PITCH] = a[ROLL] = 0;
			Vector vel, pos;
			VPhysicsGetObject()->GetVelocity(&vel,NULL);
			VPhysicsGetObject()->SetVelocityInstantaneous(&vel,&vec3_origin);
			VPhysicsGetObject()->GetPosition(&pos,NULL);
			VPhysicsGetObject()->SetPosition(pos,a,false);
		}
	}*/

	ManageBuffs();
	
	if ( !m_bSelfDestructing && LikesMaster() && GetMasterPlayer() == NULL )
	{
		SelfDestruct();
		return true;
	}
	...
	if ( IsBuffActive(DEBUFF_FREEZE) )
	{
		SetNextThink( gpGlobals->curtime + 0.1f );
		return true; // don't shoot when frozen
	}

	if (HasCondition(COND_NPC_FREEZE))
		return false; // if we just used this for disabling freeze, for some reason it stops them fading in in the first place
	...
	if ( m_flNextHealthRegenTime <= gpGlobals->curtime )
	{
		TakeHealth(1,DMG_GENERIC);
		m_flNextHealthRegenTime = gpGlobals->curtime + mod_turret_health_regen_interval.GetFloat();
	}

	if ( m_iBullets < mod_turret_clip.GetInt() && m_flNextAmmoRegenTime <= gpGlobals->curtime )
	{
		m_iBullets ++;
		m_flNextAmmoRegenTime = gpGlobals->curtime + mod_turret_ammo_regen_interval.GetFloat();
		if ( m_bOutOfAmmo && m_iBullets >= mod_turret_min_ammo.GetInt() )
			m_bOutOfAmmo = false;
	}
...
void CNPC_FloorTurret::Kill()
{
	...
	{
		if ( GetMasterPlayer() )
			ToHL2MPPlayer(GetMasterPlayer())->GetLimitedQuantities()->Remove(LQ_TURRET);
		SelfDestruct();
	}
}
...
*/
	if ( m_iHealth > 0 )
		return 0; // don't get pushed unless we've just died
...
{
	if ( GetHealth() <= 0 )
		return 0; // once we're dead, don't take damage any more
	...
	{
		if ( GetGroundEntity() )
			VPhysicsGetObject()->EnableMotion(false); // stop me moving ... if i'm held in the air at the time, that might be weird...

		m_iHealth = 0;
		//VPhysicsGetObject()->EnableMotion(true); // if enabled, it falls through the floor. if not, it doesn't move at all.
...
#define SELF_DESTRUCT_DURATION			2.5f
...
//#endif // HL2_EPISODIC
	...
	// K-boom
	RadiusDamage( CTakeDamageInfo( GetMasterPlayer(), this, 15.0f, DMG_BLAST ), vecOrigin, (10*12), CLASS_NONE, this );

	// the sound comes out REALLY QUIETLY...
...
int GetFloorTurretFaction(CBaseEntity * ent)
{
	CNPC_FloorTurret * turret = dynamic_cast<CNPC_FloorTurret*>(ent);
	if (turret)
	{
		return turret->GetFaction();
	}
	return 0;
}