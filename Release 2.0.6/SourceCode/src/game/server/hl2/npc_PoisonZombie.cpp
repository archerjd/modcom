...
//-----------------------------------------------------------------------------
// The model of the headcrab that jumps off of this kind of zombie.
//-----------------------------------------------------------------------------
...
//#endif // HL2_EPISODIC
	...
	else
	{
		...
		if ( !nCrabs )
			nCrabs = random->RandomInt(0,MAX_CRABS); // don't ALWAYS start with loads of crabs ready to throw
		m_nCrabCount = 0;
...
//-----------------------------------------------------------------------------
const char *CNPC_PoisonZombie::GetHeadcrabTypeName( void )
{
...
void CNPC_PoisonZombie::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ZOMBIE_POISON_THROW_CRAB )
	{
		...
		CBlackHeadcrab *pCrab = (CBlackHeadcrab *)CreateNoSpawn( pStats->m_szClassname, EyePosition(), vec3_angle, this );
	...
	if ( pEvent->event == AE_ZOMBIE_POISON_SPIT )
	{
		...
		forward = forward * 200;
		ClawAttack( GetClawAttackRange(), GetStatValue("dmg_one"), qaPunch, forward, ZOMBIE_BLOOD_BITE );
		return;
	}
...
void CNPC_PoisonZombie::EvacuateNest( bool bExplosion, float flDamage, CBaseEntity *pAttacker )
{
	...
	for( int i = 0; i < MAX_CRABS ; i++ )
	{
		if( m_bCrabs[i] )
		{
			...
			CNPCTypeInfo* pStats = GetNPCInfo(GetHeadcrabTypeName());
			CBlackHeadcrab *pCrab = (CBlackHeadcrab *)CreateNoSpawn( pStats->m_szClassname, vecPosition, vecAngles, this );
			pCrab->SetMasterPlayer( GetMasterPlayer() );
			pCrab->SetLikesMaster( LikesMaster() );
			pCrab->SetStats(pStats, GetLevel()); // calls DispatchSpawn
