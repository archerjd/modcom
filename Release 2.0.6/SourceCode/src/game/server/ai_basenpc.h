...
{
	GENERAL_INTERRUPTMODULE,
	DAMAGEORDEATH_INTERRUPTMODULE,
	DEATH_INTERRUPTMODULE
};
...
public:
	...
	virtual void		EmitSound( const char *soundname, float soundtime = 0.0f, float *duration = NULL );
	virtual void		EmitSound( const char *soundname, HSOUNDSCRIPTHANDLE& handle, float soundtime = 0.0f, float *duration = NULL );
	...
	void EmitSound( IRecipientFilter& filter, int iEntIndex, const char *soundname, HSOUNDSCRIPTHANDLE& handle, const Vector *pOrigin = NULL, float soundtime = 0.0f, float *duration = NULL );
	void EmitSound( IRecipientFilter& filter, int iEntIndex, const EmitSound_t & params );
	void EmitSound( IRecipientFilter& filter, int iEntIndex, const EmitSound_t & params, HSOUNDSCRIPTHANDLE& handle );
...
public:
	...
	virtual bool		IsRegularMinionType() { return true; } // do I use minion targeting rules, obey orders, apply minion drain?
...
public:
	...
	float				GetStatValue(const char *name); // for pvm
	bool				HasStat(const char *name) { return m_pType != NULL && m_pType->GetVarIndex(name) != -1; }
	
	float				GetExperienceScale() { return m_pType == NULL ? 1 : m_pType->m_flExperienceScale; } // when i'm killed, scale experience awarded by this value

protected:
	virtual void		ClearSenseConditions( void );
	CNPCTypeInfo *		m_pType;
...
public:
	...
	void				FlagForLagCompensation(bool on) { m_flLagFlag = on; }
	bool				IsLagFlagged() { return m_flLagFlag; }
...
public:
	...
	// ownership of NPC in multiplayer
	void SetMasterPlayer( CBasePlayer *pPlayer ) { m_pMyPlayer = pPlayer; }
	CBasePlayer *GetMasterPlayer() { return m_pMyPlayer; }
	CBaseCombatCharacter *MyAttacker(); // who to attribute my attacks to
	void SetLikesMaster(bool b);
	bool LikesMaster() { return m_bLikesMaster; }
	void SetStats(CNPCTypeInfo *info, int level, bool bShouldCallSpawn=true);
	const char *GetTypeName() { return m_pType == NULL ? "Unknown" : m_pType->m_szTypeName; }
	const char *GetName( void ) { return GetTypeName(); }
	
	// aux power
	virtual float GetAuxPower() { return m_flAuxPower; }
	virtual void DrainAuxPower(float f) { m_flAuxPower = max(0, m_flAuxPower - f);  }
	virtual void AddAuxPower(float f) { m_flAuxPower = min(GetMaxAuxPower(), m_flAuxPower + f); }
	virtual void SetAuxPower(float f) { m_flAuxPower = max(0,min(GetMaxAuxPower(), f));  }
	virtual float GetMaxAuxPower() { return m_flMaxAuxPower; }
	...
	virtual bool IsMechanical() { return false; }

	// different types of monster use "effect color" differently ... some don't use it at all.
	bool	HasEffectColor() { return m_bHasEffectColorOverride; }
	Color	GetEffectColor() { return m_effectColor; }
	void	SetEffectColor(Color c) { m_effectColor = c; m_bHasEffectColorOverride = true; }
	void	ClearEffectColor() { m_bHasEffectColorOverride = false; }

private:
	bool m_bIsBoss;
	int m_abilityDispositions[NUM_MODULES]; // how much i favor a particular ability, for the purposes of selecting which to use
	CNetworkVar( int, m_iTypeName );
	CNetworkVar( int, m_iSkinOverride );
	CNetworkVar( bool, m_bLikesMaster ); // i tried using a CONDITION instead of this, but it was confusing
	CNetworkHandle( CBasePlayer, m_pMyPlayer );

	float m_flMaxAuxPower, m_flAuxPower, m_flAuxRecharge;
	float m_flLastActionTime;
	
	bool m_bHasEffectColorOverride;
	Color m_effectColor;
};
