...
BEGIN_DATADESC( CBaseCombatCharacter )
	...
	DEFINE_AUTO_ARRAY( m_bBuffs, FIELD_BOOLEAN ),
...
Relationship_t	CBaseCombatCharacter::m_like;
Relationship_t	CBaseCombatCharacter::m_hate;
Relationship_t	CBaseCombatCharacter::m_loathe;
Relationship_t	CBaseCombatCharacter::m_ignore;
...
IMPLEMENT_SERVERCLASS_ST(CBaseCombatCharacter, DT_BaseCombatCharacter)
	...
	SendPropInt( SENDINFO( m_iParticleEffect ), 4, SPROP_UNSIGNED ),
...
CBaseCombatCharacter::CBaseCombatCharacter( void )
{
	...
	m_iBaseMaxHealth = 0;
	m_iParticleEffect = 0;
	m_iControllingModule = NO_MODULE;
...
CBaseCombatCharacter::~CBaseCombatCharacter( void )
{
	...
	m_storedDamageCommands.Purge();
	if ( GetNumScoreTokens() > 0 )
		RemoveScoreToken(GetNumScoreTokens());
}
...
void CBaseCombatCharacter::Precache()
{
	...
	PrecacheScriptSound("Module.ToggleFail");
...
{
	// hack as the bullseye-finding stuff doesn't seem to work for MAGDs
...
bool CTraceFilterMelee::ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask )
{
	...
	if ( pEntity )
	{
		if ( !pEntity->ShouldCollide( m_collisionGroup, contentsMask, 0 ) )
			return false;
		...
		*/
		...
		Vector	attackDir = pEntity->WorldSpaceCenter() - m_dmgInfo->GetInflictor()->WorldSpaceCenter();
		VectorNormalize( attackDir );

		CTakeDamageInfo info = (*m_dmgInfo);				
		CalculateMeleeDamageForce( &info, attackDir, info.GetInflictor()->WorldSpaceCenter(), m_flForceScale );

		CBaseCombatCharacter *pBCC = info.GetInflictor()->MyCombatCharacterPointer();
		...
		if ( pBCC && pVictimBCC )
		{
			...
			if ( m_bDamageAnyNPC || pBCC->IRelationType( pEntity ) == D_HT )
			{
				...
				CSoundEnt::InsertSound( SOUND_COMBAT, info.GetDamagePosition(), 200, 0.2f, info.GetInflictor() );
...
CBaseEntity *CBaseCombatCharacter::CheckTraceHullAttack( const Vector &vStart, const Vector &vEnd, const Vector &mins, const Vector &maxs, int iDamage, int iDmgType, float flForceScale, bool bDamageAnyNPC )
{
	...
	if ( IsNPC() )
		pAttacker = ((CAI_BaseNPC*)this)->MyAttacker();
	else
		pAttacker = this;

	CTakeDamageInfo	dmgInfo( this, pAttacker, iDamage, DMG_SLASH );
	...
	if ( IRelationType( pEntity ) == D_HT )
	{
		if ( iDamage > 0 )
		{
			CTakeDamageInfo info( this,  MyAttacker(), iDamage, iDmgType );
...
{
	int numLost = IsPlayer() ? 1 : GetNumScoreTokens();
	if ( numLost > 0 && HL2MPRules() && HL2MPRules()->ShouldUseScoreTokens() && RemoveScoreToken(numLost) )
	{// drop a score token, if there aren't too many in the game already
		int numToDrop = max(0, min(HL2MPRules()->GetTargetScoreTokenLimit() - HL2MPRules()->GetTotalNumScoreTokens(), numLost));
		if ( numToDrop > 0 ) // we've just removed 1, so there must be LESS than the target in order to drop one
		{
			if ( info.GetAttacker() && info.GetAttacker()->IsNPC() )
				info.GetAttacker()->MyCombatCharacterPointer()->AddScoreToken(numToDrop);
			else
				for ( int i=0; i<numToDrop; i++ )
					DropItem( "item_score_token", WorldSpaceCenter()+RandomVector(-4,4), RandomAngle(0,360) );
		}
	}
...
//=========================================================
CBaseEntity *CBaseCombatCharacter::DropItem ( char *pszItemName, Vector vecPos, QAngle vecAng )
{
	if ( !pszItemName )
	{
...
void CBaseCombatCharacter::Weapon_Equip( CBaseCombatWeapon *pWeapon )
{
	...
	pWeapon->Equip( this ); // do this earlier (here) so that the ammo stat considerations work, perhaps?
	...
	else if (pWeapon->GetDefaultClip1() > 0 )
	{
		pWeapon->m_iClip1 = min(pWeapon->GetDefaultClip1(), pWeapon->GetMaxClip1());

		// if ammo count is now greater than max carry (on account of the clip in the picked-up gun), then reduce carried ammo to equal max minus clip amount. Otherwise, do the default add.
		if ( GetAmmoCount(pWeapon->m_iPrimaryAmmoType, true) > GetAmmoDef()->MaxCarry(pWeapon->m_iPrimaryAmmoType) )
			m_iAmmo.Set( pWeapon->m_iPrimaryAmmoType, GetAmmoDef()->MaxCarry(pWeapon->m_iPrimaryAmmoType) - pWeapon->m_iClip1);
...
void CBaseCombatCharacter::RemoveAllWeapons()
{
	...
	for (int i = 0; i < MAX_WEAPONS; i++)
	{
		...
		{
#ifdef USE_OMNIBOT
			Omnibot::Notify_RemoveWeapon(ToBasePlayer(this), m_hMyWeapons[i]->GetClassname());
#endif
...
int CBaseCombatCharacter::TakeHealth (float flHealth, int bitsDamageType)
{
	...
	if ( IsBuffActive(DEBUFF_ATTRITION) )
		return false;
...
int CBaseCombatCharacter::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageType() & DMG_CRIT )
	{// emit the hit sound so that the hit player and the attacking player can both hear it. And do some sparks.
		...
		{
			filter.AddRecipient(ToBasePlayer(info.GetAttacker()));
			send = true;
		}
		if ( send )
			EmitSound( filter, entindex(), "Crit.Hit");
...
ConVar mc_dev_show_damage("mc_dev_show_damage", "0", FCVAR_CHEAT | FCVAR_NOTIFY, "Shows all damage taken in the console", true, 0, true, 1);
int CBaseCombatCharacter::OnTakeDamage_Alive( const CTakeDamageInfo &inputInfo )
{
	...
	if ( m_takedamage != DAMAGE_EVENTS_ONLY )
	{
		...
		info.ScaleDamage( GetDamageReceivedScale() ); // scale all damage I take according to this
		...
		if ( info.GetAttacker() )
		{
			CBaseCombatCharacter *pVictim = ToBaseCombatCharacter( this );
			if ( pVictim->IsModuleActive(ARMORREGEN) )
			{
				pVictim->ApplyBuff(DEBUFF_SUPPRESS_ARMORREGEN, NULL, max(pVictim->GetModuleLevel(ARMORREGEN),1));
			}
		}

		if ( info.GetAttacker() && info.GetAttacker()->IsPlayer() )
		{
			CBaseCombatCharacter *pAttacker = ToBaseCombatCharacter(info.GetAttacker());
			pAttacker->ApplyBuff(DEBUFF_SUPPRESS_CLOAK, NULL, max(pAttacker->GetModuleLevel(CLOAK),1));
		}

		// show all damage messages if desired
		if ( mc_dev_show_damage.GetInt() == 1 )
			UTIL_ClientPrintAll(HUD_PRINTCONSOLE, UTIL_VarArgs("%s took %i damage from %s\n", IsPlayer() ? ToHL2MPPlayer(this)->GetPlayerName() : GetClassname(), (int)flIntegerDamage, info.GetInflictor() ? info.GetInflictor()->IsPlayer() ? ToHL2MPPlayer(info.GetInflictor())->GetPlayerName() : info.GetInflictor()->GetClassname() : "nothing" ));
	}
...
void CBaseCombatCharacter::AllocateDefaultRelationships( )
{
	if (!m_DefaultRelationship)
	{
		...
		// stick in our basic like & hate relationships for minions here
		m_hate.classType = CLASS_NONE;
		m_hate.disposition = D_HT;
		m_hate.priority = 50; // don't like you

		m_loathe.classType = CLASS_NONE;
		m_loathe.disposition = D_HT;
		m_loathe.priority = 99; // highest priority of hatred

		m_like.classType = CLASS_NONE;
		m_like.disposition = D_LI;
		m_like.priority = 1; // don't bust our ass saving them

		m_ignore.classType = CLASS_NONE;
		m_ignore.disposition = D_NU;
		m_ignore.priority = 0;
	}
}
...
Relationship_t *CBaseCombatCharacter::FindEntityRelationship( CBaseEntity *pTarget )
{
	...
	// That is, if i have a grudge or affection for this particular entity
	int i;
	...
	if (pTarget->Classify() != CLASS_NONE)
	{
		...
		// That is, if I've decided that I hate all antlions, or whatever
		for (i=0;i<m_Relationship.Count();i++) 
		{
	...
	// this is now handled in CheckTraceHullAttack instead - melee attacks wont hit crows!
//	if ( pTarget->Classify() == CLASS_EARTH_FAUNA && Classify() != CLASS_VORTIGAUNT )
//		return &m_ignore; // only vortigaunts pay attention to crows!

	// then check for any relationships due to me or my target having a master player
	if (pTarget->Classify() != CLASS_NONE)
	{
		if ( HL2MPRules() && HL2MPRules()->IsFriendly(this,pTarget) )
			return &m_like;
		else if ( pTarget->Classify() == CLASS_BULLSEYE )
			return &m_loathe; // attack MAGDs (which we can get to) rather than players (which we cant)
		else
			return &m_hate;
	}
...
int CBaseCombatCharacter::GiveAmmo( int iCount, int iAmmoIndex, bool bSuppressSound)
{
	...
	int iAdd = min( iCount, iMax - GetAmmoCount(iAmmoIndex, true) );
	if ( iAdd < 1 )
		return 0;
...
//----------------------------------------------------------------------------
bool CBaseCombatCharacter::IsBuffActive(Buff *a)
{
	return a != NULL && m_iBuffs[a->GetBuffIndex()] > 0;
}

bool CBaseCombatCharacter::IsBuffActive(int i)
{
	return m_iBuffs[i] > 0;
}

void CBaseCombatCharacter::RemoveAllBuffs()
{
	for ( int i=0; i<NUM_BUFFS; i++ )
		RemoveBuff(i);
}

void CBaseCombatCharacter::RemoveBuff(int i)
{
	Buff *a = GetBuff(i);
	if ( !a || !IsBuffActive(a) )
		return;

	a->StopEffect(this,m_hInflictors[i],m_iBuffs[i]);

	if ( a->GetStartSound() != NULL )
		StopSound(a->GetStartSound());
	
	if ( a->GetTickSound() != NULL )
		StopSound(a->GetTickSound());

	m_iBuffs.Set(i, 0);
	m_bBuffs.Set(i,false);
	m_flBuffEndTimes.Set(i, 0.0f);
	m_flBuffCooldownTimes[i] = 0.0f;
	m_hInflictors[i] = NULL;
}

extern bool g_bShowdamageScaleMessages;

// returns false if (eg) a more powerful version of the same affliction is active
bool CBaseCombatCharacter::ApplyBuff(int i, CBaseCombatCharacter *pInflictor, int level)
{
	Buff *a = GetBuff(i);
	if ( level < 1 )
	{
		Warning(UTIL_VarArgs("Applying level %i of affliction %s!\n", level, a->GetName()));
		return false; // can't apply level 0... possible partial fix for #245, doesn't explain why its trying level 0 in the first place
	}
	
	if ( level > MAX_BUFF_LEVEL ) // cap buffs at a reasonable maximum level
		level = MAX_BUFF_LEVEL;

	if ( m_iBuffs[i] > level )
		return false; // more powerful version of this affliction already active

	if ( m_iBuffs[i] > 0 ) // already have it, clear it off!
	{
		g_bShowdamageScaleMessages = false;
		RemoveBuff(i);
		g_bShowdamageScaleMessages = true;
	}

	m_iBuffs.Set(i, level);
	m_bBuffs.Set(i,level > 0);
	m_flBuffEndTimes.Set(i, gpGlobals->curtime + a->GetDuration(level));
	m_flBuffCooldownTimes[i] = gpGlobals->curtime + a->GetTickInterval(level);
	m_hInflictors[i] = pInflictor;

	if ( a->GetStartSound() != NULL )
		EmitSound( a->GetStartSound() );

	a->StartEffect(this,pInflictor,level);

	return true;
}

// called by PreThink functions of players and npcs
void CBaseCombatCharacter::ManageBuffs()
{
	for ( int i=0; i<NUM_BUFFS; i++ )
	{
		Buff *a = GetBuff(i);
		...
		if ( !a->IsPermenant() && GetBuffEndTime(i) <= gpGlobals->curtime )
		{// we survied this affliction, its going away now
			RemoveBuff(i);
			continue;
		}
		
		if ( a->Ticks() && GetBuffTickTime(i) <= gpGlobals->curtime )
		{// ow!
			a->DoEffect(this,GetInflictor(i),GetBuffLevel(i));
			if ( a->GetTickSound() != NULL )
				EmitSound( a->GetTickSound() );
			SetBuffTickTime(i,gpGlobals->curtime + a->GetTickInterval(GetBuffLevel(i)));
		}
	}

	// affliction-specific logic
	if ( IsBuffActive(DEBUFF_BURN) && GetWaterLevel() > 2 )
		RemoveBuff(DEBUFF_BURN);
}


#define FULLY_CLOAKED	2.0f	// a bit less than 1% visible
#define FULLY_VISIBLE	255.0f	// utterly opaque
#define CLOAK_RATE		200.0f	// per second (must cover range 2-255)
#define DECLOAK_RATE	500.0f	// per second (must cover range 255-2)

// called by PostThink functions of players and npcs
void CBaseCombatCharacter::ManageModules()
{
	...
	for ( int i=0; i<NUM_MODULES; i++ )
	{
		...
		if ( a->IsSuppressed(this) )
			continue;

		if ( m_bEnqueuedModules[i] && m_flEnqueuedCastTime[i] <= gpGlobals->curtime )
		{
			m_bEnqueuedModules[i] = false;
			if ( a->GetCastSound() != NULL )
				StopSound( a->GetCastSound() ); // stop the casting sound when actually doing the ability
			if ( a->GetUseSound() != NULL )
				EmitSound( a->GetUseSound() );
			DoModule(a,true);
		}
		...
		int level = GetModuleLevel(i);
		if ( a->UsesSmoothAuxDrain() && pPlayer )
		{// attempt drain for this frame, at "per second" rate
			float frameDrain = a->GetAuxDrain(pPlayer, level) * gpGlobals->frametime; 
			if ( pPlayer->GetAuxPower() < frameDrain )
			{
				EmitSound( "Module.ToggleFail" );
				StopModule(a);
				continue;
			}
			pPlayer->DrainAuxPower(frameDrain);
		}

		if ( m_flModuleCooldownTime.Get(i) <= gpGlobals->curtime )
		{
			
			if ( !a->UsesSmoothAuxDrain() && pPlayer && pPlayer->GetAuxPower() < a->GetAuxDrain(pPlayer, level) ) // not enough power available
			{
				EmitSound( "Module.ToggleFail" );
				StopModule(a);
				continue;
			}
			// ticking ability management
			if ( a->Ticks() )
			{
				if ( m_flModuleCooldownTime.Get(i) <= gpGlobals->curtime )
				{
				...
				m_flModuleCooldownTime.Set(a->GetModuleIndex(),gpGlobals->curtime + a->GetCooldown(level));

			if ( a->GetMaxDuration(level) > 0 && m_flModuleStartTime[i] + a->GetMaxDuration(level) <= gpGlobals->curtime )
				StopModule(a); // max duration expired
		}
	}

	// cloaking needs extra special treatment
	if ( !IsBuffActive(BUFF_SPAWN) )
	{
		byte a = GetRenderColor().a;
		Module *cloak = GetModule(CLOAK);
		bool bCloaked = IsModuleActive( cloak ) && !cloak->IsSuppressed(this);
		if ( bCloaked &&  a > FULLY_CLOAKED )
		{
			a = max( FULLY_CLOAKED, a - CLOAK_RATE * gpGlobals->frametime );
		}
		else if ( !bCloaked && a < FULLY_VISIBLE )
		{
			a = min( FULLY_VISIBLE, a + DECLOAK_RATE * gpGlobals->frametime );
		}
...
void CBaseCombatCharacter::Extinguish()
{
...
// abilities!
int CBaseCombatCharacter::GetModuleLevel(Module *a)
{
	return GetModuleLevel(a->GetModuleIndex());
}

int CBaseCombatCharacter::GetModuleLevel(int index)
{
	return m_iModules.Get(index);
}

void CBaseCombatCharacter::SetModuleLevel(Module *a, int level)
{
	m_iModules.Set(a->GetModuleIndex(),level);
}

bool CBaseCombatCharacter::HasModule(Module *a)
{
	return GetModuleLevel(a->GetModuleIndex()) != 0;
}

bool CBaseCombatCharacter::HasModule(int index)
{
	return GetModuleLevel(index) != 0;
}

bool CBaseCombatCharacter::IsModuleActive(Module *a)
{
	return IsModuleActive(a->GetModuleIndex())/* && !a->IsSuppressed(this)*/; // suppression marking modules as inactive stopped toggles being turned off while suppressed
}

bool CBaseCombatCharacter::IsModuleActive(int index)
{
	return m_bModulesActive.Get(index)/* && !a->IsSuppressed(this)*/; // suppression marking modules as inactive stopped toggles being turned off while suppressed
}

void CBaseCombatCharacter::BuyModule(Module *a)
{
	bool bEnabled = IsModuleActive(a);
	if ( bEnabled && a->ShouldTurnOffBeforeUpgrading() )
	{
		g_bShowdamageScaleMessages = false;
		StopModule(a);
		g_bShowdamageScaleMessages = true;
	...
	int index = a->GetModuleIndex();
	m_iModules.Set(index, m_iModules.Get(index) + 1);

	if ( bEnabled || a->AutoStarts() )
		StartModule(a);
	if ( !bEnabled )
		m_flModuleCooldownTime.Set(index, gpGlobals->curtime + a->GetInitialCooldown(GetModuleLevel(a)));
	Buff *aff = a->GetAssociatedBuff();
	if ( aff != NULL )
		ApplyBuff(aff->GetBuffIndex(), NULL, GetModuleLevel(a));
}

void CBaseCombatCharacter::UseModule(Module *a)
{
	if ( GetFlags() & FL_FROZEN )
		return; // cannot control abilities while frozen

	// handle start / stop stuff
	if ( a->IsToggled() )
	{
		if ( !IsModuleActive(a) )
			StartModule(a);
		else if ( a->AllowManualStop() )
			StopModule(a);
	}
	else
	{// its a manually cast ability, if the cast time is greater than 0, enqueue it, otherwise, just do it
		if ( a->GetCastTime(GetModuleLevel(a)) > 0.0f )
		{
//			ClientPrint(this,HUD_PRINTNOTIFY,UTIL_VarArgs("Casting %s\n",a->GetDisplayName()));
			EnqueueModule(a);
		}
		else
		{
//			ClientPrint(this,HUD_PRINTNOTIFY,UTIL_VarArgs("Using %s\n",a->GetDisplayName()));
			DoModule(a);
		}
	}
}

bool CBaseCombatCharacter::EnqueueModule(Module *a)
{
	if ( !HasModule(a) )
		return false;

	int i = a->GetModuleIndex();

	if ( m_flModuleCooldownTime.Get(i) > gpGlobals->curtime )
		return false; // cooldown has not expired

	if ( a->GetCastSound() != NULL )
		EmitSound( a->GetCastSound() );

	m_bEnqueuedModules[i] = true;
	m_flEnqueuedCastTime[i] = gpGlobals->curtime + a->GetCastTime(GetModuleLevel(a));
	m_flModuleCooldownTime.Set(i,gpGlobals->curtime + a->GetCooldown(GetModuleLevel(a)));
	a->StartEffect(this,GetModuleLevel(a)); // not the full StartModule, with all its guff
	return true;
}

bool CBaseCombatCharacter::DoModule(Module *a, bool isEnqueued)
{
	if ( !IsAlive())
		return false;
	...
	if ( !isEnqueued && m_flModuleCooldownTime.Get(a->GetModuleIndex()) > gpGlobals->curtime )
		return false; // cooldown has not expired

	bool shouldDoUseSound = !a->IsToggled();
	
	// if effect succeeded, drain power & reset cooldown
	if ( a->DoEffect(this,GetModuleLevel(a)) )
	{
		if ( !isEnqueued )
		{
			if ( a->GetCastTime(GetModuleLevel(a)) == 0 )
				if ( a->Ticks() )
					m_flModuleCooldownTime.Set(a->GetModuleIndex(),gpGlobals->curtime + a->GetTickInterval(GetModuleLevel(a)));
				else
					m_flModuleCooldownTime.Set(a->GetModuleIndex(),gpGlobals->curtime + a->GetCooldown(GetModuleLevel(a)));
		}
		if ( shouldDoUseSound && a->GetUseSound() != NULL )
			EmitSound( a->GetUseSound() );
		return true;
	}
	else
...
bool CBaseCombatCharacter::StartModule(Module *a)
{
	if ( !IsAlive())
		return false;
	...
	int i = a->GetModuleIndex();
	if ( !a->AutoStarts() && m_flModuleCooldownTime.Get(i) > gpGlobals->curtime )
		return false; // still cooling down

	m_bModulesActive.Set(i,true);
	m_flModuleStartTime[i] = gpGlobals->curtime;

	if ( a->IsSuppressed(this) )
		EnqueueModule(a);
	else
	{
		a->StartEffect(this,GetModuleLevel(a));
		if ( a->GetCastSound() != NULL )
			EmitSound( a->GetCastSound() );
		m_flModuleCooldownTime.Set(i,gpGlobals->curtime);
	}
	
	return true;
}

void CBaseCombatCharacter::StopModule(Module *a)
{
	if ( !a->IsToggled() )
		return;
	...
	if ( !IsModuleActive(a) ) // suppressed active modules return false here, so can't be turned off. Oops.
		return;

	if ( a->GetUseSound() != NULL )
		StopSound( a->GetUseSound() );

	if ( a->GetTickSound() != NULL )
		StopSound( a->GetTickSound() );

	if ( a->GetCastSound() != NULL )
		StopSound( a->GetCastSound() );

	int level = GetModuleLevel(a);
	int i = a->GetModuleIndex();
	
	if ( a->GetCooldown(level) > 0 ) // this only currently has an effect for Flechettes. Other modules cooldowns are handled elsewhere
		m_flModuleCooldownTime.Set(i,gpGlobals->curtime + a->GetCooldown(level));
		
	m_bModulesActive.Set(i,false);
	a->StopEffect(this, level);
}

void CBaseCombatCharacter::RemoveAllModules()
{
	for ( int i=0; i<NUM_MODULES; i++ )
	{
		m_iModules.Set(i,0);
...
// cloaking is mostly handled in postthink
void CBaseCombatCharacter::EnableCloaking(bool bOn, bool bIsFiring)
{
	if ( bOn )
	{
		AddFlag(FL_NOTARGET);
		...
		for ( int i = 0; i < nAIs; i++ )// look through all the AIs and check each
		{
			...
			if ( pNPC->GetMasterPlayer() == this && pNPC->IsCurSchedule(SCHED_HUNT_MASTER) )
			{
				pNPC->TaskFail(FAIL_NO_ENEMY); // forget me for now
			}
		}
//		SetRenderMode(kRenderTransColor);
//		SetRenderColorA( FULLY_CLOAKED );
	}
	else
	{
		RemoveFlag(FL_NOTARGET);

		// pvms with me as their master should now resume the chase automatically

		if ( bIsFiring )
			SetRenderColorA( FULLY_VISIBLE );
	}
}

void CBaseCombatCharacter::SetRenderColorA( byte a )
{
	BaseClass::SetRenderColorA(a > m_iMaxRenderA ? m_iMaxRenderA : a);
}

void CBaseCombatCharacter::SetMaxRenderColorA( byte a )
{
	// if increasing the limit (and we're already on it), set render color to new limit
	// if decreasing the limit (and we're above the new limit) send render color to new limit
	if ( GetRenderColor().a > a || GetRenderColor().a == m_iMaxRenderA ) 
		BaseClass::SetRenderColorA(a);
	m_iMaxRenderA = a;
}

void CBaseCombatCharacter::RegisterAttackOnEntity( CTakeDamageInfo &info, CBaseCombatCharacter *pVictim, bool isKillingBlow )
{
	if( !pVictim )
		return;
	if ( !IsNPC() && !IsPlayer() )
	...
	// how do we handle all this in a non-slow manner? I reckon the victim should "store up" damage record sql insert statements, until it finally receives a killing blow, and then execute them all in a single transaction.
	// that's probably the easiest way of tracking which "blows" add up to what kills, without assigning "current database kill IDs" to every NPC / player. Although, we'll need to store characterID against each kill part
		
	// add killpart to list
	int index = pVictim->m_storedDamageCommands.AddToTail();
	if ( IsPlayer() )
	{
		// determine what this damage was done with ... was it a module, a weapon, or something else?
		const char *inflictor;
		if ( info.GetModule() != NO_MODULE )
			inflictor = GetModule(info.GetModule())->GetDisplayName();
		else if ( info.GetInflictor() )
		{
			CBaseCombatCharacter *pInflictor = info.GetInflictor()->MyCombatCharacterPointer();
			if ( pInflictor && pInflictor->GetControllingModule() != NO_MODULE )
				inflictor = GetModule(pInflictor->GetControllingModule())->GetDisplayName();
			else if ( info.GetInflictor() == info.GetAttacker() && info.GetWeapon() != NULL )
				inflictor = info.GetWeapon()->GetClassname(); // for bullets, Inflictor was giving the player rather than the weapon name
			else
				inflictor = info.GetInflictor()->GetClassname();
		}
		else
			inflictor = "unknown";
		
		pVictim->m_storedDamageCommands[index] = UTIL_VarArgs("insert into killparts (killid, gameid, killerid, killertype, inflictor, damageamount, iskillingblow) values (%%i, '%s', %i, 'player', '%s', %f, %i)", HL2MPRules()->GetGameID(), ToHL2MPPlayer(this)->GetCharacterID(), inflictor, info.GetDamage(), isKillingBlow ? 1 : 0);
	}
	else
		pVictim->m_storedDamageCommands[index] = UTIL_VarArgs("insert into killparts (killid, gameid, killerid, killertype, inflictor, damageamount, iskillingblow) values (%%i, '%s', null, '%s', '%s', %f, %i)", HL2MPRules()->GetGameID(), MyNPCPointer()->GetTypeName(), MyNPCPointer()->GetTypeName(), info.GetDamage(), isKillingBlow ? 1 : 0);

	if ( isKillingBlow )
	{// create kills entry, execute all killparts with the killid of the new kill entry
		int killID = HL2MPRules()->m_iNextKillID;
		HL2MPRules()->m_iNextKillID++;
		if ( pVictim->IsPlayer() )
			g_pDB2->Command("insert into kills (killid, gameid, victimid, victimtype, victimlevel) values (%i, '%s', %i, 'player', %i)", killID, HL2MPRules()->GetGameID(), ToHL2MPPlayer(pVictim)->GetCharacterID(), pVictim->GetLevel());
		else
			g_pDB2->Command("insert into kills (killid, gameid, victimid, victimtype, victimlevel) values (%i, '%s', null, '%s', %i)", killID, HL2MPRules()->GetGameID(), pVictim->MyNPCPointer()->GetTypeName(), pVictim->GetLevel());
		
		
		// for each killpart command stored up on the victim, run the command, substituting in the new killID
		for ( int i=0; i<pVictim->m_storedDamageCommands.Count(); i++ )
			g_pDB2->Command(pVictim->m_storedDamageCommands[i].c_str(), killID);
		pVictim->m_storedDamageCommands.Purge();
	}
}

void CBaseCombatCharacter::AddScoreToken(int num)
{
	m_iNumScoreTokens += num;
	
	// adjust global token count
	if ( HL2MPRules() )
		HL2MPRules()->AdjustTotalNumScoreTokens(num);
}

bool CBaseCombatCharacter::RemoveScoreToken(int num)
{
	if ( m_iNumScoreTokens <= 0 )
		return false;
	m_iNumScoreTokens = max(m_iNumScoreTokens-num, 0);
