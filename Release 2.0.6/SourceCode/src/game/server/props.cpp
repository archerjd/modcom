...
float GetBreakableDamage( const CTakeDamageInfo &inputInfo, IBreakableWithPropData *pProp )
{
	...
	// Poison does no damage, but acid should do enough that a poison spit can break a box / barrel
	if ( iDmgType & DMG_POISON )
		flDamage = 0;
	if ( iDmgType & DMG_POISON )
		flDamage *= 3.5f;
...
#ifdef USE_OMNIBOT
int CBreakableProp::GetOmnibotClass() const {
	if ( m_explodeDamage > 0 || m_explodeRadius > 0 )
		return Omnibot::ENT_CLASS_GENERIC_PROP_EXPLODE;
	return Omnibot::ENT_CLASS_GENERIC_PROP;
}
#endif
