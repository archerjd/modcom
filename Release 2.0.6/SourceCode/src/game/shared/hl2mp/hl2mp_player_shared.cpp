...
extern McConVar pvm_max_ally_bonus_distance;

ConVar mc_dev_force_ally_count("mc_dev_force_ally_count", "-1", FCVAR_CHEAT | FCVAR_NOTIFY | FCVAR_REPLICATED, "Forces all players' nearby ally count to this value. -1 to disable forcing.", true, -1, true, 4);

int CHL2MP_Player::NumAlliesInRange()
{
	if ( !HL2MPRules()->IsTeamplay() )
		return 0;

	int forcedVal = mc_dev_force_ally_count.GetInt();
	if ( forcedVal != -1 )
		return forcedVal;
...
#ifdef CLIENT_DLL
		if ( g_PR->IsInCharacter(i) && g_PR->GetFaction(i) == GetFaction() && i != entindex() && g_PR->IsAlive(i) )
#else
		if ( pOther && pOther->IsInCharacter() && pOther->GetFaction() == GetFaction() && pOther != this && pOther->IsAlive() )
#endif
		{
			// if a trace line can be done from them to me, they count as a "nearby ally"
			trace_t tr;
			Vector start = WorldSpaceCenter();
#ifdef CLIENT_DLL
			Vector end = g_PR->GetPos(i);
#else
			Vector end = pOther->WorldSpaceCenter();
#endif
			UTIL_TraceLine(start,end,MASK_SOLID_BRUSHONLY,this,COLLISION_GROUP_PLAYER_MOVEMENT,&tr);
			if ( (tr.fraction == 1.0f || (tr.m_pEnt == pOther && tr.m_pEnt != NULL)) && (start-end).Length() <= pvm_max_ally_bonus_distance.GetFloat() )
				num ++;
		}
	}
	
	return min(4,num); // will not return more than 4
}

bool CHL2MP_Player::ShouldCollide( int collisionGroup, int contentsMask, int playerIndex ) const
{
	if ( collisionGroup != COLLISION_GROUP_NONE )
	{
		if ( playerIndex > 32 )
			playerIndex -= 32; // its a minion, treat it like a player
		CBaseEntity *pOther = UTIL_PlayerByIndex(playerIndex);
		if ( pOther != NULL && ( pOther == this || HL2MPRules()->IsFriendly((CBaseEntity*)this, pOther) ) )
			return false;
	}

	return BaseClass::ShouldCollide(collisionGroup,contentsMask,playerIndex);
}

bool CAI_BaseNPC::ShouldCollide( int collisionGroup, int contentsMask, int playerIndex ) const
{
	if ( collisionGroup != COLLISION_GROUP_NONE )
	{
		// NPCs don't turn minion player indexes into proper player indexes, so should collide with each other
		CBaseEntity *pOther = UTIL_PlayerByIndex(playerIndex);
		if ( pOther != NULL && ( pOther == this || HL2MPRules()->IsFriendly((CBaseEntity*)this, pOther) ) )
			return false;
	}
...
// ugly location cos i'm going mad and don't really care any more :)
//#ifdef CLIENT_DLL
	for ( int i=1; i<LAST_ENTITY_EFFECT; i++ )
		PrecacheParticleSystem(g_szEntityEffectNames[i]);
//#endif
}
...
void CPlayerAnimState::ComputePlaybackRate()
{
	...
	if ( isMoving && ( maxspeed > 0.0f ) )
	{
		...
		GetOuter()->SetPlaybackRate( min(( speed * flFactor ) / maxspeed, 12) );
...
// how much exp is needed to progress all the way through the given level to the next one
extern LEVEL_EXTERN(mc_levelexp);
int ExpForLevelUp( int level )
{
	return LEVEL(mc_levelexp, level);
}

// how much exp is needed IN TOTAL to reach the next level
int TotExpForLevelUp( int level )
{
	int tot = 0;
