...
extern McConVar weapon_frag_damage, weapon_frag_radius, weapon_frag_refire;
...
public:
	...
	virtual int GetPrimaryAttackDamage() { return weapon_frag_damage.GetInt(); }
	virtual float GetPrimaryAttackRefire() { return 0; }
...
//	vForward[0] += 0.1f;
	...
	CBaseGrenade *pGrenade = Frag_Grenade_Create( vecSrc, vec3_angle, vecThrow, AngularImpulse(600,random->RandomInt(-1200,1200),0), pPlayer, GRENADE_TIMER, false );
	...
	if ( pGrenade )
	{
		...
		if ( pHL2MPPlayer )
		{
			float damage = GetPrimaryAttackDamage() * pHL2MPPlayer->GetDamageDealtScale();
			//damage += damage * pHL2MPPlayer->GetFactionalDamageScaleBoost();
			pGrenade->SetDamage( damage );
		}
		else
			pGrenade->SetDamage( GetPrimaryAttackDamage() );
		pGrenade->SetDamageRadius( weapon_frag_radius.GetFloat() );
	}
#endif
...
void CWeaponFrag::LobGrenade( CBasePlayer *pPlayer )
{
	...
	CBaseGrenade *pGrenade = Frag_Grenade_Create( vecSrc, vec3_angle, vecThrow, AngularImpulse(200,random->RandomInt(-600,600),0), pPlayer, GRENADE_TIMER, false );
...
void CWeaponFrag::RollGrenade( CBasePlayer *pPlayer )
{
	...
	CBaseGrenade *pGrenade = Frag_Grenade_Create( vecSrc, orientation, vecThrow, rotSpeed, pPlayer, GRENADE_TIMER, false );
