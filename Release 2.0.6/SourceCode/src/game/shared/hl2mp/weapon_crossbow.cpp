...
#define BOLT_WATER_VELOCITY_FRACTION	0.4286f
#define	BOLT_SKIN_NORMAL	0
#define BOLT_SKIN_GLOW		1

extern McConVar weapon_crossbow_primary_damage, weapon_crossbow_primary_refire, weapon_crossbow_clip_size, weapon_crossbow_reload_time, weapon_crossbow_bolt_speed;
...
void CCrossbowBolt::BoltTouch( CBaseEntity *pOther )
{
	...
	if ( pOther->m_takedamage != DAMAGE_NO )
	{
		...
		{
			CTakeDamageInfo	dmgInfo( this, GetOwnerEntity(), m_iDamage, DMG_SLASH );
		...
		{
			CTakeDamageInfo	dmgInfo( this, GetOwnerEntity(), m_iDamage, DMG_SLASH | DMG_NEVERGIB );
			CalculateMeleeDamageForce( &dmgInfo, vecNormalizedVel, tr.endpos, 0.7f );
			dmgInfo.SetDamagePosition( tr.endpos );
			pOther->DispatchTraceAttack( dmgInfo, vecNormalizedVel, &tr );
		}
...
#ifdef USE_OMNIBOT
	int GetOmnibotClass() const { return Omnibot::MC_CLASSEX_WEAPON + Omnibot::MC_WP_CROSSBOW; }
#endif
#endif
...
private:
	...
	virtual int GetPrimaryAttackDamage() { return weapon_crossbow_primary_damage.GetInt(); }
	virtual float GetPrimaryAttackRefire() { return weapon_crossbow_primary_refire.GetFloat(); }
	virtual int GetSecondaryAttackDamage() { return 0; }
	virtual float GetSecondaryAttackRefire() { return 0; }
	virtual float GetReloadDuration() { return weapon_crossbow_reload_time.GetFloat(); }
	virtual int GetClipSize() { return weapon_crossbow_clip_size.GetInt(); }
	
	int	GetExtraClip(int clipSizeLevel) { return LEVEL(mod_clipsize_crossbow, clipSizeLevel); }
...
void CWeaponCrossbow::FireBolt( void )
{
	...
	float damage = pHL2MPPlayer->GetDamageDealtScale() * GetPrimaryAttackDamage();
	damage += damage * pHL2MPPlayer->GetFactionalDamageScaleBoost();
	CCrossbowBolt *pBolt = CCrossbowBolt::BoltCreate( vecSrc, angAiming, damage, pOwner );

	if ( pOwner->GetWaterLevel() == 3 )
	{
		pBolt->SetAbsVelocity( vecAiming * weapon_crossbow_bolt_speed.GetFloat() * BOLT_WATER_VELOCITY_FRACTION );
	}
	else
	{
		pBolt->SetAbsVelocity( vecAiming * weapon_crossbow_bolt_speed.GetFloat() );
	}
	...
	m_flNextPrimaryAttack = m_flNextSecondaryAttack	= gpGlobals->curtime + GetPrimaryAttackRefire();
