...
extern McConVar weapon_rpg_primary_damage, weapon_rpg_primary_refire, weapon_rpg_clip_size, weapon_rpg_reload_time, weapon_rpg_rocket_speed;
...
void CMissile::AccelerateThink( void )
{
	...
	SetAbsVelocity( vecForward * weapon_rpg_rocket_speed.GetFloat() );
...
int CWeaponRPG::GetPrimaryAttackDamage() { return weapon_rpg_primary_damage.GetInt(); }
float CWeaponRPG::GetPrimaryAttackRefire() { return weapon_rpg_primary_refire.GetFloat(); }
float CWeaponRPG::GetReloadDuration() { return weapon_rpg_reload_time.GetFloat(); }
int CWeaponRPG::GetClipSize() { return weapon_rpg_clip_size.GetInt(); }
...
#ifdef USE_OMNIBOT
int CWeaponRPG::GetOmnibotClass() const 
{
	return Omnibot::MC_CLASSEX_WEAPON + Omnibot::MC_WP_RPG; 
}
#endif
#endif
...
void CWeaponRPG::PrimaryAttack( void )
{
	...
	m_flNextPrimaryAttack = gpGlobals->curtime + weapon_rpg_primary_refire.GetFloat();
	...
	float damage = GetPrimaryAttackDamage() * pPlayer->GetDamageDealtScale();
	//damage += damage * pPlayer->GetFactionalDamageScaleBoost();
	pMissile->SetDamage( damage );
	m_hMissile = pMissile;
...
int	CWeaponRPG::GetExtraClip(int clipSizeLevel)
{
	return LEVEL(mod_clipsize_rpg, clipSizeLevel);
}
