...
extern McConVar weapon_gravitygun_refire;
...
#define MEGACANNON_ENDCAP_SPRITE "sprites/blueflare3"
#define MEGACANNON_CENTER_GLOW "effects/fluttercore"
#define MEGACANNON_BLAST_SPRITE "effects/fluttercore"
	...
	CLIENTEFFECT_MATERIAL( "sprites/lgtning_noz" )
...
public:
	...
	bool ShouldUseOrangeGlow() { CHL2MP_Player *pOwner = ToHL2MPPlayer(GetOwner()); return pOwner && pOwner->GetFaction() == FACTION_RESISTANCE; }

	void	GetLaunchForce( CBaseEntity *pEntity, float &lVel, AngularImpulse &aVel );
	
	virtual int GetPrimaryAttackDamage() { return 0; }
	virtual float GetPrimaryAttackRefire() { return weapon_gravitygun_refire.GetFloat(); }
...
protected:
	...
	int GetOmnibotClass() const { return Omnibot::MC_CLASSEX_WEAPON + Omnibot::MC_WP_GRAVGUN; }
#endif
#endif	// !CLIENT_DLL
...
#ifdef CLIENT_DLL
void CPhysCannonEffectBeam::Init( int startAttachment, int endAttachment, CBaseEntity *pEntity, bool firstPerson )
{
	if ( m_pBeam != NULL )
		return;
	...
	CWeaponPhysCannon *pPhysCannon = dynamic_cast<CWeaponPhysCannon *>(pEntity);
	beamInfo.m_pszModelName = (pPhysCannon && pPhysCannon->ShouldUseOrangeGlow()) ? (( firstPerson ) ? PHYSCANNON_BEAM_SPRITE_NOZ : PHYSCANNON_BEAM_SPRITE) : (( firstPerson ) ? MEGACANNON_BEAM_SPRITE_NOZ : MEGACANNON_BEAM_SPRITE);
...
void CWeaponPhysCannon::PuntVPhysics( CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr )
{
	...
	m_nChangeState = ELEMENT_STATE_CLOSED;
	m_flElementDebounce = gpGlobals->curtime + GetPrimaryAttackRefire();
	m_flCheckSuppressTime = gpGlobals->curtime + GetPrimaryAttackRefire() * 0.5f;

	// Don't allow the gun to regrab a thrown object!!
	m_flNextSecondaryAttack = m_flNextPrimaryAttack = gpGlobals->curtime + GetPrimaryAttackRefire();
}

void CWeaponPhysCannon::GetLaunchForce( CBaseEntity *pEntity, float &lVel, AngularImpulse &aVel )
{
#ifndef CLIENT_DLL
...
void CWeaponPhysCannon::ApplyVelocityBasedForce( CBaseEntity *pEntity, const Vector &forward )
{
	...
	float flForce = 0.f;
	AngularImpulse aVel;	
	GetLaunchForce( pEntity, flForce, aVel );
...
void CWeaponPhysCannon::PrimaryAttack( void )
{
	...
	m_flNextPrimaryAttack = gpGlobals->curtime + GetPrimaryAttackRefire();
...
void CWeaponPhysCannon::SecondaryAttack( void )
{
	...
	if ( ( m_bActive ) && ( pOwner->m_afButtonPressed & IN_ATTACK2 ) )
	{
		...
		m_flNextPrimaryAttack = gpGlobals->curtime + GetPrimaryAttackRefire();
		m_flNextSecondaryAttack = gpGlobals->curtime + GetPrimaryAttackRefire();
...
/*	if( pOwner )
	...
	m_flNextSecondaryAttack = gpGlobals->curtime + GetPrimaryAttackRefire() * 0.8f;
...
void CWeaponPhysCannon::ItemPostFrame()
{
	...
	{
		// pulling stuff in shouldn't decloak you
		SecondaryAttack();
	}
	else
	{
...
bool CWeaponPhysCannon::CanPickupObject( CBaseEntity *pTarget )
{
	...
	if ( pTarget->ClassMatches("npc_turret_floor") )
	{
		CAI_BaseNPC *pTurret = static_cast<CAI_BaseNPC*>(pTarget);
		return pTurret->LikesMaster() && pTurret->GetMasterPlayer() == pOwner;
	}
...
{
#ifdef CLIENT_DLL
	bool bShouldUseOrangeGlow = ShouldUseOrangeGlow();
	...
	if ( m_Parameters[PHYSCANNON_CORE].GetMaterial() == NULL )
	{
		...
		if ( m_Parameters[PHYSCANNON_CORE].SetMaterial( bShouldUseOrangeGlow ? PHYSCANNON_CENTER_GLOW : MEGACANNON_CENTER_GLOW ) == false )
		{
	...
	if ( m_Parameters[PHYSCANNON_BLAST].GetMaterial() == NULL )
	{
		...
		if ( m_Parameters[PHYSCANNON_BLAST].SetMaterial( bShouldUseOrangeGlow ? PHYSCANNON_BLAST_SPRITE : MEGACANNON_BLAST_SPRITE ) == false )
		{
			// This means the texture was not found
			Assert( 0 );
		}
	...
	for ( int i = PHYSCANNON_GLOW1; i < (PHYSCANNON_GLOW1+NUM_GLOW_SPRITES); i++ )
	{
		...
		m_Parameters[i].SetColor( bShouldUseOrangeGlow ? Vector( 255, 128, 0 ) : Vector ( 96, 192, 255 ) );
	...
	for ( int i = PHYSCANNON_ENDCAP1; i < (PHYSCANNON_ENDCAP1+NUM_ENDCAP_SPRITES); i++ )
	{
		...
		if ( m_Parameters[i].SetMaterial( bShouldUseOrangeGlow ? PHYSCANNON_ENDCAP_SPRITE : MEGACANNON_ENDCAP_SPRITE ) == false )
		{
			// This means the texture was not found
			Assert( 0 );
		}
...
void CallbackPhyscannonImpact( const CEffectData &data )
{
	...
	if ( pWeapon->GetOwner() == CBasePlayer::GetLocalPlayer() )
	{
		...
		beamInfo.m_pszModelName = (pPhysCannon && pPhysCannon->ShouldUseOrangeGlow()) ? PHYSCANNON_BEAM_SPRITE : MEGACANNON_BEAM_SPRITE;
		beamInfo.m_flHaloScale = 0.0f;
