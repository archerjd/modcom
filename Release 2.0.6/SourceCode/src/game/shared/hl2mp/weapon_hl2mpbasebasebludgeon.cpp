...
extern float BruteForceDamageScale(int level);
extern float CalcStrengthScale(int level);
extern LEVEL_EXTERN(mod_impact);
extern LEVEL_EXTERN(mod_bruteforce);

void CBaseHL2MPBludgeonWeapon::Hit( trace_t &traceHit, Activity nHitActivity, bool isCrit )
{
...
#ifndef CLIENT_DLL
		int nDamageType = isCrit ? DMG_CLUB | DMG_CRIT : DMG_CLUB;
		float critDamageScaler = isCrit ? critDamageScaler = LEVEL(mod_crits_scale, pPlayer->GetModuleLevel(CRITS)) : 1.0f;
		float bruteForceScaler = 1 + LEVEL(mod_bruteforce, pPlayer->GetModuleLevel(BRUTEFORCE));
		float damageDealtScale = pPlayer->GetDamageDealtScale() - LEVEL(mod_impact, pPlayer->GetModuleLevel(GetModule(STRENGTH))); // consider other damage scaling effects, not NOT impact/strength. It doesn't affect melee.
		float damage = critDamageScaler * bruteForceScaler * damageDealtScale * GetDamageForActivity( nHitActivity );
		damage += damage * pPlayer->GetFactionalDamageScaleBoost();

		CTakeDamageInfo info( GetOwner(), GetOwner(), damage, nDamageType );
...
void CBaseHL2MPBludgeonWeapon::Swing( int bIsSecondary )
{
	...
	if ( pOwner->IsModuleActive(GetModule(CRITS)) )
	{
		Module *crits = GetModule(CRITS);
		int level = pOwner->GetModuleLevel(crits);
		float auxNeeded = GetDamageForActivity( nHitActivity ) * LEVEL(mod_crits_drain_proportion, level);
		if ( pOwner->GetAuxPower() < auxNeeded )
#ifndef CLIENT_DLL
			pOwner->StopModule(crits);
#else
			;
#endif
		else
		{
			RandomSeed(iSeed);
			int level = pOwner->GetModuleLevel(GetModule(CRITS));
			if ( random->RandomFloat(0,100) <= LEVEL(mod_crits_chance, level) )
			{
				isCrit = true;
#ifdef CLIENT_DLL
				if ( C_BasePlayer::GetLocalPlayer() == pOwner )
				{// when shooting a crit, play a pew pew noise
					EmitSound("Crit.Shoot");
				}
#else
				// drain aux proportional to (unmodified) base damage
				pOwner->DrainAuxPower(auxNeeded);
#endif
			}
		}
	}
	...
	{
		Hit( traceHit, nHitActivity, isCrit );
	}
