...
int CWeaponHL2MPBase::GetMaxClip1( void )
{
	int extra = 0;
	...
	return GetClipSize() + extra;
}

bool CWeaponHL2MPBase::DefaultReload( int iClipSize1, int iClipSize2, int iActivity )
{
	bool retVal = BaseClass::DefaultReload(iClipSize1, iClipSize2, iActivity);
	if ( retVal )
	{
		float flSequenceEndTime = gpGlobals->curtime + GetReloadDuration();
		GetOwner()->SetNextAttack( flSequenceEndTime );
...
void CWeaponHL2MPBase::FireBullets( const FireBulletsInfo_t &info )
{
	...
	modinfo.m_iPlayerDamage = modinfo.m_iDamage = GetPrimaryAttackDamage(); //GetHL2MPWpnData().m_iPlayerDamage;
