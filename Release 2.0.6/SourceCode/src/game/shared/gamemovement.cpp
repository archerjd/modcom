...
#define JETPACK_IMPULSE_PER_SECOND	800
#define JETPACK_SIDE_IMPULSE 150
//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CGameMovement::FullWalkMove( )
{
	...
	// Not fully underwater
	{
		...
		// if jetpack doesn't happen after CategorisePosition, can't get off the ground
		if ( player->IsBuffActive(BUFF_JETPACK_FLAME) )
		{
			player->SetGroundEntity(NULL);
			//CheckJumpButton(); // jump to kick us off? no, too bouncy
			Vector thrust = Vector(0,0,JETPACK_IMPULSE_PER_SECOND);

			// now check forward/back/left/right keys, apply extra thrust
			Vector forward, right, up;
			...
			if ( player->m_nButtons & IN_FORWARD )
				thrust += forward * JETPACK_SIDE_IMPULSE;
			if ( player->m_nButtons & IN_BACK )
				thrust -= forward * JETPACK_SIDE_IMPULSE;
			if ( player->m_nButtons & IN_MOVERIGHT )
				thrust += right * JETPACK_SIDE_IMPULSE;
			if ( player->m_nButtons & IN_MOVELEFT )
				thrust -= right * JETPACK_SIDE_IMPULSE;
			mv->m_vecVelocity += thrust * gpGlobals->frametime;
			//player->ApplyAbsVelocityImpulse(thrust * gpGlobals->frametime);
		}
	}



//	now handle gravity wells from magmines
	...
	for ( int i=0; i<MAX_MAGMINES; i++ )
	{
		...
		gravWellForce += CalcMagdForce(well,player,gpGlobals->frametime);
	}
	mv->m_vecVelocity += gravWellForce;
...
bool CGameMovement::CheckJumpButton( void )
{
	...
	// In the air now.
    SetGroundEntity( NULL );
	bool bLongJumped = false;
	CHL2MP_Player *plr = ToHL2MPPlayer(player);
	Module *a = GetModule(LONG_JUMP);
	int ljLevel = plr->GetModuleLevel(a);
	if ( plr->HasModule(a) && ( player->m_Local.m_flDucktime > GAMEMOVEMENT_DUCK_TIME - LEVEL(mod_longjump_use_interval, ljLevel) )
		&& player->GetWaterLevel() < 2 )
	{
		Vector moveDir = player->GetAbsVelocity();
		moveDir.z = 0;
		...
		if ( moveDir.Length2D() > 0 )
		{
			...
			Vector forward, forward2d;
			player->EyeVectors( &forward, NULL, NULL );
			forward2d = forward;
			forward2d.z = 0;
			...
			if ( forward.z < 0 )
				...
				forward.z *= LEVEL(mod_longjump_vertical_scale_factor, ljLevel);

			if ( DotProduct(forward2d, moveDir) > 0.5 )
			{// do long jump if we have the energy, then
				if ( plr->GetAuxPower() >= a->GetAuxDrain(plr,ljLevel) )
				{
					mv->m_vecVelocity = forward * LEVEL(mod_longjump_speed, ljLevel);
					bLongJumped = true;
#ifndef CLIENT_DLL
					plr->DrainAuxPower(a->GetAuxDrain(plr,plr->GetModuleLevel(a)));
#endif
				}
			}
		}
	...
	if ( !bLongJumped && ( (  player->m_Local.m_bDucking ) || (  player->GetFlags() & FL_DUCKING ) ) )
	{
...
//-----------------------------------------------------------------------------
#define PLAYER_CRUSH_HEADCRAB_THRESHOLD		32.0f // how fast you must be falling to crush a headcrab
extern McConVar mc_headcrab_bounce;
#ifndef CLIENT_DLL
...
void CGameMovement::CheckFalling( void )
{
	if ( player->GetGroundEntity() != NULL &&
		 !IsDead() &&
		 player->m_Local.m_flFallVelocity >= PLAYER_CRUSH_HEADCRAB_THRESHOLD &&
		  //dynamic_cast<CBaseHeadcrab*>( player->GetGroundEntity()) != NULL )
		  player->GetGroundEntity() != NULL && player->GetGroundEntity()->IsNPC() &&
#ifndef CLIENT_DLL
		 (static_cast<CAI_BaseNPC*>(player->GetGroundEntity()))->CanBeBouncedOn() )
#else
		 (static_cast<C_AI_BaseNPC*>(player->GetGroundEntity()))->CanBeBouncedOn() )
#endif
	{
#ifndef CLIENT_DLL
		CBaseEntity *pGroundCrab = player->GetGroundEntity();		
		CTakeDamageInfo crabSmashInfo = CTakeDamageInfo( pGroundCrab, player, pGroundCrab->GetHealth()*2, DMG_CRUSH);
		pGroundCrab->TakeDamage(crabSmashInfo);//More than enough to squish the bugger
		pGroundCrab->EmitSound("Flesh_Bloody.ImpactHard");
		player->EmitSound( "Headcrab.JumpedOn" );
		ToHL2MPPlayer(player)->AddAuxPower( mc_auxpower_pickup_energy.GetInt() );
#endif
		// bounce the player for jumping on a crab
		if ( mc_headcrab_bounce.GetFloat() > 0 )
		{
			mv->m_vecVelocity[2] = /*pGroundCrab->GetAbsVelocity().z +*/ mc_headcrab_bounce.GetFloat();
			player->m_Local.m_flFallVelocity = 0; // don't take fall damage when bouncing
		}
	}
