...
// only hit entities on the list. Hit nothing else! Still obeys collision group rules, etc
class CTraceFilterHitList : public CTraceFilterSimple
{
public:
	CTraceFilterHitList( int collisionGroup );
	virtual bool ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask );

	void	AddEntityToHit( IHandleEntity *pEntity );
protected:
	CUtlVector<IHandleEntity*>	m_HitEntities;
};
