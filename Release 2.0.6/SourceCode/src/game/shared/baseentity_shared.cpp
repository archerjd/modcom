...
//-----------------------------------------------------------------------------
bool CBaseEntity::ShouldCollide( int collisionGroup, int contentsMask, int playerIndex ) const
{
...
void CBaseEntity::FireBullets( const FireBulletsInfo_t &info )
{
	...
	if ( IsPlayer() )
	{
		...
		CHL2MP_Player *pPlayer = (CHL2MP_Player*)this;
		if ( pPlayer->IsModuleActive(GetModule(CRITS)) )
		{
			Module *crits = GetModule(CRITS);
			int level = pPlayer->GetModuleLevel(crits);
			float auxNeeded = flActualDamage * LEVEL(mod_crits_drain_proportion, level);
			if ( pPlayer->GetAuxPower() < auxNeeded )
#ifndef CLIENT_DLL
				pPlayer->StopModule(crits);
#else
				;
#endif
			else
			{
				...
				if ( random->RandomFloat(0,100) <= LEVEL(mod_crits_chance, level ))
				{
					...
					pPlayer->DrainAuxPower(auxNeeded);
#endif
					flActualDamage *= LEVEL(mod_crits_scale, level);
					iPlayerDamage *= LEVEL(mod_crits_scale, level);
					nDamageType |= DMG_CRIT;
				}
			}
		}
	}
	...
	// bullet's final direction can be changed by passing through a portal
		...
		if (tr.fraction != 1.0)
		{
			...
			if ( !bHitWater || ((info.m_nFlags & FIRE_BULLETS_DONT_HIT_UNDERWATER) == 0) )
			{
				...
				CBaseEntity *pInflictor = this != pAttacker ? this : (pAttacker->MyCombatCharacterPointer() != NULL && pAttacker->MyCombatCharacterPointer()->GetActiveWeapon() != NULL ? pAttacker->MyCombatCharacterPointer()->GetActiveWeapon() : this);
		...
		if ( ( info.m_iTracerFreq != 0 ) && ( tracerCount++ % info.m_iTracerFreq ) == 0 && ( bHitGlass == false ) )
		{
			if ( bDoServerEffects == true )
			{
				...
				MakeTracer( vecTracerSrc, Tracer, nDamageType & DMG_CRIT ? TRACER_CRIT : pAmmoDef->TracerType(info.m_iAmmoType) );
	...
	if ( IsPlayer() && flCumulativeDamage > 0.0f )
	{
		...
		CBaseEntity *pInflictor = this != pAttacker ? this : (pAttacker->MyCombatCharacterPointer() != NULL && pAttacker->MyCombatCharacterPointer()->GetActiveWeapon() != NULL ? pAttacker->MyCombatCharacterPointer()->GetActiveWeapon() : this);
		CTakeDamageInfo dmgInfo( pInflictor, pAttacker, flCumulativeDamage, nDamageType );
...
void CBaseEntity::MakeTracer( const Vector &vecTracerSrc, const trace_t &tr, int iTracerType )
{
	...
	case TRACER_CRIT:
		UTIL_ParticleTracer( "CritTracer", vNewSrc, tr.endpos, entindex(), iAttachment, true );
		break;
	}
}
