...
BEGIN_SIMPLE_DATADESC( CTakeDamageInfo )
	...
	DEFINE_FIELD( m_iModule, FIELD_INTEGER),
END_DATADESC()
...
void CTakeDamageInfo::Init( CBaseEntity *pInflictor, CBaseEntity *pAttacker, CBaseEntity *pWeapon, const Vector &damageForce, const Vector &damagePosition, const Vector &reportedPosition, float flDamage, int bitsDamageType, int iCustomDamage )
{
	...
	m_iModule = NO_MODULE;
	SetInflictor(pInflictor);
}

void CTakeDamageInfo::SetInflictor( CBaseEntity *pInflictor )
{
	m_hInflictor = pInflictor;
#ifndef CLIENT_DLL
	if ( pInflictor != NULL && pInflictor->MyCombatCharacterPointer() != NULL && pInflictor->MyCombatCharacterPointer()->GetControllingModule() != NO_MODULE )
		m_iModule = pInflictor->MyCombatCharacterPointer()->GetControllingModule();
#endif
}
...
void AddMultiDamage( const CTakeDamageInfo &info, CBaseEntity *pEntity )
{
	...
	if ( g_MultiDamage.GetModule() == NO_MODULE )
		g_MultiDamage.SetModule( info.GetModule() );
