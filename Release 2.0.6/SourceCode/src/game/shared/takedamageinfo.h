...
public:
	...
	// if this was damage from a player's ability, there will be no inflictor,
	// instead the ability int will be >= 0 (-1 means off)
	int				GetModule() const;
...
protected:
	...
	int				m_iModule;				// the player ability used to create this damage, if there was one (-1 otherwise)
...
inline int CTakeDamageInfo::GetModule() const
{
	return m_iModule;
}


inline void CTakeDamageInfo::SetModule( int i )
{
	m_iModule = i;
}
