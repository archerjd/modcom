...
#ifdef USE_OMNIBOT
#ifndef CLIENT_DLL
#include "../../server/omnibot/omnibot_interface.h"
#endif
#endif
...
#ifndef CLIENT_DLL
int CMultiplayRules::m_nMapCycleTimeStamp = -1;
...
/*			break;
	...
	void CMultiplayRules::PlayerSpawn( CBasePlayer *pPlayer )
	{
		...
		if ( pHL2MPPlayer && !pHL2MPPlayer->IsInCharacter() )
			return; // don't give the suit if not logged on
	...
	void CMultiplayRules::DeathNotice( CBasePlayer *pVictim, const CTakeDamageInfo &info )
	{
		...
		else
		{
			...
			if ( pPlayerKiller )
				Omnibot::Notify_KilledSomeone(pPlayerKiller, pVictim, killer_weapon_name);
#endif
#endif
		}
	...
	*/
	...
	void CMultiplayRules::PopulateMapList()
	{
		...
		m_nMapCycleTimeStamp = filesystem->GetPathTime( mapcfile, "GAME" );
		...
		m_MapList.RemoveAll();
		//g_pMapListStringTable->RemoveAll(); // this method doesn't exist, therefore I'm removing the code to rescan the mapcycle file when it changes

		if ( 0 == m_nMapCycleTimeStamp )
		{
			...
			g_pMapListStringTable->AddString(true, szCurrentMapName);
		}
		else
		{
			...
			if ( aFileList && nFileLength )
			{
				...
				for ( int i = 0; i < m_MapList.Count(); i++ )
				{
					...
					else if ( i < 32 ) // don't overflow the string table
						g_pMapListStringTable->AddString(true, m_MapList[i]);
				}
		...
		if ( 0 == m_MapList.Count() )
		{
			...
			g_pMapListStringTable->AddString(true, szDefaultMapName);
		}
	}

	void CMultiplayRules::GetNextLevelName( char *pszNextMap, int bufsize, bool bRandom /* = false */ )
	{
		// disable rescanning the mapcycle file when it changes, as we can't remove maps from the networked map list
		/*const char *mapcfile = mapcyclefile.GetString();
		Assert( mapcfile != NULL );
	...
	{
		// remove all NPCs. Completely. Just in case the engine crash is related to AI and level changes.
