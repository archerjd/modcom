...
void CBaseCombatWeapon::Precache( void )
{
	...
	PrecacheParticleSystem("CritTracer");
}
...
/*	if ( GetOwner() && GetOwner()->IsPlayer() )
	{
		...
		switch ( pPlayer->GetFaction() )
		{
			case FACTION_COMBINE:
				if ( GetWpnData().bHasViewModelCombine )
					return GetWpnData().szViewModel_c;
				break;
			case FACTION_RESISTANCE:
				if ( GetWpnData().bHasViewModelResistance )
					return GetWpnData().szViewModel_r;
				break;
			case FACTION_APERTURE:
				if ( GetWpnData().bHasViewModelAperture )
					return GetWpnData().szViewModel_a;
				break;
		}
	}
*/	
...
/*	if ( GetOwner() && GetOwner()->IsPlayer() )
	{
		...
		{
			case FACTION_COMBINE:
				if ( GetWpnData().bHasWorldModelCombine )
					return GetWpnData().szWorldModel_c;
				break;
			case FACTION_RESISTANCE:
				if ( GetWpnData().bHasWorldModelResistance )
					return GetWpnData().szWorldModel_r;
				break;
			case FACTION_APERTURE:
				if ( GetWpnData().bHasWorldModelAperture )
					return GetWpnData().szWorldModel_a;
				break;
		}
	}
*/
...
// Purpose: 
//-----------------------------------------------------------------------------
int CBaseCombatWeapon::GetMaxClip2( void )
{
...
{
	int extra = 0;
	if ( GetOwner() && GetOwner()->IsPlayer() && GetMaxClip1() > 1 )
	{
		CHL2MP_Player *pPlayer = ToHL2MPPlayer(GetOwner());
		extra = GetExtraClip(pPlayer->GetModuleLevel(GetModule(CLIP_SIZE)));
	}

	return GetWpnData().iDefaultClip1 + extra;
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
int CBaseCombatWeapon::GetDefaultClip2( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::UsesClipsForAmmo2( void )
{
...
void CBaseCombatWeapon::DefaultTouch( CBaseEntity *pOther )
{
	...
	// characters with attrition active can't pick this up
	CBaseCombatCharacter *pBCC = pOther->MyCombatCharacterPointer();
	if ( pBCC && pBCC->IsBuffActive(DEBUFF_ATTRITION) )
		return;
...
void CBaseCombatWeapon::SendViewModelAnim( int nSequence )
{
	...
	if ( vm->ViewModelIndex() != m_nViewModelIndex )
		return; // desperate attempt to fix #279
...
#ifndef CLIENT_DLL
			// don't decloak for crossbow zooming
			if ( !ClassMatches("weapon_crossbow") )
			{
				pOwner->RemoveBuff(BUFF_SPAWN);
