...
//-----------------------------------------------------------------------------
int CBaseCombatCharacter::GetAmmoCount( int iAmmoIndex, bool considerWeaponClips ) const
{
	if ( iAmmoIndex == -1 )
		return 0;
	...
	// we only want this affecting what ammo we can pick up, don't screw with ammo display on the HUD or deciding whether we can reload!
	int ammoInWeaponClips = 0;
	if ( considerWeaponClips )
		for	(int i=0;i<MAX_WEAPONS;i++) 
		{
			CBaseCombatWeapon *pWeapon = m_hMyWeapons[i].Get();
			if ( pWeapon )
			{
				if ( pWeapon->m_iPrimaryAmmoType == iAmmoIndex && pWeapon->GetMaxClip1() > 0 )
					ammoInWeaponClips += pWeapon->m_iClip1;
				//else if ( pWeapon->m_iSecondaryAmmoType == iAmmoIndex && pWeapon->GetMaxClip2() > 0 )
					//ammoInWeaponClips += pWeapon->m_iClip2;
			}
		}
	return m_iAmmo[ iAmmoIndex ] + ammoInWeaponClips;
}

//-----------------------------------------------------------------------------
// Purpose: Returns the amount of ammunition of the specified type the character's carrying
//-----------------------------------------------------------------------------
int	CBaseCombatCharacter::GetAmmoCount( char *szName, bool considerWeaponClips ) const
{
	return GetAmmoCount( GetAmmoDef()->Index(szName), considerWeaponClips );
}
...
void CBaseCombatCharacter::EyeVectors( Vector *pForward, Vector *pRight, Vector *pUp )
{
	AngleVectors( GetAbsAngles(), pForward, pRight, pUp );
}
