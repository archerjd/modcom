...
bool CGameRules::CanHaveAmmo( CBaseCombatCharacter *pPlayer, int iAmmoIndex )
{
	if ( iAmmoIndex > -1 )
	{
		...
		if ( pPlayer->GetAmmoCount( iAmmoIndex, true ) < iMaxCarry )
			return true;
	}
...
bool CGameRules::IsSpawnPointValid( CBaseEntity *pSpot, CBasePlayer *pPlayer  )
{
	...
	if ( !pSpot ) //Always check for null pointers before testing them!!!
	{
		return false;
	}

	if ( pPlayer && !pSpot->IsTriggered( pPlayer ) )
	{
		return false;
	}
	...
	trace_t tr;
	UTIL_TraceHull(pSpot->GetAbsOrigin(), pSpot->GetAbsOrigin(), VEC_HULL_MIN, VEC_HULL_MAX, MASK_PLAYERSOLID, pPlayer, COLLISION_GROUP_PLAYER_MOVEMENT, &tr );
	if ( tr.fraction == 0.0f )
			return false;
