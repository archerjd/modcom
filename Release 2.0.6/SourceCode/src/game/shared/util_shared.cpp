...
bool CTraceFilterSimple::ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask )
{
	...
	if ( m_pPassEnt )
	{
		const CBaseEntity *pThis = EntityFromEntityHandle( m_pPassEnt );
		if ( pThis )
			playerIndex = ((CBaseEntity*)pThis)->GetPlayerIndex();
	}
	if ( !pEntity )
		return false;
	if ( !pEntity->ShouldCollide( m_collisionGroup, contentsMask, playerIndex ) )
		return false;
...
// Trace filter that can take a list of entities to ignore
//-----------------------------------------------------------------------------
CTraceFilterHitList::CTraceFilterHitList( int collisionGroup ) :
	CTraceFilterSimple( NULL, collisionGroup )
{
}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTraceFilterHitList::ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask )
{
	if ( m_HitEntities.Find(pHandleEntity) == m_HitEntities.InvalidIndex() )
		return false;
...
// Purpose: Add an entity to my list of entities to ignore in the trace
//-----------------------------------------------------------------------------
void CTraceFilterHitList::AddEntityToHit( IHandleEntity *pEntity )
{
	m_HitEntities.AddToTail( pEntity );
}
...
{
	// Cull it....
	if ( pentModel && pentModel->ShouldCollide( collisionGroup, MASK_ALL, 0 ) )
	{
		Ray_t ray;
