...
FileWeaponInfo_t::FileWeaponInfo_t()
{
	...
	//bHasViewModelResistance = bHasViewModelCombine = bHasViewModelAperture = false;
	//bHasWorldModelResistance = bHasWorldModelCombine = bHasWorldModelAperture = false;
}
...
void FileWeaponInfo_t::Parse( KeyValues *pKeyValuesData, const char *szWeaponName )
{
	...
	/*
	const char *pValue = pKeyValuesData->GetString( "viewmodel_r", "None" );
	if ( strcmp("None", pValue) == 0 )
	{	Q_strncpy( szViewModel_r, "", MAX_WEAPON_STRING ); bHasViewModelResistance = false; }
	else
	{	Q_strncpy( szViewModel_r, pValue, MAX_WEAPON_STRING ); bHasViewModelResistance = true; }
	
	pValue = pKeyValuesData->GetString( "viewmodel_c", "None" );
	if ( strcmp("None", pValue) == 0 )
	{	Q_strncpy( szViewModel_c, "", MAX_WEAPON_STRING ); bHasViewModelCombine = false; }
	else
	{	Q_strncpy( szViewModel_c, pValue, MAX_WEAPON_STRING ); bHasViewModelCombine = true; }
	
	pValue = pKeyValuesData->GetString( "viewmodel_a", "None" );
	if ( strcmp("None", pValue) == 0 )
	{	Q_strncpy( szViewModel_a, "", MAX_WEAPON_STRING ); bHasViewModelAperture = false; }
	else
	{	Q_strncpy( szViewModel_a, pValue, MAX_WEAPON_STRING ); bHasViewModelAperture = true; }
	*/
	...
	/*
	pValue = pKeyValuesData->GetString( "worldmodel_r", "None" );
	if ( strcmp("None", pValue) == 0 )
	{	Q_strncpy( szWorldModel_r, "", MAX_WEAPON_STRING ); bHasWorldModelResistance = false; }
	else
	{	Q_strncpy( szWorldModel_r, pValue, MAX_WEAPON_STRING ); bHasWorldModelResistance = true; }
	
	pValue = pKeyValuesData->GetString( "worldmodel_c", "None" );
	if ( strcmp("None", pValue) == 0 )
	{	Q_strncpy( szWorldModel_c, "", MAX_WEAPON_STRING ); bHasWorldModelCombine = false; }
	else
	{	Q_strncpy( szWorldModel_c, pValue, MAX_WEAPON_STRING ); bHasWorldModelCombine = true; }
	
	pValue = pKeyValuesData->GetString( "worldmodel_a", "None" );
	if ( strcmp("None", pValue) == 0 )
	{	Q_strncpy( szWorldModel_a, "", MAX_WEAPON_STRING ); bHasWorldModelAperture = false; }
	else
	{	Q_strncpy( szWorldModel_a, pValue, MAX_WEAPON_STRING ); bHasWorldModelAperture = true; }
	*/
